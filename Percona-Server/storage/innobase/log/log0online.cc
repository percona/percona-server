...
/*==============================*/
{
	...
	if (!os_file_set_eof_at(log_bmp_sys->out.file,
		...
		fprintf(stderr, "InnoDB: Warning: failed truncating "
			"changed page bitmap file \'%s\' to " UINT64PF
			" bytes\n",
					...
					current server startup */
{
	...
	fprintf(stderr, "InnoDB: last tracked LSN in \'%s\' is " LSN_PF
...
/*==================*/
{
	...
	if (!success) {
		...
		if (last_tracked_lsn > tracking_start_lsn) {
			...
			fprintf(stderr, "InnoDB: last tracked LSN is " LSN_PF
				", but last the checkpoint LSN is " LSN_PF ". "
	...
	const byte* log_block)	/*!< in: read log data */
{
	...
	if (!checksum_is_ok) {
		...
		fprintf(stderr,
			...
			"expected " ULINTPF ", calculated checksum " ULINTPF
			"\n",
	...
	lsn_t				range_end)	/*!<in: end LSN */
{
	...
	lsn_t		first_file_start_lsn	= IB_ULONGLONG_MAX;
