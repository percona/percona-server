...
/*==================================*/
{
	...
	result = (checksum_ok && is_last_page)
		? mach_read_ull(page + MODIFIED_PAGE_END_LSN) : 0;
...
/*==============================*/
{
	...
	log_bmp_sys->out.file
		= os_file_create(log_bmp_sys->out.name, OS_FILE_OVERWRITE,
...
/*======================*/
{
	...
	log_bmp_sys->out.file
		...
		(log_bmp_sys->out.name, OS_FILE_OPEN, OS_FILE_READ_WRITE,
		 &success);
	...
	log_bitmap_iterator_t *i) /*!<in/out: iterator */
{
	...
	i->start_lsn = mach_read_ull(i->page + MODIFIED_PAGE_START_LSN);
