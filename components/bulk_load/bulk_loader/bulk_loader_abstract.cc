...
const size_t parser_buffer_size = 1024;
...
void Bulk_loader_impl::set_string(Bulk_string type, std::string value) noexcept {
...
Bulk_compression_algorithm Bulk_loader_impl::get_compression_algorithm() const noexcept {
...
SERVICE_TYPE(bulk_data_convert) *Bulk_loader_impl::data_convert_service() noexcept {
  return reinterpret_cast<SERVICE_TYPE(bulk_data_convert) *>(m_svc_data_convert);
}
...
void *Bulk_loader_impl::start_session(size_t data_size, size_t se_memory_size, size_t num_threads) noexcept {
  m_load_session_start_done = true;
  auto *load_ctx = data_load_service()->begin(m_thd, m_table, data_size, se_memory_size, num_threads);
...
void Bulk_loader_impl::end_session(void *load_ctx) noexcept {
  ...
  if (!data_load_service()->end(m_thd, load_ctx, m_table, load_ctx == nullptr)) {
    LogComponentErr(ERROR_LEVEL, ER_BULK_LOADER_COMPONENT_ERROR,
...
bool Bulk_loader_impl::load(size_t &affected_rows [[maybe_unused]]) noexcept {
  ...
  std::cout << "Bulk_loader_local::load file_size: " << data_size << std::endl;
  std::cout << "Bulk_loader_local::load se_memory_size: " << se_memory_size << std::endl;

  load_ctx = start_session(data_size, se_memory_size, 1);
  ...
  if (!srv_data_convert->get_row_metadata(m_thd, m_table, false, row_metadata)) {
    LogComponentErr(ERROR_LEVEL, ER_BULK_LOADER_COMPONENT_ERROR,
                    "Failed to init row metadata");
    return false;
  }

  auto parser_params = get_parser_params();
  StreamParser stream_parser{row_metadata.m_num_columns,
                             std::move(parser_params),
                             data_stream.get()};

  // init buffers
  Rows_text text_rows{row_metadata.m_num_columns};
  text_rows.set_num_rows(1);
  size_t next_index = 0;

  Rows_mysql sql_rows{row_metadata.m_num_columns};
  char sql_rows_buffer[100] = "";
  size_t sql_rows_buffer_length = 100;
  Bulk_load_error_location_details error_details;

  // process data
  auto row_iterator = stream_parser.row_iterator();

  for (auto &row : row_iterator) {
    text_rows.process_columns(row.index, &row.getter);
  }

  auto ret = srv_data_convert->mysql_format(
      m_thd, m_table, text_rows, next_index, sql_rows_buffer,
      sql_rows_buffer_length, m_charset, row_metadata, sql_rows, error_details);

  std::cout << "Bulk_loader_local::load srv_data_convert->mysql_format_from_raw ret: " << ret << std::endl;
  std::cout << "Bulk_loader_local::load text_rows.get_num_rows: " << text_rows.get_num_rows() << std::endl;
  std::cout << "Bulk_loader_local::load text_rows.get_num_cols: " << text_rows.get_num_cols() << std::endl;
  std::cout << "Bulk_loader_local::load sql_rows.get_num_rows: " << sql_rows.get_num_rows() << std::endl;
  std::cout << "Bulk_loader_local::load sql_rows.get_num_cols: " << sql_rows.get_num_cols() << std::endl;
  ...
  if (!srv_data_load->load(m_thd, load_ctx, m_table, sql_rows, 0, wait_cbks)) {
    LogComponentErr(ERROR_LEVEL, ER_BULK_LOADER_COMPONENT_ERROR,
                    "Failed to load data");
    return false;
  }
