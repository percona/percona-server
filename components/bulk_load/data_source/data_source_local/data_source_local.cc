...
DataSourceImpl<Bulk_source::LOCAL>::DataSourceImpl(const std::string &path)
  : m_file_path{path} {}

DataSourceImpl<Bulk_source::LOCAL>::~DataSourceImpl() {
  m_stream.close();
}

bool DataSourceLocal::open() noexcept {
...
size_t DataSourceLocal::read(char *buffer, size_t size) noexcept {
