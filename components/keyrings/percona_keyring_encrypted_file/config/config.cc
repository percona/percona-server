...
/* Config names */
const std::string config_options[] = {"read_local_config", "path",
                                      "read_only",         "password",
                                      "password_file",     "iterations"};
...
bool find_and_read_config_file(std::unique_ptr<Config_pod> &config_pod,
  ...
  }
  {
    ...
    if (!config_reader->get_element<uint32_t>(config_options[5], iterations))
      config_pod->iterations_ = iterations;
  }
  return false;
error_custom:
  config_pod.reset();
...
bool create_config(
  ...
  /* Password_file */
  metadata.get()->push_back(std::make_pair(
      "Password_file", ((global_config_available == true)
                            ? ((config_pod.password_file_.length() == 0)
                                   ? "<NONE>"
                                   : config_pod.password_file_)
                            : "<NOT APPLICABLE>")));

  /* Iterations */
  metadata.get()->push_back(
      std::make_pair("Iterations", (global_config_available == true)
                                       ? std::to_string(config_pod.iterations_)
                                       : "<NOT APPLICABLE>"));
