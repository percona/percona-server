...
class gen_blocklist_impl {
 public:
  ...
  mysqlpp::udf_result_t<STRING_RESULT> calculate(
    ...
    {
      auto sresult =
          global_query_cache::instance().check_term_presence_in_dictionary(
              cs_dict_a_escaped, cs_term_escaped);
    ...
    auto sresult =
        global_query_cache::instance().select_random_term_for_dictionary(
            cs_dict_b_escaped);
...
class masking_dictionary_remove_impl {
 public:
  ...
  mysqlpp::udf_result_t<STRING_RESULT> calculate(
    ...
    if (!global_query_cache::instance().delete_for_dictionary(
            cs_dictionary_escaped)) {
      return std::nullopt;
    }
...
class masking_dictionary_term_add_impl {
 public:
  ...
  mysqlpp::udf_result_t<STRING_RESULT> calculate(
    ...
    if (!global_query_cache::instance().insert_ignore_record(
            cs_dictionary_escaped, cs_term_escaped)) {
      return std::nullopt;
    }
...
class masking_dictionary_term_remove_impl {
 public:
  ...
  mysqlpp::udf_result_t<STRING_RESULT> calculate(
    ...
    if (!global_query_cache::instance().delete_for_dictionary_and_term(
            cs_dictionary_escaped, cs_term_escaped)) {
      return std::nullopt;
    }
