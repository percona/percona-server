...
#include "components/masking_functions/include/udf/udf_gen_rnd_uuid.h"
...
static bool gen_rnd_uuid_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  DBUG_TRACE;

  if (args->arg_count != 0) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: gen_rnd_uuid()");
    return true;
  }
...
static void gen_rnd_uuid_deinit(UDF_INIT *initid) {
  DBUG_TRACE;
 ...
 */
static char *gen_rnd_uuid(UDF_INIT *initid, UDF_ARGS *args [[maybe_unused]],
                          char *result [[maybe_unused]], unsigned long *length,
                          char *, char *) {
  DBUG_TRACE;

  std::string uuid = mysql::plugins::random_uuid();
  *length = uuid.size();
  initid->ptr = new char[*length + 1];
  strcpy(initid->ptr, uuid.c_str());

  return initid->ptr;
}

udf_descriptor udf_gen_rnd_uuid() {
  return {"gen_rnd_uuid", Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(gen_rnd_uuid), gen_rnd_uuid_init,
          gen_rnd_uuid_deinit};
}
