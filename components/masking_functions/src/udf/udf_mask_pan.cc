...
static bool mask_pan_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  ...
  if (args->arg_count > 2) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: mask_pan(string, [mask_char])");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument type: mask_pan(string, [mask_char])");
    return true;
  }
...
static const char *mask_pan(UDF_INIT *initid, UDF_ARGS *args,
  ...
  const std::string sresult = mysql::plugins::mask_inner(
      args->args[0], args->lengths[0], 0, 4, original_charset, masking_char);
