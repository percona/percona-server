...
static bool mask_pan_relaxed_init(UDF_INIT *initid, UDF_ARGS *args,
  ...
  if (args->arg_count > 2) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: mask_pan_relaxed(string, [mask_char])");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument type: mask_pan_relaxed(string, [mask_char])");
    return true;
  }

  if (args->lengths[0] < 14 || args->lengths[0] > 19) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument: PAN must be between 14 and 19 characters");
    return true;
  }
...
static const char *mask_pan_relaxed(UDF_INIT *initid, UDF_ARGS *args,
  ...
  const std::string sresult = mysql::plugins::mask_inner(
      args->args[0], args->lengths[0], 6, 4, original_charset, masking_char);
