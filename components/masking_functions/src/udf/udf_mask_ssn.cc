...
static bool mask_ssn_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  ...
  if (args->arg_count > 2) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: mask_ssn(string, [mask_char])");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument type: mask_ssn(string, [mask_char])");
    return true;
  }

  if (args->lengths[0] != 9 && args->lengths[0] != 11) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument: SSN must be specified either as nine "
                  "digits, or in the format xxx-xx-xxxxx");
    return true;
  }
...
static const char *mask_ssn(UDF_INIT *initid, UDF_ARGS *args,
  ...
  if (args->lengths[0] == 11) {
    sresult = mysql::plugins::mask_inner(args->args[0], args->lengths[0], 4, 5,
                                         original_charset, masking_char);
    sresult = mysql::plugins::mask_inner(sresult.c_str(), sresult.size(), 0, 8,
                                         original_charset, masking_char);
  } else {
    sresult = mysql::plugins::mask_inner(args->args[0], args->lengths[0], 0, 4,
                                         original_charset, masking_char);
  }
