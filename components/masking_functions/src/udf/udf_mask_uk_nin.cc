...
static bool mask_uk_nin_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  DBUG_TRACE;

  if (args->arg_count > 2) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: mask_uk_nin(string, [mask_char])");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument type: mask_uk_nin(string, [mask_char])");
    return true;
  }

  if (args->lengths[0] < 9 || args->lengths[0] > 11) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument: NIN must be between 9 and 11 characters");
    return true;
  }
...
static void mask_uk_nin_deinit(UDF_INIT *initid) {
  DBUG_ENTER("mask_uk_nin_deinit");
 ...
 */
static const char *mask_uk_nin(UDF_INIT *initid, UDF_ARGS *args,
...
udf_descriptor udf_mask_uk_nin() {
  return {"mask_uk_nin", Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(mask_uk_nin), mask_uk_nin_init,
          mask_uk_nin_deinit};
}
