...
static bool mask_uuid_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  DBUG_TRACE;

  if (args->arg_count > 2) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument list: mask_uuid(string, [mask_char])");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument type: mask_uuid(string, [mask_char])");
    return true;
  }

  if (args->lengths[0] != 36) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "Wrong argument: UUID must be 36 characters");
    return true;
  }
...
static void mask_uuid_deinit(UDF_INIT *initid) {
  DBUG_TRACE;
 ...
 */
static const char *mask_uuid(UDF_INIT *initid, UDF_ARGS *args,
  ...
  sresult = mysql::plugins::mask_inner(args->args[0], args->lengths[0], 0,
                                       36 - 8, original_charset, masking_char);
  sresult =
      mysql::plugins::mask_inner(sresult.c_str(), sresult.size(), 9, 36 - 9 - 4,
  ...
  sresult = mysql::plugins::mask_inner(sresult.c_str(), sresult.size(),
                                       ...
                                       original_charset, masking_char);
  sresult =
      mysql::plugins::mask_inner(sresult.c_str(), sresult.size(), 9 + 5 + 5 + 5,
                                 0, original_charset, masking_char);
...
udf_descriptor udf_mask_uuid() {
  return {"mask_uuid", Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(mask_uuid), mask_uuid_init,
          mask_uuid_deinit};
}
