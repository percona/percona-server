...
static bool masking_dictionary_term_remove_init(UDF_INIT *initid,
                                                UDF_ARGS *args, char *message) {
  DBUG_TRACE;

  if (!mysql::components::have_masking_admin_privilege()) {
    std::snprintf(message, MYSQL_ERRMSG_SIZE,
                  "masking_dictionary_term_remove requires the "
  ...
  if (args->arg_count != 2) {
    std::snprintf(
        ...
        "Wrong argument list: masking_dictionary_term_remove(dictionary "
        "name, term)");
    return true;
  }
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    std::snprintf(
        message, MYSQL_ERRMSG_SIZE,
        "Wrong argument type: masking_dictionary_term_remove(string)");
    return true;
  }
...
static void masking_dictionary_term_remove_deinit(UDF_INIT *initid) {
  DBUG_TRACE;

  if (initid->ptr) delete[] initid->ptr;
}

static char *masking_dictionary_term_remove(UDF_INIT *initid, UDF_ARGS *args,
...
udf_descriptor udf_masking_dictionary_term_remove() {
  return {"masking_dictionary_term_remove", Item_result::STRING_RESULT,
          reinterpret_cast<Udf_func_any>(masking_dictionary_term_remove),
          masking_dictionary_term_remove_init,
          masking_dictionary_term_remove_deinit};
}
