...
:ref:`expand_fast_index_creation` may also optimize index access for
...
:ref:`expand_fast_index_creation`. It is a session variable, so you can
...
* ``ALTER TABLE`` and ``OPTIMIZE TABLE`` always process tables containing
  foreign keys as if :ref:`expand_fast_index_creation` is OFF to avoid
