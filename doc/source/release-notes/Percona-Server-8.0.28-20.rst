...
* :jirabug:`PS-7044`: Implements support for encryption user-defined functions (UDFs) for OpenSSL.
* :jirabug:`PS-7672`: Implements support for the Amazon Key Management Service component in Percona Server for MySQL.
* :jirabug:`PS-7748`: To ensure that log messages are not lost if a server shuts down or exits, log messages are written to a memory buffer.
