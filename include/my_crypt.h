...
[[nodiscard]] int my_aes_crypt_init(MyEncryptionCTX *&ctx, enum my_aes_mode mode,
                                   int flags, const unsigned char *key,
                                   size_t klen, const unsigned char *iv,
                                   size_t ivlen);
[[nodiscard]] int my_aes_crypt_update(MyEncryptionCTX *ctx,
                                     const unsigned char *src, size_t slen,
                                     unsigned char *dst, size_t *dlen) noexcept;
[[nodiscard]] int my_aes_crypt_finish(MyEncryptionCTX *&ctx, uchar *dst,
                                     size_t *dlen);
void my_aes_crypt_free_ctx(MyEncryptionCTX *ctx) noexcept;

[[nodiscard]] int my_aes_crypt(enum my_aes_mode mode, int flags,
                              const unsigned char *src, size_t slen,
                              unsigned char *dst, size_t *dlen,
                              const unsigned char *key, size_t klen,
                              const unsigned char *iv, size_t ivlen);

[[nodiscard]] int my_random_bytes(unsigned char *buf, int num) noexcept;
[[nodiscard]] size_t my_aes_crypt_get_size(enum my_aes_mode mode,
