...
class MyRcuLock {
  ...
  */
  ~MyRcuLock() { write_wait_and_delete(NULL); }
  ...
  */
  class ReadLock {
   ...
   private:
    ...
    MyRcuLock operator=(const ReadLock) {};
  };
  ...
  */
  const T *rcu_read() {
    rcu_readers_.fetch_add(1, boost::memory_order_relaxed);
    return rcu_global_.load(boost::memory_order_relaxed);
  }
  ...
  */
  void rcu_end_read() { rcu_readers_.fetch_sub(1, boost::memory_order_relaxed); }
  ...
  */
  const T *rcu_write(const T *newT) {
    return rcu_global_.exchange(newT, boost::memory_order_release);
  }
  ...
  */
  bool wait_for_no_readers() {
    bool stopped = false;
    while (rcu_readers_.load(boost::memory_order_relaxed) > 0)
      ;
    return stopped;
  }
 ...
 protected:
  ...
  boost::atomic<const T *> rcu_global_;
  /** padding to break the CPU cache lines */
  char rcu_padding_[128];
  /** the readers counter */
  boost::atomic<long> rcu_readers_;

 private:
  /** disabled */
  MyRcuLock(const MyRcuLock &) {};
  /** disabled */
  void operator=(const MyRcuLock &) {};
};
#endif  //MY_RCU_LOCK_INCLUDED
