...
enum_binlog_checksum_alg Log_event_footer::get_checksum_alg(const char *buf,
  ...
  BAPI_ASSERT(static_cast<unsigned char>(buf[EVENT_TYPE_OFFSET]) ==
...
/**
  ...
  @param binlog_version       the binlog_version. It is unused now, since only
...
Log_event_header::Log_event_header(
  ...
  when.tv_usec = 0;
  type_code = static_cast<Log_event_type>(
      static_cast<unsigned char>(buf[EVENT_TYPE_OFFSET]));
