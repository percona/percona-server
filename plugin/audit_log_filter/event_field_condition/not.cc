...
EventFieldConditionNot::EventFieldConditionNot(
    ...
    : EventFieldConditionBase{action}, m_condition{std::move(condition)} {}

AuditAction EventFieldConditionNot::check_applies(
    const AuditRecordFieldsList &fields) const noexcept {
  if (m_condition->check_applies(fields) == get_match_action()) {
