...
#include "plugin/audit_log_filter/log_writer_strategy/perf.h"
#include "plugin/audit_log_filter/log_writer/file_handle.h"

namespace audit_log_filter::log_writer_strategy {

bool FileWriterStrategyPerformace::do_open_file(
  ...
  return file_handle->init_buffer(file_buffer_size, /*drop_if_full*/ true);
}

bool FileWriterStrategyPerformace::do_close_file(
    log_writer::FileHandle *file_handle) noexcept {
  file_handle->close_buffer();
  return file_handle->close_file();
}

void FileWriterStrategyPerformace::do_write(
    log_writer::FileHandle *file_handle, const std::string &record) noexcept {
