...
/** Check if a MeCab node should be filtered out for FTS.
@param[in]	node	MeCab node to check
@param[in]	cs	character set info
@param[in]	doc_end	document end pointer
@retval	true	if node should be filtered (skipped)
@retval	false	if node should be indexed */
static inline bool mecab_should_filter_node(const MeCab::Node *node,
                                             const CHARSET_INFO *cs,
                                             const char *doc_end) {
  /* Filter BOS/EOS virtual nodes */
  if (node->stat == MECAB_BOS_NODE || node->stat == MECAB_EOS_NODE) {
    return true;
  }

  /* Use MeCab-ko's POS feature for efficient filtering.
     We only need the first field (POS tag). */
  const char *feature = node->feature;
  if (feature == nullptr || feature[0] == '\0') {
    return false;  /* Keep unknown features */
  }

  /* Filter symbol tokens (punctuation) based on POS tags from mecab-ko-dic:
     - SF: 마침표 (period, exclamation, question mark)
     - SC: 쉼표, 콜론 등 (comma, colon, semicolon)
     - SE: 줄임표 (ellipsis)
     - SSO: 여는 괄호 (opening parenthesis)
     - SSC: 닫는 괄호 (closing parenthesis)
     - SY: 기타 기호 (other symbols)

     Keep these meaningful symbols:
     - SL: 외국어 (foreign words)
     - SH: 한자 (Chinese characters)
     - SN: 숫자 (numbers) */
  if (feature[0] == 'S' && (feature[1] == 'F' || feature[1] == 'C' ||
                             feature[1] == 'E' || feature[1] == 'Y' ||
                             (feature[1] == 'S' && (feature[2] == 'O' || feature[2] == 'C')))) {
    return true;
  }

  /* Fallback: filter control characters */
  int ctype = 0;
  cs->cset->ctype(cs, &ctype, reinterpret_cast<const uchar *>(node->surface),
                  reinterpret_cast<const uchar *>(doc_end));
...
static int mecab_parse(MeCab::Lattice *mecab_lattice,
  ...
  /* Count meaningful tokens for phrase conversion in boolean mode */
  if (param->mode == MYSQL_FTPARSER_FULL_BOOLEAN_INFO) {
    for (const MeCab::Node *node = mecab_lattice->bos_node(); node != nullptr;
         node = node->next) {
      if (!mecab_should_filter_node(node, cs, doc_end)) {
        token_num++;
      }
    }
  ...
  for (const MeCab::Node *node = mecab_lattice->bos_node(); node != nullptr;
    ...
    /* Calculate position as byte offset from document start.
       Position accuracy is critical for phrase matching. */
    bool_info->position = static_cast<int>(node->surface - doc_start);
    ...
    case MYSQL_FTPARSER_FULL_BOOLEAN_INFO:
      ...
      while (fts_get_word(param->cs, extra_word_chars, &start, end, &word,
        ...
        if (bool_info.type == FT_TOKEN_WORD) {
          ...
          if (!bool_info.trunc) {
            /* Check if this is phrase search or term search.
               When bool_info.quot != nullptr, it means we're inside quotes,
               so this is phrase search and tokens must be consecutive. */
            ret = mecab_parse(mecab_lattice, param,
                              reinterpret_cast<char *>(word.pos), word.len,
                              &bool_info);
          } else {
            /* Wildcard search - pass through as is */
            ret = param->mysql_add_word(param,
                                        reinterpret_cast<char *>(word.pos),
                                        word.len, &bool_info);
          }
        } else {
          /* FT_TOKEN_LEFT_PAREN, FT_TOKEN_RIGHT_PAREN, etc. - pass through */
...
mysql_declare_plugin(mecab_custom){
    MYSQL_FTPARSER_PLUGIN,                 /*!< type	*/
    &mecab_parser_descriptor,              /*!< descriptor	*/
    "mecab_custom",                               /*!< name	*/
    PLUGIN_AUTHOR_ORACLE,                  /*!< author	*/
    "Custom Mecab Full-Text Parser", /*!< description*/
