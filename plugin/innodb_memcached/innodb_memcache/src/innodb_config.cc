...
static bool innodb_config_parse_value_col(
    /*==========================*/
    meta_cfg_info_t *item, /*!< in: meta info structure */
    ...
    int len)               /*!< in: length of above string */
{
  ...
  for (column_str = strtok_r(my_str, sep, &last); column_str;
  ...
  if (num_cols > 1) {
    ...
    for (column_str = strtok_r(my_str, sep, &last); column_str;
      ...
      item->extra_col_info[i].col_name =
          my_strdupl(column_str, item->extra_col_info[i].col_name_len);
...
/**********************************************************************/ /**
 ...
 @return true if everything works out fine */
static bool innodb_read_cache_policy(
    /*=====================*/
    meta_cfg_info_t *item, /*!< in: meta info structure */
    void *thd)             /*!< in/out: MySQL THD */
{
  ...
  err = innodb_api_begin(NULL, MCI_CFG_DB_NAME, MCI_CFG_CACHE_POLICIES, NULL,
                         ib_trx, &crsr, &idx_crsr, IB_LOCK_S);

  if (err != DB_SUCCESS) {
    fprintf(stderr,
            " InnoDB_Memcached: Cannot open config table"
            "'%s' in database '%s'. Error %d\n",
            MCI_CFG_CACHE_POLICIES, MCI_CFG_DB_NAME, err);
    err = DB_ERROR;
    goto func_exit;
  }
  ...
  for (i = 0; i < CACHE_POLICY_NUM_COLS; ++i) {
    ...
    if (opt_val >= META_CACHE_NUM_OPT || opt_val < META_CACHE_OPT_INNODB) {
      fprintf(stderr,
...
/**********************************************************************/ /**
 ...
 table and column info that used for memcached data
 @return true if everything works out fine */
static bool innodb_read_config_option(
    /*======================*/
    meta_cfg_info_t *item, /*!< in: meta info structure */
    void *thd)             /*!< in/out: MySQL THD */
{
  ...
  ib_trx = ib_cb_trx_begin(IB_TRX_READ_COMMITTED, true, false, thd);
  err = innodb_api_begin(NULL, MCI_CFG_DB_NAME, MCI_CFG_CONFIG_OPTIONS, NULL,
                         ib_trx, &crsr, &idx_crsr, IB_LOCK_S);
    ...
    for (i = 0; i < CONFIG_OPT_NUM_COLS; ++i) {
      ...
      if (i == CONFIG_OPT_KEY) {
        ...
        for (j = 0; j < OPTION_ID_NUM_OPTIONS; j++) {
          ...
          "separator" */
          if (strcmp(key, config_option_names[j].name) == 0) {
            current_option = config_option_names[j].id;
            break;
          }
        }
      }
      ...
      if (i == CONFIG_OPT_VALUE && current_option >= 0) {
        ...
        MAX_DELIMITER_LEN */
        max_len = (data_len > MAX_DELIMITER_LEN) ? MAX_DELIMITER_LEN : data_len;

        memcpy(item->options[current_option].value,
               innodb_cb_col_get_value(tpl, i), max_len);

        item->options[current_option].value[max_len] = 0;

        item->options[current_option].value_len = max_len;
      }
    }
...
static meta_cfg_info_t *innodb_config_add_item(
    /*===================*/
    ...
    void *thd)                   /*!< in/out: MySQL THD */
{
  ...
  if (n_cols < CONTAINER_NUM_COLS) {
    fprintf(stderr,
            ...
            MCI_CFG_CONTAINER_TABLE, MCI_CFG_DB_NAME, n_cols,
            CONTAINER_NUM_COLS);
    err = DB_ERROR;
    goto func_exit;
  }
  ...
  for (i = 0; i < CONTAINER_NUM_COLS; ++i) {
    ...
    item->col_info[i].col_name =
        my_strdupl((char *)innodb_cb_col_get_value(tpl, i), data_len);

    item->col_info[i].field_id = -1;

    if (i == CONTAINER_VALUE) {
      innodb_config_parse_value_col(item, item->col_info[i].col_name, data_len);
    }
  }
    ...
    void *thd)               /*!< in/out: MySQL THD */
{
  ...
  ib_trx = ib_cb_trx_begin(IB_TRX_READ_COMMITTED, true, false, thd);
  err = innodb_api_begin(NULL, MCI_CFG_DB_NAME, MCI_CFG_CONTAINER_TABLE, NULL,
                         ib_trx, &crsr, &idx_crsr, IB_LOCK_S);
  ...
  while (err == DB_SUCCESS) {
    ...
    if (default_item == NULL ||
        (item && strcmp(item->col_info[0].col_name, "default") == 0)) {
      default_item = item;
    }
...
/**********************************************************************/ /**
 ...
 table and column info that used for memcached data
 @return true if everything works out fine */
static meta_cfg_info_t *innodb_config_container(
    /*====================*/
    ...
    void *thd)               /*!< in/out: MySQL THD */
{
  ...
  if (err != DB_SUCCESS) {
    fprintf(stderr,
            ...
            MCI_CFG_CONTAINER_TABLE, MCI_CFG_DB_NAME, err);
    err = DB_ERROR;
    goto func_exit;
  }
  ...
  for (i = 0; i < CONTAINER_NUM_COLS; ++i) {
    ...
    item->col_info[i].col_name =
        my_strdupl((char *)innodb_cb_col_get_value(read_tpl, i), data_len);
    ...
    meta_column_t *col_verify)  /*!< in: verify structure */
{
  ...
  if (!meta_info->n_extra_col) {
    ...
    if (strcmp(name, cinfo[CONTAINER_VALUE].col_name) == 0) {
      if (col_meta->type != IB_VARCHAR && col_meta->type != IB_CHAR &&
          col_meta->type != IB_BLOB && col_meta->type != IB_CHAR_ANYCHARSET &&
          col_meta->type != IB_VARCHAR_ANYCHARSET && col_meta->type != IB_INT) {
        fprintf(stderr,
    ...
    for (i = 0; i < meta_info->n_extra_col; i++) {
      if (strcmp(name, meta_info->extra_col_info[i].col_name) == 0) {
        ...
        meta_info->col_info[CONTAINER_VALUE].field_id = col_id;
        meta_info->col_info[CONTAINER_VALUE].col_meta = *col_meta;
    ...
    bool runtime)          /*!< in: verify at the runtime */
{
  ...
  if (runtime && info->n_extra_col) {
    col_verify =
        (meta_column_t *)malloc(info->n_extra_col * sizeof(meta_column_t));
  ...
  for (i = 0; i < n_cols; i++) {
    ...
    result =
        innodb_config_value_col_verify(name, info, &col_meta, i, col_verify);
    ...
    if (strcmp(name, cinfo[CONTAINER_KEY].col_name) == 0) {
      ...
      if (col_meta.type != IB_VARCHAR && col_meta.type != IB_CHAR &&
          col_meta.type != IB_VARCHAR_ANYCHARSET &&
          col_meta.type != IB_CHAR_ANYCHARSET && col_meta.type != IB_INT) {
        fprintf(stderr,
  ...
  if (info->n_extra_col) {
    ...
    col_check = (runtime && col_verify) ? col_verify : info->extra_col_info;
...
bool innodb_verify(
    /*==========*/
    meta_cfg_info_t *info, void *thd) {
  ib_crsr_t crsr = NULL;
  char table_name[MAX_TABLE_NAME_LEN + MAX_DATABASE_NAME_LEN];
  char *dbname;
  char *name;
  ib_err_t err = DB_SUCCESS;
  ib_trx_t ib_trx = ib_cb_trx_begin(IB_TRX_READ_COMMITTED, false, false, thd);
  ...
  if (ib_cb_is_virtual_table(crsr)) {
    fprintf(stderr,
            " InnoDB_Memcached: table '%s' cannot"
