...
System_keys_container::~System_keys_container() {
  for (System_key_id_to_system_key::iterator iter =
           system_key_id_to_system_key.begin();
       iter != system_key_id_to_system_key.end(); ++iter)
    delete iter->second;
}
...
IKey *System_keys_container::get_latest_key_if_system_key_without_version(
    IKey *key) {
...
bool System_keys_container::parse_system_key_id_with_version(
    std::string &key_id, std::string &system_key_id, uint &key_version) {
  ...
  const std::string version =
      key_id.substr(colon_position + 1, key_id.length() - colon_position);
...
bool System_keys_container::is_system_key_without_version(IKey *key) noexcept {
  return key->get_user_id()->empty() &&
         key->get_key_id()->compare(0, system_key_prefix.length(),
                                    system_key_prefix) == 0 &&
         key->get_key_id()->find_first_of(':') ==
             std::string::npos;  // system keys cannot have ':' in their name
}

bool System_keys_container::is_system_key_with_version(
    IKey *key, std::string &system_key_id, uint &key_version) {
  return key->get_user_id()->empty() &&
         !parse_system_key_id_with_version(*key->get_key_id(), system_key_id,
                                           key_version) &&
         key->get_key_id()->compare(0, system_key_prefix.length(),
                                    system_key_prefix) == 0;
}

void System_keys_container::update_system_key(IKey *key,
                                              const std::string &system_key_id,
                                              uint key_version) {
  if (system_key_id_to_system_key[system_key_id]->get_key_version() <
      key_version)
    system_key_id_to_system_key[system_key_id]->set_keyring_key(key,
                                                                key_version);
}

bool System_keys_container::rotate_key_id_if_system_key_without_version(
    IKey *key) {
  if (!is_system_key_without_version(key)) return false;

  uint key_version =
     1;  // if we rotate from plain system key, we assign version 1 to it
  if (system_key_id_to_system_key.count(*key->get_key_id()) != 0) {
    ...
    if (key_version == UINT_MAX) {
      logger->log(MY_ERROR_LEVEL,
                  "System key cannot be rotated anymore, "
...
void System_keys_container::store_or_update_if_system_key_with_version(
    IKey *key) {
  std::string system_key_id;
  uint key_version;

  if (is_system_key_with_version(key, system_key_id, key_version)) {
    if (system_key_id_to_system_key.count(system_key_id) ==
        0)  // add a new system key
      system_key_id_to_system_key.emplace(
          system_key_id, new System_key_adapter(key_version, key));
    else
