...
static void *generate(void *arg)
{
  ...
  for (uint i= 0; my_atomic_load32(&number_of_keys_generated) <
       ...
       i= (i + 1) % number_of_keys_to_generate)
  {
    ...
    if (verbose)
    {
      ...
      std::cout << "Key generated " << key_id << ' ' << key_type << ' '
                << user << ' ';
...
int main(int argc, char **argv)
{
  ...
  if (generate_credential_file(credential_file_name, mount_point,
  ...
  BOOST_SCOPE_EXIT(void)
  {
    ...
    for (std::set<Key_in_keyring>::const_iterator iter=
             keys_in_keyring.begin();
