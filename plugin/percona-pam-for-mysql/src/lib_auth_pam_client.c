...
int authenticate_user_with_pam_client_common(
    MYSQL_PLUGIN_VIO *vio, struct st_mysql *mysql __attribute__((unused)),
    prompt_fn echoless_prompt_fn, prompt_fn echo_prompt_fn,
    info_fn show_error_fn, info_fn show_info_fn) {
  do {
    char *buf;
    int pkt_len;
    ...
    if (buf[0] == '\2' || buf[0] == '\3') {
      ...
      char *reply = (buf[0] == '\2') ? echoless_prompt_fn(&buf[1])
                                     : echo_prompt_fn(&buf[1]);
