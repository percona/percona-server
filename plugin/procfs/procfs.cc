...
static ST_FIELD_INFO procfs_view_fields[] = {
    ...
    {"CONTENTS", IS_PROCFS_CONTENTS_SIZE, MYSQL_TYPE_STRING, 0, MY_I_S_UNSIGNED,
     0, 0},
...
static std::vector<std::string> limited_glob_files(std::string path,
  ...
  if (max_results <= 0) return files_found;

  DIR *dir = opendir(path.c_str());
  struct dirent *dir_entry;

  if (!dir) return files_found;
...
static std::vector<std::string> limited_glob(std::string query, int max_results,
                                             int max_depth) {
  std::vector<std::string> files_found;

  if (max_results <= 0 || max_depth <= 0) return files_found;
