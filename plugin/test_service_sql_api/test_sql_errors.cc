...
static void get_data_str(struct st_plugin_ctx *pctx)
{
  ...
  for (uint col= 0; col < pctx->num_cols; col++)
  {
    WRITE_VAL("%s(%u) ", fieldtype2str(pctx->sql_field[col].type), pctx->sql_field[col].type);
  }
  WRITE_STR("\n");
...
static void exec_test_cmd(MYSQL_SESSION session, const char *test_cmd, void *p, void * ctx)
{
  ...
  if (fail)
  {
    if (!srv_session_close(session))
      my_plugin_log_message(&p, MY_ERROR_LEVEL, "test_sql_2_sessions - ret code : %d", fail);
  }
  else
  {
    if (pctx->num_cols)
...
/* Disabled open without close as these 5 sessions stay open until server shutdow, That
   violates the rules of a valid regression test. */
  ...
  for (int i= 0; i < nb_sessions; i++)
  {
    ...
    {
      my_plugin_log_message(&p, MY_INFORMATION_LEVEL, "Opened session %d .", i);
    }
  }
