...
static struct st_test_statement test_query_plan[]=
{
  ...
  { NULL , true , "SELECT SPECIFIC_NAME FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = 'proc'" },
  // Function
  {"test", true , "SELECT avg_func(10, 20)" },
  { NULL , true , "SELECT SPECIFIC_NAME FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'FUNCTION' AND ROUTINE_NAME = 'avg_func'" }
};
...
{
  struct st_plugin_ctx *pctx= (struct st_plugin_ctx*) ctx;//  WRITE_STR("sql_start_row\n");
...
static ulong sql_get_client_capabilities(void *ctx)
{
  ...
  DBUG_RETURN(CLIENT_MULTI_RESULTS);
}
