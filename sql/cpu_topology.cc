...
through a shared Sql_cpu_topology snapshot that can be reused
*******************************************************/
...
Sql_cpu_topology sql_cpu_topology;
...
sql_cpu_get_logical() {
  ...
  if (si.dwNumberOfProcessors > 0) {
    n = (unsigned long) si.dwNumberOfProcessors;
  }
#elif defined(__APPLE__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__)
  int mib[2];
  int cores = 0;
...
void
sql_cpu_topology_init(Sql_cpu_topology *topology) {
  if (topology == nullptr) {
    return;
  }
  ...
  topology->logical_cpus = sql_cpu_get_logical();
  ...
  if (dir != nullptr) {
    ...
    std::map<int, std::map<int, Sql_cpu_core> > socket_cores;
    struct dirent *entry;
    ...
    while ((entry = readdir(dir)) != nullptr) {
      ...
      Sql_cpu_core &core_ref = cores_map[core_id];

      if (core_ref.threads.empty()) {
        core_ref.core_id = core_id;
      }

      Sql_cpu_thread thread;
    ...
    for (auto &s_pair : socket_cores) {
      ...
      std::sort(socket.cores.begin(), socket.cores.end(),
                [](const Sql_cpu_core &a,
                   const Sql_cpu_core &b) {
    ...
    std::sort(topology->sockets.begin(), topology->sockets.end(),
              [](const Sql_cpu_socket &a,
                 const Sql_cpu_socket &b) {
    ...
    if (!topology->logical_ids.empty()) {
      topology->logical_cpus =
        static_cast<unsigned long>(topology->logical_ids.size());
    }
    ...
    for (const auto &socket : topology->sockets) {
      topology->physical_cores +=
        static_cast<unsigned long>(socket.cores.size());
    }
  }
#endif /* __linux__ */
  ...
  if (topology->physical_cores == 0) {
    unsigned long logical = topology->logical_cpus;

    if (logical == 0) {
      sql_print_warning("sql_cpu_topology_init(): logical CPU count is 0; cannot determine physical cores.");
    } else {
      unsigned long physical = logical;
      if (logical % 2 == 0) {
        physical = logical / 2;
      }
      ...
      sql_print_warning(
       "sql_cpu_topology_init(): using heuristic estimate: logical CPUs=%lu, physical cores (estimated)=%lu",
        (unsigned long) logical, (unsigned long) topology->physical_cores);
    }
  }
...
bool
sql_cpu_get_core_by_index(unsigned long index, Sql_cpu_core *out) {
  sql_cpu_topology_init(&sql_cpu_topology);
