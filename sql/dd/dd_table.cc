  ...
  for (int key_nr = 1; key != end; ++key, ++key_nr) {
    ...
    */
    ...
    if (key->flags & HA_VECTOR) {
      // Vector indexes must have vector_index_type set.
      assert(key->vector_index_type.length > 0);

      idx_options->set(dd::PERCONA_VECTOR_INDEX_MARKER_KEY, "1");
      idx_options->set(dd::PERCONA_VECTOR_INDEX_TYPE_KEY,
                       dd::String_type(key->vector_index_type.str,
                                       key->vector_index_type.length));

      if (!key->vector_index_params.empty()) {
        ...
        if (idx_options->set(PERCONA_VECTOR_INDEX_PARAMS_KEY,
