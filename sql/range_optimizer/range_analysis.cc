...
/**
  Try to extract the Field and constant value from a MULT_EQUAL predicate.

  Constant propagation rewrites simple col = const to MULT_EQUAL during
  optimization. Subquery conditions (col = (SELECT ...)) are not rewritten,
  but that narrow case is out of scope for the oversized-value check.

  @param func       The predicate to extract from.
  @param out_field  Set to the Field on success.
  @param out_value  Set to the constant value on success.
