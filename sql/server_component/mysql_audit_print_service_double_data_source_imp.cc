...
DEFINE_BOOL_METHOD(mysql_audit_print_service_double_data_source_imp::get,
                   (MYSQL_THD thd, my_h_string name, double *out)) {
  *out = 0;
  ...
  if (EQUALS("query_time")) {
    ...
    *out = static_cast<double>(thd->get_examined_row_count());
  } else if (EQUALS("bytes_received")) {
    if (!thd->copy_status_var_ptr) return true;
    *out = static_cast<double>(thd->status_var.bytes_received -
                               thd->copy_status_var_ptr->bytes_received);
  } else if (EQUALS("bytes_sent")) {
    if (!thd->copy_status_var_ptr) return true;
    *out = static_cast<double>(thd->status_var.bytes_sent -
