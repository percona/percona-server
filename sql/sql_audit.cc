  ...
  if (thd->lex->sql_command == SQLCOM_END && msg_len > 0 && error_code == 0)
  {
    ...
    else
    {
      ...
      case COM_STMT_RESET:
        ...
        break;
      default:
        std::string cmd_class= msg;
        std::transform(cmd_class.begin(), cmd_class.end(), cmd_class.begin(), ::tolower);
        MYSQL_LEX_CSTRING command_class= { C_STRING_WITH_LEN(cmd_class.c_str()) } ;
