...
int mysql_create_zip_dict(THD *thd, const char *name, ulong name_len,
  ...
  ha_create_zip_dict_result create_result = hton->create_zip_dict(
      hton, thd, name, &local_name_len, data, &local_data_len);
  if (create_result != ha_create_zip_dict_result::OK) {
    switch (create_result) {
      case ha_create_zip_dict_result::NAME_TOO_LONG:
        error = ER_COMPRESSION_DICTIONARY_NAME_TOO_LONG;
        my_error(error, MYF(0), name, local_name_len);
        break;
      case ha_create_zip_dict_result::DATA_TOO_LONG:
        error = ER_COMPRESSION_DICTIONARY_DATA_TOO_LONG;
        my_error(error, MYF(0), name, local_data_len);
        break;
      case ha_create_zip_dict_result::ALREADY_EXISTS:
        if (if_not_exists) {
          push_warning_printf(
        ...
        break;
      case ha_create_zip_dict_result::READ_ONLY:
        error = ER_READ_ONLY_MODE;
        my_error(error, MYF(0));
        break;
      case ha_create_zip_dict_result::OUT_OF_MEMORY:
        error = ER_OUT_OF_RESOURCES;
        my_error(error, MYF(0));
        break;
      case ha_create_zip_dict_result::OUT_OF_FILE_SPACE:
        error = ER_RECORD_FILE_FULL;
        my_error(error, MYF(0), "SYS_ZIP_DICT");
        break;
      case ha_create_zip_dict_result::TOO_MANY_CONCURRENT_TRXS:
...
int mysql_drop_zip_dict(THD *thd, const char *name, ulong name_len,
  ...
  ulong local_name_len = name_len;
  ha_drop_zip_dict_result drop_result =
      hton->drop_zip_dict(hton, thd, name, &local_name_len);
  if (drop_result != ha_drop_zip_dict_result::OK) {
    switch (drop_result) {
      case ha_drop_zip_dict_result::DOES_NOT_EXIST:
        if (if_exists) {
          push_warning_printf(
        ...
        break;
      case ha_drop_zip_dict_result::IS_REFERENCED:
        error = ER_COMPRESSION_DICTIONARY_IS_REFERENCED;
        my_error(error, MYF(0), name);
        break;
      case ha_drop_zip_dict_result::READ_ONLY:
        error = ER_READ_ONLY_MODE;
        my_error(error, MYF(0));
        break;
      case ha_drop_zip_dict_result::OUT_OF_MEMORY:
        error = ER_OUT_OF_RESOURCES;
        my_error(error, MYF(0));
        break;
      case ha_drop_zip_dict_result::OUT_OF_FILE_SPACE:
        error = ER_RECORD_FILE_FULL;
        my_error(error, MYF(0), "SYS_ZIP_DICT");
        break;
      case ha_drop_zip_dict_result::TOO_MANY_CONCURRENT_TRXS:
        error = ER_TOO_MANY_CONCURRENT_TRXS;
        my_error(error, MYF(0));
        break;
      default:
