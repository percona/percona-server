...
bool SslAcceptorContext::singleton_init(bool use_ssl_arg) {
  ...
  */
  if (use_ssl_arg) {
    ...
    {
      my_error(ER_SSL_TRYING_DATADIR_DEFAULTS, MYF(0),
               DEFAULT_SSL_CA_CERT, DEFAULT_SSL_SERVER_CERT, DEFAULT_SSL_SERVER_KEY);
  ...
  */
  ...
  if (!lock) {
    my_error(ER_SSL_LIBRARY_ERROR, MYF(0),
...
SslAcceptorContext::SslAcceptorContext(bool use_ssl_arg, bool report_ssl_error,
    ...
    : ssl_acceptor_fd(NULL), acceptor(NULL) {
  ...
  if (use_ssl_arg) {
    ...
    if (!ssl_acceptor_fd && report_ssl_error) {
      my_error(ER_SSL_LIBRARY_ERROR, MYF(0), sslGetErrString(error));
    }

    if (ssl_acceptor_fd)
      acceptor = SSL_new(ssl_acceptor_fd->ssl_context);
  }
...
static int warn_one(const char *file_name) {
  ...
  // FIXME need to implement MY_FOPEN_BINARY, to port on windows
  if (!(fp = my_fopen(file_name, O_RDONLY /* | MY_FOPEN_BINARY */, MYF(MY_WME)))) {
    my_error(ER_CANT_OPEN_CA, MYF(0));
    return 1;
  }

  bio = BIO_new(BIO_s_file());
  if (!bio) {
    my_error(ER_FAILED_TO_ALLOCATE_SSL_BIO, MYF(0));
    my_fclose(fp, MYF(0));
    return 1;
  }
  ...
  if (!strcmp(issuer, subject) &&
    ...
    my_error(ER_CA_SELF_SIGNED, MYF(0), file_name);
  }
...
int SslAcceptorContext::warn_self_signed_ca() {
  ...
  if (opt_ssl_capath && opt_ssl_capath[0]) {
    ...
    if (!(ca_dir =
      ...
      my_error(ER_CANT_ACCESS_CAPATH, MYF(0));
      return 1;
    }
