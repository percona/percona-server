...
File_purge::~File_purge() {
  /**
    Clear the file nodes, those will be purged when DDL log recovery after
    reboot
 */
...
bool File_purge::add_file(ulint id, const char *path) {
  ...
  if (srv_print_data_file_purge_process)
    ib::info(ER_IB_MSG_1285) << "File purge add file : " << id << ";" << path;
  return false;
}
...
*/
int File_purge::purge_file(ulong size) {
  bool success = false;
  uint truncated = 0;
  ...
  if (node) {
    ...
    if (srv_print_data_file_purge_process)
      ib::info(ER_IB_MSG_1285) << "File purge purge file : "
                               << node->m_file_path;
...
void File_purge::remove_file(file_purge_node_t *node) {
  ...
  if (srv_print_data_file_purge_process)
    ib::info(ER_IB_MSG_1285) << "File purge remove file : "
                             << node->m_file_path;
...
char *File_purge::generate_file(const char *filepath) {
  ...
  if (srv_print_data_file_purge_process)
    ib::info(ER_IB_MSG_1285) << "File purge generate file : " << filepath << ";"
                             << new_file;
  return new_file;
}
...
dberr_t row_purge_single_table_tablespace(space_id_t space_id,
  ...
  if (srv_print_data_file_purge_process)
    ib::info(ER_IB_MSG_1285) << "File purge write log : " << log_id << ";"
                             << new_filepath;
...
dberr_t row_drop_or_purge_single_table_tablespace(space_id_t space_id,
  ...
  if (srv_data_file_purge)
    return row_purge_single_table_tablespace(space_id, tablename, filepath);
  else
    return row_drop_single_table_tablespace(space_id, tablename, filepath);
}
