...
class trx_stats {
	...
	*/
	static void
	end_io_read(trx_t *trx, ib_time_monotonic_ms_t start_time);
	...
	@param	start_time	return value of start_io_read*/
	static void
	end_io_read(const trx_t &trx, ib_time_monotonic_ms_t start_time);
	...
	*/
	...
	ib_time_monotonic_ms_t lock_que_wait_ustarted;
	bool	take_stats;
};
...
inline void
trx_stats::end_io_read(const trx_t& trx, ib_time_monotonic_ms_t start_time)
{
	const ib_time_monotonic_ms_t finish_time
...
inline void
trx_stats::end_io_read(trx_t *trx, ib_time_monotonic_ms_t start_time)
{
...
trx_stats::stop_lock_wait(const trx_t& trx)
{
	if (UNIV_LIKELY(!take_stats))
		return;
	const ib_time_monotonic_ms_t now = ut_time_monotonic_us();
	thd_report_innodb_stat(trx.mysql_thd, trx.id,
