...
void recv_apply_hashed_log_recs(log_t &log, bool allow_ibuf) {
  ...
  if (!allow_ibuf) {
    ...
    /* The LRU manager threads may still have LRU batches in flight; wait
    for them so that no write IO is running when the pools are
    invalidated. buf_pool_invalidate() additionally parks each pool's LRU
    manager for the duration of the invalidation. */
