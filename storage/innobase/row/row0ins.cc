    ...
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  while (node->index != nullptr) {
    /* FTS and vector indexes have no B-tree at the InnoDB layer: FTS has
    its own aux tables, and a vector index is written by vec_insert_row()
    once the row is in. Without this skip, btr_cur_search_to_nth_level
    asserts on index->page == FIL_NULL. */
