...
static int w_search(MI_INFO *info, MI_KEYDEF *keyinfo, uint comp_flag,
  ...
  if (flag == MI_FOUND_WRONG_KEY) DBUG_RETURN(-1);
...
int _mi_insert(MI_INFO *info, MI_KEYDEF *keyinfo, uchar *key, uchar *anc_buff,
  ...
  DBUG_RETURN(
      _mi_split_page(info, keyinfo, key, anc_buff, key_buff, insert_last));
} /* _mi_insert */
...
static void keys_free(void *vkey, TREE_FREE mode, const void *vparam) {
  ...
  */
  ...
  uint keylen;
  MI_KEYDEF *keyinfo;
  uchar *key = static_cast<uchar *>(vkey);
  auto *param = static_cast<const bulk_insert_param *>(vparam);
...
int mi_init_bulk_insert(MI_INFO *info, ulong cache_size, ha_rows rows) {
  ...
  if (!info->bulk_insert) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
