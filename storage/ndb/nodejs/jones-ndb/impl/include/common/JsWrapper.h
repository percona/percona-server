...
#define CHECK_CLASS_ID(env, PTR) \
  check_class_id(env->class_id, typeid(PTR).name())
#else
...
class GcReclaimer;  // forward declaration
...
class Envelope {
 public:
  ...
  template <typename T>
  Local<Value> wrap(const T *ptr) {
    return wrap(const_cast<T *>(ptr));
  }
  ...
  */
  Local<Value> wrap(const char *str) {
    return String::NewFromUtf8(isolate, str, v8::NewStringType::kNormal)
        .ToLocalChecked();
  }
  ...
  void addAccessor(Local<String> name, Getter getter, Setter setter = 0,
                   Local<Value> data = Local<Value>()) {
    stencil.Get(isolate)->SetNativeDataProperty(
        name, getter, setter, data, v8::DontDelete,
        Local<v8::AccessorSignature>(), v8::DEFAULT);
  }
...
******************************************************************/
template <typename PTR>
void wrapPointerInObject(PTR ptr, Envelope &env, Local<Object> obj) {
...
template <>
inline void wrapPointerInObject(int, Envelope &, Local<Object>) {
  assert(0);
}
template <>
inline void wrapPointerInObject(unsigned long long int, Envelope &,
                                Local<Object>) {
  assert(0);
}
template <>
inline void wrapPointerInObject(unsigned int, Envelope &, Local<Object>) {
  assert(0);
}
template <>
inline void wrapPointerInObject(double, Envelope &, Local<Object>) {
  assert(0);
}
...
#ifdef UNIFIED_DEBUG
  Envelope *env =
      static_cast<Envelope *>(obj->GetAlignedPointerFromInternalField(0));
...
class NativeCodeError {
 public:
  ...
  virtual Local<Value> toJS() {
    ...
    return scope.Escape(
        Exception::Error(String::NewFromUtf8(Isolate::GetCurrent(), message,
