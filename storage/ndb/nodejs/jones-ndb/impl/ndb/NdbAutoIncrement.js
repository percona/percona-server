...
var conf = require("./path_config"), jones = require("database-jones"),
    adapter = require(conf.binary).ndb, stats_module = require(jones.api.stats),
...
NdbAutoIncrementCache.prototype.getValue = function(callback) {
  ...
  udebug.log(
      "NdbAutoIncrementCache getValue table:", this.table.name,
  ...
  apiCall.run = function() {
    adapter.impl.getAutoIncrementValue(
        cache.impl, cache.table, this.batch_size, this.callback);
    cache.batch_size--;
  };
  apiCall.enqueue();
};
...
function NdbAutoIncrementHandler(operations) {
  ...
  udebug.log(
      "New Handler", this.autoinc_op_list.length, "/", operations.length);
}
...
function makeOperationCallback(handler, op) {
  return function(err, value) {
    udebug.log(
        "getValue operation callback value:", value,
        "qlen:", op.tableHandler.dbTable.autoIncrementCache.execQueue.length);
