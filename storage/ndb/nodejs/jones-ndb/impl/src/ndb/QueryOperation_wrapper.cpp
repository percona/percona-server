...
Eternal<String> /* keys of NdbProjection */
    K_next, K_root, K_keyFields, K_joinTo, K_serial, K_parent, K_tableHandler,
    K_rowRecord, K_indexHandler, K_keyRecord, K_isPrimaryKey, K_relatedField,
    K_dbTable, K_dbIndex, K_level, K_data, K_tag;

V8WrapperFn queryPrepareAndExecute, querySetTransactionImpl,
    queryFetchAllResults, queryGetResult, queryClose;
...
void setRowBuffers(Isolate *isolate, QueryOperation *queryOp,
                   Local<Object> spec, int parentId) {
  DEBUG_ENTER();
  Record *record = 0;
  int level = GetInt32Property(isolate, spec, GET_KEY(K_serial));
  if (Get(isolate, spec, GET_KEY(K_rowRecord))->IsObject()) {
    record =
        unwrapPointer<Record *>(ElementToObject(spec, GET_KEY(K_rowRecord)));
  }
  assert(record);
...
const NdbQueryOperationDef *createTopLevelQuery(Isolate *isolate,
  ...
  if (!isPrimaryKey) {
    ...
    if (v->IsObject()) {
      ...
      if (v->IsObject()) {
        index =
            unwrapPointer<const NdbDictionary::Index *>(ToObject(isolate, v));
      }
    }
    assert(index);
  }
  ...
  DEBUG_PRINT("Creating root QueryOperationDef for table: %s",
              table->getName());
  for (int i = 0; i < nKeyParts; i++) {
    ...
    DEBUG_PRINT_DETAIL("Key part %d: %s", i,
                       keyRecord->getColumn(i)->getName());
  }
...
const NdbQueryOperationDef *createNextLevel(
    Isolate *isolate, QueryOperation *queryOp, Local<Object> spec,
  ...
  DEBUG_PRINT("Creating QueryOperationDef at level %d for table: %s", depth,
              table->getName());
...
void createQueryOperation(const Arguments &args) {
  ...
  current =
      createTopLevelQuery(isolate, queryOperation, spec, ArgToObject(args, 1));
...
void queryGetResult(const Arguments &args) {
  ...
  if (header) {
    if (header->data) {
      SetProp(wrapper, GET_KEY(K_data),
              NewJsBuffer(isolate, header->data,
                          op->getResultRowSize(header->sector),
                          doNotFreeQueryResultAtGC));
    } else {
      SetProp(wrapper, GET_KEY(K_data), Null(isolate));
    }
    SetProp(wrapper, GET_KEY(K_level),
            v8::Uint32::New(isolate, header->sector));
...
void QueryOperation_initOnLoad(Local<Object> target) {
  ...
  SET_KEY(K_rowRecord, "rowRecord"), SET_KEY(K_indexHandler, "indexHandler");
