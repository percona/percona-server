...
size_t ndb_openssl_evp::get_needed_key_iv_pair_count(
  ...
  if (m_evp_cipher == EVP_aes_256_cbc()) {
    ...
    const Uint64 data_size_per_key_iv_pair = Uint64{m_data_unit_size}
                                             << XTS_SEQNUM_LEN;
...
int ndb_openssl_evp::add_key_iv_pairs(const byte key_pairs[], size_t pair_count,
                                      size_t pair_size) {
...
ndb_openssl_evp::key256_iv256_set::key256_iv256_set() : m_key_iv_count(0) {}
...
int ndb_openssl_evp::key256_iv256_set::get_key_iv_pair(size_t index,
                                                       const byte **key,
                                                       const byte **iv) const {
  ...
  if (m_context->m_data_unit_size > 0) {
    ...
    if (m_context->m_has_key_iv) {
      ...
      *iv = &m_key_iv[KEY_LEN];
      ...
      int rc = m_context->m_key_iv_set->get_key_iv_mixed_pair(key_iv_pair_index,
                                                              key, iv);
      if (rc == -1) {
        RETURN(-1);
      }
      ...
      if (m_context->m_evp_cipher == my_EVP_aes_256_xts()) {
        ...
        *iv = xts_seq_num;
        for (int i = 0; i < 14; i++) xts_seq_num[i] = 0;
...
int ndb_openssl_evp::operation::setup_decrypt_key_iv(ndb_off_t position,
  ...
  int r;
  r = EVP_DecryptInit_ex(m_evp_context, m_context->m_evp_cipher, nullptr, key,
                         iv);
  if (r != 1) {
    RETURN(-1);
  }
...
int ndb_openssl_evp::operation::setup_encrypt_key_iv(ndb_off_t position) {
  ...
  int r;
  r = EVP_EncryptInit_ex(m_evp_context, m_context->m_evp_cipher, nullptr, key,
                         iv);
  if (r != 1) {
    return -1;
  }
...
int ndb_openssl_evp::operation::encrypt(output_iterator *out,
  ...
  if (data_unit_size == 0) {
    ...
    if (inl > 0) {
      int outl;
      r = EVP_EncryptUpdate(m_evp_context, out->begin(), &outl, in->cbegin(),
                            inl);
      if (r != 1) {
        RETURN(-1);
      }
...
int ndb_openssl_evp::operation::decrypt(output_iterator *out,
  ...
  if (data_unit_size == 0) {
    ...
    if (inl > 0) {
      r = EVP_DecryptUpdate(m_evp_context, out->begin(), &outl, in->cbegin(),
                            inl);
      if (r != 1) {
        RETURN(-1);
      }
...
int ndb_openssl_evp::operation::decrypt_reverse(output_reverse_iterator *out,
  ...
  int real_outl;
  r = EVP_DecryptUpdate(m_evp_context, out->end(), &real_outl,
                        in->cbegin() - inl, inl);
  if (r != 1) {
    RETURN(-1);
  }
  ...
  memmove(out->begin() - real_outl - final_outl, out->end(),
...
void require_fn(bool cc, const char cc_str[], const char file[], int line,
  ...
  fprintf(stderr, "FATAL: %s: %u: %s: require(%s) failed.\n", file, line, func,
          cc_str);
  abort();
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1)
    enc.set_aes_256_cbc(true, 0);
  else
    switch (argv[1][0]) {
      case 'c':
        enc.set_aes_256_cbc(false, argv[1][1] ? atoi(&argv[1][1]) : 0);
        break;
      case 'p':
        enc.set_aes_256_cbc(true, 0);
        break;
      case 'x':
        enc.set_aes_256_xts(false, argv[1][1] ? atoi(&argv[1][1]) : 32);
        break;
    }
  ...
  require(0 ==
          enc.derive_and_add_key_iv_pair(reinterpret_cast<const byte *>(pwd),
                                         strlen(pwd), 100000, salt));
