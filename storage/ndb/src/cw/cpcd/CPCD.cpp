...
bool CPCD::defineProcess(const class Properties &args,
  ...
  *id = proc->m_id;
  ...
  logger.debug("Process %s:%s:%d defined", proc->m_group.c_str(),
               proc->m_name.c_str(), proc->m_id);

  notifyChanges();
  return true;
}

bool CPCD::undefineProcess(const int id, const uintptr_t sessionid,
  ...
  logger.debug("Process %s:%s:%d undefined", proc->m_group.c_str(),
               proc->m_name.c_str(), proc->m_id);
...
bool CPCD::stopProcess(const int id, uintptr_t sessionid,
                       CPCD::RequestStatus *rs) {
  Guard tmp(m_processes);
...
bool CPCD::notifyChanges() {
  bool ret = true;
  if (!loadingProcessList) ret = saveProcessList();
...
bool CPCD::saveProcessList() {
  ...
  if (link(newfile, curfile) != 0) { /* put the new config file in place */
    ...
    logger.critical("Cannot rename '%s' -> '%s': %s", curfile, newfile,
                    strerror(errno));
    return false;
  }
...
bool CPCD::loadProcessList() {
  ...
  */
  ...
  for (i = 0; i < m_processes.size(); i++) {
    ...
    proc->readPid();
    logger.debug("Loading Process %s:%s:%d with pid %d ", proc->m_group.c_str(),
                 proc->m_name.c_str(), proc->m_id, proc->getPid());
...
MutexVector<CPCD::Process *> *CPCD::getProcessList() { return &m_processes; }

void CPCD::RequestStatus::err(enum RequestStatusCode status, const char *msg) {
  m_status = status;
