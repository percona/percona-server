...
class LHLevel {
 ...
 public:
  ...
  bool getSplitBucket(Uint32 &from,
                      Uint32 &to) const;  // true if data move needed
  bool shouldMoveBeforeExpand(LHBits32 hash_value) const;
  void expand();
  bool getMergeBuckets(Uint32 &from,
                       Uint32 &to) const;  // true if data move needed
  void shrink();
...
template <typename Int>
inline LHBits<Int>::LHBits() : m_bits(1) {}

template <typename Int>
inline LHBits<Int>::LHBits(Int bits) : m_bits(bits | highbit()) {}

template <typename Int>
template <typename Int2>
inline LHBits<Int>::LHBits(LHBits<Int2> const &bits) : m_bits(bits.pack()) {
  if (m_bits != bits.pack()) m_bits |= highbit();
}

template <typename Int>
inline void LHBits<Int>::clear() {
  m_bits = 1;
}

template <typename Int>
inline LHBits<Int> LHBits<Int>::unpack(Int packed) {
  LHBits<Int> x;
  x.m_bits = packed;
  return x;
}
...
template <typename Int>
inline Int LHBits<Int>::pack() const {
  return m_bits;
}

template <typename Int>
inline bool LHBits<Int>::match(LHBits<Int> other) const {
  assert(sizeof(Int) <= sizeof(Uint32));
  assert(m_bits != 0);
  assert(other.m_bits != 0);
  assert(sizeof(Int) <= sizeof(Uint32) && (m_bits != 0) && (other.m_bits != 0));
  // Warning: dont reduce to one shift below.
  // << is only defined for right values < 32.
  // and since m_bits != 0, clz(MIN(...)) is guaranteed <= 31
  return ((Uint32(m_bits ^ other.m_bits)
           << BitmaskImpl::clz(MIN(m_bits, other.m_bits)))
          << 1) == 0;
}

template <typename Int>
inline void LHBits<Int>::shift_out() {
  m_bits >>= 1;
  if (m_bits == 0) m_bits++;
}

template <typename Int>
inline void LHBits<Int>::shift_out(Uint8 bits) {
...
template <typename Int>
inline void LHBits<Int>::shift_in(bool bit) {
  if (m_bits >= highbit()) m_bits |= (highbit() >> 1);
  m_bits = (m_bits << 1) | (bit ? 1 : 0);
}

template <typename Int>
inline void LHBits<Int>::shift_in(Uint8 bits, Int value) {
  assert(m_bits != 0);
  assert(bits < 8 * sizeof(m_bits));
...
template <typename Int>
inline Uint8 LHBits<Int>::valid_bits() const {
  assert(m_bits != 0);
  return BitmaskImpl::fls(m_bits);
}

template <typename Int>
inline bool LHBits<Int>::valid_bits(Int bits) const {
...
template <typename Int>
inline bool LHBits<Int>::valid_bit(Int bit) const {
...
template <typename Int>
inline Int LHBits<Int>::get_bits(Int bits) const {
  assert(valid_bits(bits));
  return m_bits & bits;
}

template <typename Int>
inline Int LHBits<Int>::get_bit(Int bit) const {
  assert(valid_bit(bit));
  return m_bits & bit;
}

template <typename Int>
inline Int LHBits<Int>::highbit() const {
...
inline LHLevel::LHLevel() { setSize(0); }

inline LHLevel::LHLevel(Uint32 _size) { setSize(_size); }
...
inline bool LHLevel::isEmpty() const { return maxp() == MAXP_EMPTY; }

inline bool LHLevel::isFull() const {
  return !isEmpty() && (getTop() == ADDR_MAX);
}

inline Uint32 LHLevel::max_size() const { return MAX_SIZE; }
...
inline LHBits16 LHLevelRH::reduce(LHBits32 hash_value) const {
  ...
  if (!hash_value.valid_bits(maxp())) return LHBits16();

  Uint32 addr = hash_value.get_bits(maxp());
  LHBits32 hv(hash_value);
  hv.shift_out(hashcheckbit());
  if (addr < p()) hv.shift_out();
  return LHBits16(hv);
}
...
inline LHBits32 LHLevelRH::enlarge(LHBits16 reduced_hash_value,
                                   Uint32 bucket_number) const {
  assert(!isEmpty());
