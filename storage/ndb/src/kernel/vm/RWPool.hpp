...
template <typename T>
inline void *RWPool<T>::getPtr(Uint32 i) const {
...
template <typename T>
inline void *RWPool<T>::getPtr(const Record_info &ri, Uint32 i) const {
  Uint32 page_no = i >> POOL_RECORD_BITS;
  Uint32 page_idx = i & POOL_RECORD_MASK;
  RWPage *page = m_memroot + page_no;
  Uint32 *record = page->m_data + page_idx;
...
template <typename T>
void RWPool<T>::init(const Record_info &ri, const Pool_context &pc) {
  m_ctx = pc;
  m_record_info = ri;
...
bool RWPool<T>::seize(Ptr<T> &ptr) {
  ...
  Uint32 page_no = RNIL;
  if ((page =
           (RWPage *)m_ctx.alloc_page19(m_record_info.m_type_id, &page_no))) {
    pos = 0;
...
template <typename T>
void RWPool<T>::handle_invalid_release(Ptr<T> ptr) {
  char buf[255];
...
template <typename T>
void RWPool<T>::handle_invalid_get_ptr(Uint32 ptrI) const {
  char buf[255];
