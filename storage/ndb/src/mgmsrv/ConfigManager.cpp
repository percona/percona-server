...
ConfigManager::ConfigManager(const MgmtSrvr::MgmtOpts &opts,
    ...
    : MgmtThread("ConfigManager"),
      ...
      m_config_retriever(opt_ndb_connectstring, opt_ndb_nodeid, NDB_VERSION,
                         NDB_MGM_NODE_TYPE_MGM, opts.bind_address),
...
bool ConfigManager::init(void) {
  ...
  if (m_opts.initial) {
    ...
    if (!delete_saved_configs()) DBUG_RETURN(false);
  }
...
void ConfigManager::commitConfigChange(void) {
...
void ConfigManager::abortConfigChange(void) {
...
void ConfigManager::execCONFIG_CHECK_REQ(SignalSender &ss, SimpleSignal *sig) {
  ...
  g_eventLogger->debug(
      ...
      nodeId, generation, other_generation, m_config_state, other_state,
...
void ConfigManager::execCONFIG_CHECK_CONF(SignalSender &ss, SimpleSignal *sig) {
  ...
  g_eventLogger->debug("Got CONFIG_CHECK_CONF from node: %d", nodeId);
