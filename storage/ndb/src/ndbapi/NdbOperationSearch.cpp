...
******************************************************************************/
int NdbOperation::equal_impl(const NdbColumnImpl *tAttrInfo,
                             const char *aValuePassed) {
  DBUG_ENTER("NdbOperation::equal_impl");
  DBUG_PRINT("enter",
             ("col: %s  op: %d  val: %p",
              (tAttrInfo == nullptr) ? "NULL" : tAttrInfo->m_name.c_str(),
  ...
  if ((theStatus == OperationDefined) && (aValue != nullptr) &&
      (tAttrInfo != nullptr)) {
    /******************************************************************************
  ...
  keyEntryFound:
    ...
    Uint32 tKeyInfoPosition =
        i == 0 ? 1
               : theTupleKeyDefined[i - 1][1] + theTupleKeyDefined[i - 1][2];
        ...
        if (tOpType == UpdateRequest || tOpType == WriteRequest) {
          ...
          if (tOpType == DeleteRequest && m_currentTable->m_noOfBlobs != 0) {
            for (unsigned i = 0; i < m_currentTable->m_columns.size(); i++) {
              ...
              assert(c != nullptr);
              if (c->getBlobType()) {
                if (getBlobHandle(theNdbCon, c) == nullptr) DBUG_RETURN(-1);
              }
            }
          }
          DBUG_RETURN(0);
 ...
 *                 or KEYINFO signal.
 *****************************************************************************/
int NdbOperation::insertKEYINFO(const char *aValue, Uint32 aStartPosition,
  ...
  while (tEndPos > theTotalNrOfKeyWordInSignal) {
    ...
    if (tSignal->setSignal(m_keyInfoGSN, refToBlock(theNdbCon->m_tcRef)) ==
        -1) {
...
void NdbOperation::reorderKEYINFO() {
...
int NdbOperation::getKeyFromTCREQ(Uint32 *data, Uint32 &size) {
  /* Check that we can correctly return a valid key */
  if ((size < theTupKeyLen) || (theTupKeyLen == 0)) return -1;
  ...
  while (pos < size) {
    ...
    data[pos++] = tSignal->getDataPtrSend()[KeyInfo::HeaderLength + n++];
  }
  return 0;
}
...
void NdbOperation::setPartitionId(Uint32 value) {
   ...
   */
  assert(
      ((m_type == PrimaryKeyAccess) && (m_currentTable->getFragmentType() ==
                                        NdbDictionary::Object::UserDefined)) ||
      (m_type == OrderedIndexScan) || (m_type == TableScan));

  theDistributionKey = value;
  theDistrKeyIndicator_ = 1;
  DBUG_PRINT("info", ("NdbOperation::setPartitionId: %u", theDistributionKey));
}

Uint32 NdbOperation::getPartitionId() const {
