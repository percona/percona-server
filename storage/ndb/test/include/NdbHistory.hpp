...
struct EpochRange {
  ...
  bool spansGciBoundary() const {
    ...
    return ((m_end >> 32) > (m_start >> 32));
  }

  static Uint32 hi(const Uint64 epoch) { return Uint32(epoch >> 32); }

  static Uint32 lo(const Uint64 epoch) { return Uint32(epoch & 0xffffffff); }
...
struct EpochRangeSet {
   ...
   */
  void addEpochRange(const EpochRange &er) { m_ranges.push_back(er); }
...
struct RecordRange {
  ...
  RecordRange(const Uint32 start, const Uint32 len)
...
class NdbHistory {
 public:
   ...
   */
  struct RecordState {
    enum RowState { RS_NOT_EXISTS = 0, RS_EXISTS = 1 };
    Uint32 m_state;
  ...
  class Version {
   public:
     ...
     */
    void dump(bool full = false, const char *indent = "") const;
  ...
  void commitVersion(const Version *version, Uint64 commitEpoch);
  ...
  class VersionMatchIterator {
   public:
    VersionMatchIterator(const NdbHistory &history, const Version *match);
  ...
  class MatchingEpochRangeIterator {
   public:
    MatchingEpochRangeIterator(const NdbHistory &history, const Version *match);
   ...
   */
  const Version *findFirstClosestMatch(const Version *match,
