...
class NdbRestarter {
 public:
  ...
  int restartAll2(Uint32 flags) {
    return restartAll(flags & NRRF_INITIAL, flags & NRRF_NOSTART,
                      flags & NRRF_ABORT, flags & NRRF_FORCE);
  }
  ...
  int waitNodesStartPhase(const int *_nodes, int _num_nodes, int _startphase,
                          unsigned int _timeout = 120);
  ...
  int insertError2InNodes(const int *_nodes, int _num_nodes, int error,
                          int extra);
 ...
 protected:
  ...
  int waitNodesState(const int *_nodes, int _num_nodes,
                     ndb_mgm_node_status _status, unsigned int _timeout,
                     int _startphase = -1);
