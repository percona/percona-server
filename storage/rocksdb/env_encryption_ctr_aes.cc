                       ...
                       size_t prefixLength) const
{
  ...
  auto encryptor = cipherStreamFactory_->CreateCipherStream(masterKey, kHeaderIV);
...
rocksdb::Status CTRAesEncryptionProvider::ReencryptPrefix(rocksdb::Slice& prefix) const {
    ...
    auto cipher = cipherStreamFactory_->CreateCipherStream(fileMasterKey, kHeaderIV);
      ...
      std::unique_ptr<rocksdb::BlockAccessCipherStream>* result)
{
  ...
  rocksdb::Slice masterKeyIdSlice(prefix.data() + MASTER_KEY_ID_OFFSET, MASTER_KEY_ID_SIZE);
