...
int Rdb_key_def::unpack_record(TABLE *table, uchar *buf,
  ...
  for (uint i = 0; i < m_key_parts; i++) {
    ...
    */
    ...
    bool do_unpack =
        secondary_key || !fpi->uses_unpack_info() ||
    ...
    if (fpi->m_unpack_func && do_unpack) {
      ...
      res = fpi->m_unpack_func(fpi, field, field->ptr + ptr_diff, &reader,
...
static int rdb_skip_variable_length(const Rdb_field_packing *fpi
                                    __attribute__((__unused__)),
                                    const Field *field,
                                    Rdb_string_reader *reader) {
  const uchar *ptr;
...
static int rdb_skip_variable_space_pad(const Rdb_field_packing *fpi,
                                       const Field *field,
                                       Rdb_string_reader *reader) {
  const uchar *ptr;
  bool finished = false;
...
int rdb_unpack_integer(Rdb_field_packing *fpi, Field *field, uchar *to,
                       Rdb_string_reader *reader,
                       Rdb_string_reader *unp_reader
                       __attribute__((__unused__))) {
...
static int
rdb_unpack_floating_point(uchar *dst, Rdb_string_reader *reader, size_t size,
...
*/
static int rdb_unpack_double(Rdb_field_packing *fpi __attribute__((__unused__)),
                             Field *field __attribute__((__unused__)),
                             uchar *field_ptr, Rdb_string_reader *reader,
                             Rdb_string_reader *unp_reader
                             __attribute__((__unused__))) {
...
/*
  ...
  allowed in the database.
*/
static int rdb_unpack_float(Rdb_field_packing *,
                            Field *field __attribute__((__unused__)),
                            uchar *field_ptr, Rdb_string_reader *reader,
                            Rdb_string_reader *unp_reader
                            __attribute__((__unused__))) {
...
int rdb_unpack_newdate(Rdb_field_packing *fpi, Field *field, uchar *field_ptr,
                       Rdb_string_reader *reader,
                       Rdb_string_reader *unp_reader
                       __attribute__((__unused__))) {
  const char *from;
...
static int rdb_unpack_binary_str(Rdb_field_packing *fpi, Field *field,
...
static int rdb_unpack_utf8_str(Rdb_field_packing *fpi, Field *field, uchar *dst,
                               Rdb_string_reader *reader,
                               Rdb_string_reader *unp_reader
                               __attribute__((__unused__))) {
...
static void rdb_pack_with_varchar_encoding(Rdb_field_packing *fpi, Field *field,
                                           uchar *buf, uchar **dst,
                                           Rdb_pack_field_context *pack_ctx
                                           __attribute__((__unused__))) {
  /*
  ...
  */
  ...
  size_t value_length = (field_var->length_bytes == 1) ? (uint)*field->ptr
                                                       : uint2korr(field->ptr);
  size_t xfrm_len;
  xfrm_len = charset->coll->strnxfrm(
...
static int
rdb_compare_string_with_spaces(const uchar *buf, const uchar *buf_end,
...
static void rdb_pack_with_varchar_space_pad(Rdb_field_packing *fpi,
                                            Field *field, uchar *buf,
                                            uchar **dst,
                                            Rdb_pack_field_context *pack_ctx) {
  ...
  size_t trimmed_len = charset->cset->lengthsp(
...
static int rdb_unpack_binary_or_utf8_varchar(Rdb_field_packing *fpi,
                                             Field *field, uchar *dst,
                                             Rdb_string_reader *reader,
                                             Rdb_string_reader *unp_reader
                                             __attribute__((__unused__))) {
  const uchar *ptr;
...
static int rdb_unpack_binary_or_utf8_varchar_space_pad(
    Rdb_field_packing *fpi, Field *field, uchar *dst, Rdb_string_reader *reader,
    Rdb_string_reader *unp_reader) {
  const uchar *ptr;
  size_t len = 0;
  bool finished = false;
...
static int rdb_unpack_unknown(Rdb_field_packing *fpi, Field *field, uchar *dst,
                              Rdb_string_reader *reader,
                              Rdb_string_reader *unp_reader) {
  const uchar *ptr;
...
static void rdb_make_unpack_unknown_varchar(const Rdb_collation_codec *codec
                                            __attribute__((__unused__)),
                                            const Field *field,
                                            Rdb_pack_field_context *pack_ctx) {
...
static void rdb_make_unpack_simple_varchar(const Rdb_collation_codec *codec,
                                           const Field *field,
                                           Rdb_pack_field_context *pack_ctx) {
  auto f = static_cast<const Field_varstring *>(field);
...
int rdb_unpack_simple_varchar_space_pad(Rdb_field_packing *fpi, Field *field,
                                        uchar *dst, Rdb_string_reader *reader,
                                        Rdb_string_reader *unp_reader) {
  const uchar *ptr;
  size_t len = 0;
...
static void rdb_get_mem_comparable_space(const CHARSET_INFO *cs,
  ...
  if (!rdb_mem_comparable_space[cs->number].get()) {
    ...
    if (!rdb_mem_comparable_space[cs->number].get()) {
      ...
      size_t space_mb_len = cs->cset->wc_mb(cs, (my_wc_t)cs->pad_char, space_mb,
                                            space_mb + sizeof(space_mb));

      uchar space[20]; // mem-comparable image of the space character

      size_t space_len = cs->coll->strnxfrm(cs, space, sizeof(space), 1,
...
rdb_init_collation_mapping(const my_core::CHARSET_INFO *cs) {
...
bool Rdb_field_packing::setup(const Rdb_key_def *key_descr, const Field *field,
  ...
  case MYSQL_TYPE_BLOB: {
    if (key_descr && key_descr->index_format_min_check(
...
*/
const uchar *Rdb_ddl_manager::get_hash_key(Rdb_tbl_def *rec, size_t *length,
                                           my_bool not_used
                                           __attribute__((__unused__))) {
...
bool Rdb_ddl_manager::init(Rdb_dict_manager *dict_arg,
  ...
  (void)my_hash_init(&m_ddl_hash,
                     /*system_charset_info*/ &my_charset_bin, 32, 0, 0,
...
Rdb_tbl_def *Rdb_ddl_manager::find(const std::string &table_name, bool lock) {
  ...
  Rdb_tbl_def *rec = reinterpret_cast<Rdb_tbl_def *>(my_hash_search(
...
bool Rdb_dict_manager::is_index_operation_ongoing(
    const GL_INDEX_ID &gl_index_id, Rdb_key_def::DATA_DICT_TYPE dd_type) {
  DBUG_ASSERT(dd_type == Rdb_key_def::DDL_DROP_INDEX_ONGOING ||
              dd_type == Rdb_key_def::DDL_CREATE_INDEX_ONGOING);
...
void Rdb_dict_manager::log_start_drop_table(
    const std::shared_ptr<Rdb_key_def> *key_descr, uint32 n_keys,
...
void Rdb_dict_manager::add_stats(rocksdb::WriteBatch *batch,
