...
public:
  ...
  uint pack_record(const TABLE *tbl, uchar *pack_buffer, const uchar *record,
                   ...
                   bool should_store_checksums, longlong hidden_pk_id = 0,
                   uint n_key_parts = 0, uint *n_null_fields = nullptr) const;
  /* Pack the hidden primary key into mem-comparable form. */
  uint pack_hidden_pk(longlong hidden_pk_id, uchar *packed_tuple) const;
  ...
  */
  ...
  uint get_primary_key_tuple(TABLE *tbl, const Rdb_key_def &pk_descr,
...
public:
  bool setup(const Rdb_key_def *key_descr, const Field *field, uint keynr_arg,
             uint key_part_arg, uint16 key_length);
...
public:
  ...
  bool put_dict(Rdb_dict_manager *dict, rocksdb::WriteBatch *batch, uchar *key,
                size_t keylen);
...
private:
  ...
  void log_start_drop_index(GL_INDEX_ID gl_index_id, const char *log_action);
...
public:
  ...
  /* CF id => CF flags */
  void add_cf_flags(rocksdb::WriteBatch *batch, const uint cf_id,
                    const uint cf_flags);
  ...
  inline void
  get_ongoing_create_indexes(std::vector<GL_INDEX_ID> *gl_index_ids) {
    get_ongoing_index_operation(gl_index_ids,
  ...
  inline void
  finish_drop_indexes(const std::unordered_set<GL_INDEX_ID> &gl_index_ids) {
    finish_indexes_operation(gl_index_ids, Rdb_key_def::DDL_DROP_INDEX_ONGOING);
  }
  inline void
  finish_create_indexes(const std::unordered_set<GL_INDEX_ID> &gl_index_ids) {
  ...
  bool update_max_index_id(rocksdb::WriteBatch *batch, const uint32_t index_id);
