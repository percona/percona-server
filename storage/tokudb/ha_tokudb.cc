...
void get_db_table_name_from_canonical_name(
...
partition_info *
ha_tokudb::parse_partition_info(const std::string &partition_info_str) {
    THD *thd = ha_thd();
    DBUG_ASSERT(thd);
...
int ha_tokudb::delete_rename_partitioned_table(
    ...
    partition_info *part_info = parse_partition_info(partition_info_str);
...
int ha_tokudb::delete_table(const char *name) {
    ...
    if (!get_part_str(name, partition_info_str))
