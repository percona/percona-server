    ...
    namespace analyze {
        ...
        class recount_rows_t : public tokudb::background::job_manager_t::job_t {
           public:
            ...
            recount_rows_t(bool user_schedued,
                           THD* thd,
                           TOKUDB_SHARE* share,
                           DB_TXN* txn);
           ...
           private:
            ...
            static int analyze_recount_rows_progress(uint64_t count,
                                                     uint64_t deleted,
                                                     void* extra);
        ...
        recount_rows_t::recount_rows_t(bool user_scheduled,
                                       THD* thd,
                                       TOKUDB_SHARE* share,
                                       DB_TXN* txn)
        ...
        void recount_rows_t::on_run() {
            ...
            if (_txn == NULL) {
                _result =
                    db_env->txn_begin(db_env, NULL, &_txn, DB_READ_UNCOMMITTED);
            ...
            _result = _share->file->recount_rows(
                _share->file, analyze_recount_rows_progress, this);
        ...
        class standard_t : public tokudb::background::job_manager_t::job_t {
           public:
            ...
            standard_t(bool user_scheduled,
                       THD* thd,
                       TOKUDB_SHARE* share,
                       DB_TXN* txn);
        ...
        standard_t::standard_t(bool user_scheduled,
                               THD* thd,
                               TOKUDB_SHARE* share,
                               DB_TXN* txn)
            : tokudb::background::job_manager_t::job_t(user_scheduled),
        ...
        void standard_t::on_run() {
            ...
            _result =
                _share->key_file[0]->stat64(_share->key_file[0], _txn, &stat64);
            if (_result != 0) {
                _result = HA_ADMIN_FAILED;
                goto cleanup;
            }
            ...
            for (ulonglong current_key = 0;
                ...
                if (_thd && (_result == HA_ADMIN_FAILED ||
                    ...
                    _thd->get_protocol()->store(
                        name, namelen, system_charset_info);
                    _thd->get_protocol()->store(
                        "analyze", 7, system_charset_info);
                ...
                total_key_parts +=
                    _share->_key_descriptors[_current_key]._parts;
            }
            if (_result == HA_ADMIN_OK) {
                int error = tokudb::set_card_in_status(_share->status_block,
                                                       _txn,
                                                       total_key_parts,
                                                       rec_per_key_part);
                if (error)
                ...
                _share->lock();
                _share->update_cardinality_counts(total_key_parts,
                                                  rec_per_key_part);
            ...
            if (_ticks > 1000) {
                ...
                if ((_thd && thd_killed(_thd)) || cancelled()) {
                    ...
                    return ER_ABORTING_CONNECTION;
                           ...
                           static_cast<uint64_t>(_key_elapsed_time) >
                               _time_limit) {
                    // time limit reached
                    return ETIME;
                }
        ...
        int standard_t::analyze_key(uint64_t* rec_per_key_part) {
            ...
            while (1) {
                ...
                if (copy_key == false) {
                    ...
                    for (uint64_t i = 0; i < num_key_parts; i++) {
                        int cmp = tokudb_cmp_dbt_key_parts(
                            db, &prev_key, &key, i + 1);
                        if (cmp != 0) {
                            unique_rows[i]++;
                            copy_key = true;
...
int ha_tokudb::do_optimize(THD* thd) {
    ...
    for (uint i = 0; i < curr_num_DBs; i++) {
        ...
        if (optimize_index_name) {
            const char* this_index_name = i >= table_share->keys
                                              ? "primary"
        ...
        uint64_t loops_run;
        error = db->hot_optimize(
            db, NULL, NULL, hot_optimize_progress_fun, &hc, &loops_run);
        if (error) {
            goto cleanup;
        }
    }
...
static void ha_tokudb_check_info(THD* thd, TABLE* table, const char* msg) {
    ...
    thd->get_protocol()->store(
        tablename, strlen(tablename), system_charset_info);
    ...
    if (result == HA_ADMIN_OK) {
        ...
        if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_CHECK))) {
            ...
            TOKUDB_HANDLER_TRACE(
                "%.24s %s", ctime_r(&now, timebuf), write_status_msg);
        }
