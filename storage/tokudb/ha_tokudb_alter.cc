...
static bool tables_have_same_keys(TABLE *table, TABLE *altered_table,
                                  bool print_error, bool check_field_index) {
  bool retval;
  if (table->s->keys != altered_table->s->keys) {
    if (print_error) {
      LogPluginErrMsg(ERROR_LEVEL, 0, "Tables have different number of keys");
    }
    retval = false;
    goto cleanup;
  }
  if (table->s->primary_key != altered_table->s->primary_key) {
    if (print_error) {
      LogPluginErrMsg(ERROR_LEVEL, 0,
                      ...
                      table->s->primary_key, altered_table->s->primary_key);
    }
    retval = false;
    goto cleanup;
  }
  for (uint32_t i = 0; i < table->s->keys; i++) {
    ...
    if (strcmp(curr_orig_key->name, curr_altered_key->name)) {
      if (print_error) {
        LogPluginErrMsg(ERROR_LEVEL, 0, "Key %d has different name, %s %s", i,
                        curr_orig_key->name, curr_altered_key->name);
      }
      retval = false;
      goto cleanup;
    }
...
static uint32_t fill_static_row_mutator(uchar *buf, TABLE *orig_table,
                                        TABLE *altered_table,
...
static uint32_t fill_dynamic_row_mutator(uchar *buf, uint32_t *columns,
                                         uint32_t num_columns, TABLE *src_table,
...
static uint32_t fill_static_blob_row_mutator(uchar *buf, TABLE *src_table,
                                             KEY_AND_COL_INFO *src_kc_info) {
  uchar *pos = buf;
...
static uint32_t fill_dynamic_blob_row_mutator(uchar *buf, uint32_t *columns,
...
uint32_t ha_tokudb::fill_row_mutator(uchar *buf, uint32_t *columns,
  ...
  if (is_add) {
    pos += fill_dynamic_row_mutator(pos, columns, num_columns, altered_table,
  ...
  if (has_blobs) {
    ...
    if (is_add) {
      pos += fill_dynamic_blob_row_mutator(
          pos, columns, num_columns, altered_table, altered_kc_info, is_add);
    } else {
      pos += fill_dynamic_blob_row_mutator(pos, columns, num_columns, table,
                                           &share->kc_info, is_add);
    }
  }
  return pos - buf;
}
...
static bool all_fields_are_same_type(TABLE *table_a, TABLE *table_b) {
  if (table_a->s->fields != table_b->s->fields) return false;
  for (uint i = 0; i < table_a->s->fields; i++) {
    Field *field_a = table_a->field[i];
    Field *field_b = table_b->field[i];
    if (!fields_are_same_type(field_a, field_b)) return false;
  }
  return true;
}

static bool column_rename_supported(TABLE *orig_table, TABLE *new_table,
  ...
  //
  keys_same_for_cr = tables_have_same_keys(orig_table, new_table, false, true);
  if (!keys_same_for_cr) {
    retval = false;
    goto cleanup;
  }
...
static int find_changed_columns(uint32_t *changed_columns,
                                ...
                                TABLE *smaller_table, TABLE *bigger_table) {
  int retval;
  ...
  for (uint i = 0; i < smaller_table->s->fields; i++, curr_new_col_index++) {
    if (curr_new_col_index >= bigger_table->s->fields) {
      LogPluginErrMsg(ERROR_LEVEL, 0, "Error in determining changed columns");
      retval = 1;
      goto cleanup;
    }
...
static bool tables_have_same_keys_and_columns(TABLE *first_table,
  ...
  if (first_table->s->null_bytes != second_table->s->null_bytes) {
    ...
    if (print_error) {
      LogPluginErrMsg(ERROR_LEVEL, 0,
                      ...
                      first_table->s->null_bytes, second_table->s->null_bytes);
    }
    goto exit;
  }
  if (first_table->s->fields != second_table->s->fields) {
    ...
    if (print_error) {
      LogPluginErrMsg(ERROR_LEVEL, 0,
                      ...
                      first_table->s->fields, second_table->s->fields);
    }
    goto exit;
  }
  for (uint i = 0; i < first_table->s->fields; i++) {
    ...
    if (!are_two_fields_same(a, b)) {
      retval = false;
      LogPluginErrMsg(ERROR_LEVEL, 0,
                      "Tables have different fields at position %d", i);
      goto exit;
    }
  }
...
// The tokudb alter context contains the alter state that is set in the check if
// supported method and used later when the alter operation is executed.
...
void ha_tokudb::print_alter_info(TABLE *altered_table,
                                 Alter_inplace_info *ha_alter_info) {
  TOKUDB_TRACE("***are keys of two tables same? %d",
               tables_have_same_keys(table, altered_table, false, false));
  if (ha_alter_info->handler_flags) {
    TOKUDB_TRACE("***alter flags set ***");
    for (int i = 0; i < 32; i++) {
      if (ha_alter_info->handler_flags & (1 << i)) TOKUDB_TRACE("%d", i);
    }
  }
...
static int find_changed_fields(TABLE *table_a, TABLE *table_b,
                               std::vector<uint> &changed_fields) {
  for (uint i = 0; i < table_a->s->fields; i++) {
    Field *field_a = table_a->field[i];
    Field *field_b = table_b->field[i];
    if (!fields_are_same_type(field_a, field_b)) changed_fields.push_back(i);
  }
  return changed_fields.size();
}

static bool change_length_is_supported(TABLE *table, TABLE *altered_table,
                                       tokudb_alter_ctx *ctx);

static bool change_type_is_supported(TABLE *table, TABLE *altered_table,
                                     tokudb_alter_ctx *ctx);

// The ha_alter_info->handler_flags can not be trusted.
// This function maps the bogus handler flags to something we like.
static ulong fix_handler_flags(THD *thd, TABLE *table, TABLE *altered_table,
                               Alter_inplace_info *ha_alter_info) {
  ...
  if (handler_flags &
    ...
    if (handler_flags &
      ...
      if (tables_have_same_keys(table, altered_table,
...
enum_alter_inplace_result ha_tokudb::check_if_supported_inplace_alter(
  ...
  if (tokudb::sysvars::disable_hot_alter(thd)) {
    ;  // do nothing
  } else if (only_flags(ctx->handler_flags,
    ...
    if (table->s->null_bytes == altered_table->s->null_bytes &&
        ...
        !tables_have_same_keys(table, altered_table,
                               tokudb::sysvars::alter_print_error(thd) != 0,
                               false) &&
        is_disjoint_add_drop(ha_alter_info)) {
      if (ctx->handler_flags & (Alter_inplace_info::DROP_INDEX +
                                Alter_inplace_info::DROP_UNIQUE_INDEX)) {
    ...
    if (table->s->null_bytes == altered_table->s->null_bytes) {
      bool cr_supported = column_rename_supported(
          table, altered_table,
          (ctx->handler_flags &
           Alter_inplace_info::ALTER_STORED_COLUMN_ORDER) != 0);
      if (cr_supported) result = HA_ALTER_INPLACE_EXCLUSIVE_LOCK;
    }
    ...
    int r = find_changed_columns(added_columns, &num_added_columns, table,
                                 altered_table);
    if (r == 0) {
      if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ALTER_TABLE))) {
        for (uint32_t i = 0; i < num_added_columns; i++) {
          ...
          TOKUDB_TRACE("Added column: index %d, name %s", curr_added_index,
    ...
    int r = find_changed_columns(dropped_columns, &num_dropped_columns,
                                 altered_table, table);
    if (r == 0) {
      if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ALTER_TABLE))) {
        for (uint32_t i = 0; i < num_dropped_columns; i++) {
          ...
          TOKUDB_TRACE("Dropped column: index %d, name %s", curr_dropped_index,
      ...
      result = HA_ALTER_INPLACE_EXCLUSIVE_LOCK;
             ...
             find_changed_fields(table, altered_table, ctx->changed_fields) >
                 0 &&
             setup_kc_info(altered_table, ctx->altered_table_kc_info) == 0) {
    ...
    } else if (only_flags(create_info->used_fields,
...
bool ha_tokudb::prepare_inplace_alter_table(
    TOKUDB_UNUSED(TABLE *altered_table), Alter_inplace_info *ha_alter_info,
...
bool ha_tokudb::inplace_alter_table(
  ...
  if (error == 0 &&
      (ctx->handler_flags & (Alter_inplace_info::DROP_INDEX +
                             Alter_inplace_info::DROP_UNIQUE_INDEX))) {
  ...
  if (error == 0 && ctx->reset_card) {
    error = tokudb::alter_card(share->status_block, ctx->alter_txn, table->s,
                               altered_table->s);
  }
  ...
  if (error == 0 && (altered_table->part_info == NULL)) {
    error = write_frm_data(share->status_block, ctx->alter_txn,
...
int ha_tokudb::alter_table_add_index(Alter_inplace_info *ha_alter_info) {
  ...
  KEY *key_info = (KEY *)tokudb::memory::malloc(
      sizeof(KEY) * ha_alter_info->index_add_count, MYF(MY_WME));
  for (uint i = 0; i < ha_alter_info->index_add_count; i++) {
    KEY *key = &key_info[i];
    *key = ha_alter_info->key_info_buffer[ha_alter_info->index_add_buffer[i]];
    for (KEY_PART_INFO *key_part = key->key_part;
         key_part < key->key_part + key->user_defined_key_parts; key_part++) {
  ...
  int error = tokudb_add_index(table, key_info, ha_alter_info->index_add_count,
                               ctx->alter_txn, &ctx->incremented_num_DBs,
                               &ctx->modified_DBs);
  ...
  if (error == 0) ctx->reset_card = true;

  return error;
}

static bool find_index_of_key(const char *key_name, TABLE *table,
...
static bool find_index_of_key(const char *key_name, KEY *key_info,
                              uint key_count, uint *index_offset_ptr) {
...
int ha_tokudb::alter_table_drop_index(Alter_inplace_info *ha_alter_info) {
  ...
  for (uint i = 0; i < ha_alter_info->index_drop_count; i++) {
    bool found;
    found = find_index_of_key(ha_alter_info->index_drop_buffer[i]->name, table,
                              &index_drop_offsets[i]);
    if (!found) {
      // undo of add key in partition engine
      found =
          find_index_of_key(ha_alter_info->index_drop_buffer[i]->name,
                            ha_alter_info->key_info_buffer,
                            ha_alter_info->key_count, &index_drop_offsets[i]);
      assert_always(found);
  ...
  int error = drop_indexes(index_drop_offsets, ha_alter_info->index_drop_count,
                           key_info, ctx->alter_txn);
...
int ha_tokudb::alter_table_add_or_drop_column(
  ...
  if (ha_alter_info->handler_flags & Alter_inplace_info::DROP_COLUMN) {
    find_changed_columns(columns, &num_columns, altered_table, table);
  } else if (ha_alter_info->handler_flags & Alter_inplace_info::ADD_COLUMN) {
    find_changed_columns(columns, &num_columns, table, altered_table);
  } else {
    assert_unreachable();
  }
  max_column_extra_size =
      ...
      (num_columns * (1 + 4 + 1 + 4));
  column_extra =
      (uchar *)tokudb::memory::malloc(max_column_extra_size, MYF(MY_WME));
  ...
  for (uint32_t i = 0; i < curr_num_DBs; i++) {
    ...
    error =
        new_row_descriptor(altered_table, ha_alter_info, i, &row_descriptor);
    if (error) goto cleanup;
    error = share->key_file[i]->change_descriptor(
        share->key_file[i], ctx->alter_txn, &row_descriptor, 0);
    ...
    if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
      num_column_extra = fill_row_mutator(
          column_extra, columns, num_columns, altered_table,
          ctx->altered_table_kc_info, i,
          // true if adding columns, otherwise is a drop
          (ha_alter_info->handler_flags & Alter_inplace_info::ADD_COLUMN) != 0);
...
#if defined(TOKU_INCLUDE_WRITE_FRM_DATA) && TOKU_INCLUDE_WRITE_FRM_DATA
    int error = write_frm_data(share->status_block, ctx->alter_txn,
                               altered_table->s->path.str);
    if (error) {
      commit = false;
      result = true;
  ...
  if (!commit) {
    if (table->mdl_ticket->get_type() != MDL_EXCLUSIVE &&
      ...
      for (volatile uint i = 0;
           ...
           i++) {
        if (thd->killed != THD::NOT_KILLED) thd->killed = THD::NOT_KILLED;
        sleep(1);
      }
      assert_always(table->mdl_ticket->get_type() == MDL_EXCLUSIVE);
      if (thd->killed == THD::NOT_KILLED) thd->killed = saved_killed_state;
    }
    ...
    if (ctx->add_index_changed) {
      restore_add_index(table, ha_alter_info->index_add_count,
                        ctx->incremented_num_DBs, ctx->modified_DBs);
    }
    if (ctx->drop_index_changed) {
      ...
      for (uint i = 0; i < ha_alter_info->index_drop_count; i++) {
        bool found =
            find_index_of_key(ha_alter_info->index_drop_buffer[i]->name, table,
                              &index_drop_offsets[i]);
        assert_always(found);
      }
      restore_drop_indexes(index_drop_offsets, ha_alter_info->index_drop_count);
    }
    if (ctx->compression_changed) {
      ...
      for (uint32_t i = 0; i < curr_num_DBs; i++) {
        ...
        int error =
            db->change_compression_method(db, ctx->orig_compression_method);
  ...
  for (uint32_t i = 0; i < curr_num_DBs; i++) {
    ...
    if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
      uint32_t offset_start =
          table_share->null_bytes + share->kc_info.mcp_info[i].fixed_field_size;
      uint32_t offset_end =
          offset_start + share->kc_info.mcp_info[i].len_of_offsets;
      ...
      error = share->key_file[i]->update_broadcast(
          share->key_file[i], ctx->alter_txn, &expand, DB_IS_RESETTING_OP);
...
static bool field_in_key_of_table(TABLE *table, Field *field) {
  for (uint i = 0; i < table->s->keys; i++) {
    if (field_in_key(&table->key_info[i], field)) return true;
  }
  return false;
}
...
static bool change_length_is_supported(TABLE *table, TABLE *altered_table,
                                       tokudb_alter_ctx *ctx) {
  if (table->s->fields != altered_table->s->fields) return false;
  if (table->s->null_bytes != altered_table->s->null_bytes) return false;
...
static bool is_sorted(const std::vector<uint> &a) {
  if (a.size() > 0) {
    ...
    for (uint i = 1; i < a.size(); i++)
      if (lastelement > a[i]) return false;
  }
  return true;
}

int ha_tokudb::alter_table_expand_columns(TABLE *altered_table,
                                          Alter_inplace_info *ha_alter_info) {
  int error = 0;
  tokudb_alter_ctx *ctx =
      static_cast<tokudb_alter_ctx *>(ha_alter_info->handler_ctx);
  ...
  for (uint ai = 0; error == 0 && ai < ctx->changed_fields.size(); ai++) {
    ...
    error = alter_table_expand_one_column(altered_table, ha_alter_info,
                                          expand_field_num);
  }
...
static bool is_unsigned(Field *f) { return (f->is_flag_set(UNSIGNED_FLAG)) != 0; }
...
// TODO: replace this?
static uint32_t alter_table_field_offset(uint32_t null_bytes,
                                         KEY_AND_COL_INFO *kc_info, int idx,
...
int ha_tokudb::alter_table_expand_one_column(TABLE *altered_table,
                                             Alter_inplace_info *ha_alter_info,
                                             int expand_field_num) {
  int error = 0;
  tokudb_alter_ctx *ctx =
  ...
  for (uint32_t i = 0; i < curr_num_DBs; i++) {
    ...
    if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
      uint32_t old_offset = alter_table_field_offset(
          table_share->null_bytes, ctx->table_kc_info, i, expand_field_num);
      uint32_t new_offset = alter_table_field_offset(table_share->null_bytes,
      ...
      DBT expand;
      memset(&expand, 0, sizeof(expand));
      expand.size = sizeof(operation) + sizeof(new_offset) +
...
static void marshall_blob_lengths(tokudb::buffer &b, uint32_t n, TABLE *table,
...
int ha_tokudb::alter_table_expand_blobs(TABLE *altered_table,
                                        Alter_inplace_info *ha_alter_info) {
  int error = 0;
  tokudb_alter_ctx *ctx =
      static_cast<tokudb_alter_ctx *>(ha_alter_info->handler_ctx);
  ...
  for (uint32_t i = 0; i < curr_num_DBs; i++) {
    ...
    if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
      ...
      b.append_ui<uint32_t>(table->s->null_bytes +
      ...
      b.append_ui<uint32_t>(
          var_offset_bytes == 0 ? 0 : ctx->table_kc_info->num_offset_bytes);
      ...
      marshall_blob_lengths(b, num_blobs, altered_table,
                            ctx->altered_table_kc_info);
...
static bool change_fixed_length_is_supported(Field *old_field, Field *new_field,
                                             tokudb_alter_ctx *ctx) {
  // no change in size is supported
  if (old_field->pack_length() == new_field->pack_length()) return true;
  // shrink is not supported
  if (old_field->pack_length() > new_field->pack_length()) return false;
  ctx->expand_fixed_update_needed = true;
  return true;
}

static bool change_blob_length_is_supported(Field *old_field, Field *new_field,
                                            tokudb_alter_ctx *ctx) {
...
static bool change_field_type_is_supported(Field *old_field, Field *new_field,
                                           tokudb_alter_ctx *ctx) {
...
int ha_tokudb::new_row_descriptor(TABLE *altered_table,
                                  Alter_inplace_info *ha_alter_info,
                                  uint32_t idx, DBT *row_descriptor) {
  int error = 0;
  tokudb_alter_ctx *ctx =
      static_cast<tokudb_alter_ctx *>(ha_alter_info->handler_ctx);
  row_descriptor->size =
  ...
  if (row_descriptor->data == NULL) {
    ...
    KEY *prim_key =
        hidden_primary_key ? NULL : &altered_table->s->key_info[primary_key];
    if (idx == primary_key) {
      row_descriptor->size = create_main_key_descriptor(
          (uchar *)row_descriptor->data, prim_key, hidden_primary_key,
          primary_key, altered_table, ctx->altered_table_kc_info);
    } else {
      row_descriptor->size = create_secondary_key_descriptor(
          (uchar *)row_descriptor->data, &altered_table->key_info[idx],
          prim_key, hidden_primary_key, altered_table, primary_key, idx,
          ctx->altered_table_kc_info);
    }
    error = 0;
  }
