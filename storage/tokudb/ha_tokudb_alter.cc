...
static int find_changed_fields(TABLE* table_a,
                               TABLE* table_b,
                               std::vector<uint>& changed_fields) {
    for (uint i = 0; i < table_a->s->fields; i++) {
        Field* field_a = table_a->field[i];
...
static ulong fix_handler_flags(THD* thd,
                               TABLE* table,
                               TABLE* altered_table,
                               Alter_inplace_info* ha_alter_info) {
...
static bool is_disjoint_add_drop(Alter_inplace_info* ha_alter_info) {
    for (uint d = 0; d < ha_alter_info->index_drop_count; d++) {
        ...
        for (uint a = 0; a < ha_alter_info->index_add_count; a++) {
            KEY* add_key =
                &ha_alter_info
                     ->key_info_buffer[ha_alter_info->index_add_buffer[a]];
...
enum_alter_inplace_result ha_tokudb::check_if_supported_inplace_alter(
    ...
    if (tokudb::sysvars::disable_hot_alter(thd)) {
        ...
        int r = find_changed_columns(
            added_columns, &num_added_columns, table, altered_table);
        if (r == 0) {
            if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ALTER_TABLE))) {
                for (uint32_t i = 0; i < num_added_columns; i++) {
                    ...
                    TOKUDB_TRACE("Added column: index %d, name %s",
                                 curr_added_index,
        ...
        int r = find_changed_columns(
            dropped_columns, &num_dropped_columns, altered_table, table);
        if (r == 0) {
            if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ALTER_TABLE))) {
                for (uint32_t i = 0; i < num_dropped_columns; i++) {
                    uint32_t curr_dropped_index = dropped_columns[i];
                    Field* curr_dropped_field =
                        table->field[curr_dropped_index];
                    TOKUDB_TRACE("Dropped column: index %d, name %s",
                                 curr_dropped_index,
    ...
    if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ALTER_TABLE)) &&
        ...
        TOKUDB_HANDLER_TRACE("null bytes %u -> %u",
                             table->s->null_bytes,
                             altered_table->s->null_bytes);
    }
...
bool ha_tokudb::inplace_alter_table(TABLE* altered_table,
    ...
    if (error == 0 && ctx->expand_varchar_update_needed)
        error =
            alter_table_expand_varchar_offsets(altered_table, ha_alter_info);
    ...
    if (error == 0 && ctx->reset_card) {
        error = tokudb::alter_card(
            share->status_block, ctx->alter_txn, table->s, altered_table->s);
    }
...
int ha_tokudb::alter_table_add_index(Alter_inplace_info* ha_alter_info) {
    ...
    int error = tokudb_add_index(table,
                                 key_info,
...
static bool find_index_of_key(const char* key_name,
                              TABLE* table,
...
int ha_tokudb::alter_table_drop_index(Alter_inplace_info* ha_alter_info) {
    ...
    for (uint i = 0; i < ha_alter_info->index_drop_count; i++) {
        bool found;
        found = find_index_of_key(ha_alter_info->index_drop_buffer[i]->name,
                                  table,
...
int ha_tokudb::alter_table_add_or_drop_column(
    ...
    max_column_extra_size =
        ...
        4 + num_columns * (1 + 1 + 4 + 1 + 1 + 4) +
        altered_table->s->reclength +
    ...
    for (uint32_t i = 0; i < curr_num_DBs; i++) {
        ...
        if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
            ...
            error = share->key_file[i]->update_broadcast(share->key_file[i],
                                                         ctx->alter_txn,
                                                         &column_dbt,
                                                         DB_IS_RESETTING_OP);
            if (error) {
...
bool ha_tokudb::commit_inplace_alter_table(
    ...
    if (!commit) {
        if (table->mdl_ticket->get_type() != MDL_EXCLUSIVE &&
            ...
            for (volatile uint i = 0; wait_while_table_is_used(
                     thd, table, HA_EXTRA_PREPARE_FOR_RENAME);
                 i++) {
        ...
        if (ctx->compression_changed) {
            ...
            for (uint32_t i = 0; i < curr_num_DBs; i++) {
                ...
                int error = db->change_compression_method(
                    db, ctx->orig_compression_method);
                assert_always(error == 0);
            }
        }
    }
...
int ha_tokudb::setup_kc_info(TABLE* altered_table,
    ...
    if (error == 0)
        error = initialize_key_and_col_info(altered_table->s,
                                            altered_table,
                                            altered_kc_info,
                                            hidden_primary_key,
                                            primary_key);
...
int ha_tokudb::alter_table_expand_varchar_offsets(
    ...
    for (uint32_t i = 0; i < curr_num_DBs; i++) {
        ...
        if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
            uint32_t offset_start = table_share->null_bytes +
...
int ha_tokudb::alter_table_expand_columns(TABLE* altered_table,
    ...
    for (uint ai = 0; error == 0 && ai < ctx->changed_fields.size(); ai++) {
        ...
        error = alter_table_expand_one_column(
            altered_table, ha_alter_info, expand_field_num);
    }
...
int ha_tokudb::alter_table_expand_one_column(TABLE* altered_table,
    ...
    for (uint32_t i = 0; i < curr_num_DBs; i++) {
        ...
        if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
            uint32_t old_offset =
                alter_table_field_offset(table_share->null_bytes,
                                         ctx->table_kc_info,
                                         i,
                                         expand_field_num);
            uint32_t new_offset =
...
static void marshall_blob_lengths(tokudb::buffer& b,
                                  uint32_t n,
                                  TABLE* table,
                                  KEY_AND_COL_INFO* kc_info) {
    for (uint i = 0; i < n; i++) {
...
int ha_tokudb::alter_table_expand_blobs(TABLE* altered_table,
    ...
    for (uint32_t i = 0; i < curr_num_DBs; i++) {
        ...
        if (i == primary_key || key_is_clustering(&table_share->key_info[i])) {
            ...
            b.append_ui<uint32_t>(var_offset_bytes == 0
                                      ? 0
            ...
            marshall_blob_lengths(
                b, num_blobs, altered_table, ctx->altered_table_kc_info);
...
int ha_tokudb::new_row_descriptor(TABLE* altered_table,
    ...
    if (row_descriptor->data == NULL) {
        ...
        KEY* prim_key = hidden_primary_key
                            ? NULL
                            : &altered_table->s->key_info[primary_key];
        if (idx == primary_key) {
            row_descriptor->size =
                create_main_key_descriptor((uchar*)row_descriptor->data,
                                           prim_key,
                                           hidden_primary_key,
                                           primary_key,
                                           altered_table,
            ...
            row_descriptor->size =
                create_secondary_key_descriptor((uchar*)row_descriptor->data,
