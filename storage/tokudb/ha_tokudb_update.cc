...
static void dump_item(Item *item) {
  ...
  switch (item->type()) {
    case Item::FUNC_ITEM: {
      ...
      fprintf(stderr, ":func=%u,%s,%u(", func->functype(), func->func_name(),
              n);
      for (uint i = 0; i < n; i++) {
    ...
    case Item::FIELD_ITEM: {
      ...
      fprintf(stderr, ":field=%s.%s.%s", field_item->db_name,
              field_item->table_name, field_item->field_name);
      break;
    }
    case Item::COND_ITEM: {
...
// idx) of a particular field (selected by expand_field_num). This only works
// for fixed length fields
static uint32_t fixed_field_offset(uint32_t null_bytes,
                                   KEY_AND_COL_INFO *kc_info, uint idx,
                                   uint expand_field_num) {
  uint32_t offset = null_bytes;
  for (uint i = 0; i < expand_field_num; i++) {
    if (bitmap_is_set(&kc_info->key_filters[idx], i)) continue;
    offset += kc_info->field_lengths[i];
  }
  return offset;
}

static uint32_t var_field_index(TABLE *table, KEY_AND_COL_INFO *kc_info,
                                uint idx, uint field_num) {
  assert_always(field_num < table->s->fields);
  uint v_index = 0;
  for (uint i = 0; i < table->s->fields; i++) {
    if (bitmap_is_set(&kc_info->key_filters[idx], i)) continue;
    if (kc_info->length_bytes[i]) {
      if (i == field_num) break;
      v_index++;
    }
  }
  return v_index;
...
static uint32_t blob_field_index(TABLE *table, KEY_AND_COL_INFO *kc_info,
                                 uint field_num) {
  assert_always(field_num < table->s->fields);
  uint b_index;
  for (b_index = 0; b_index < kc_info->num_blobs; b_index++) {
    if (kc_info->blob_fields[b_index] == field_num) break;
  }
...
int ha_tokudb::fast_update(THD *thd, mem_root_deque<Item *> &update_fields,
  ...
  if (error) {
    ...
    if (mapped_error == error) error = HA_ERR_UNSUPPORTED;
  }

exit:
  if (error != 0 && error != ENOTSUP) print_error(error, MYF(0));
...
static bool check_int_result(Item *item) {
  ...
  if (t == Item::INT_ITEM) {
    ...
    if (strcmp(item_func->func_name(), "+") != 0 &&
      ...
      return false;
    if (item_func->argument_count() != 1) return false;
    Item **arguments = item_func->arguments();
    if (arguments[0]->type() != Item::INT_ITEM) return false;
    return true;
  } else
    return false;
}
...
static bool check_x_op_constant(const char *field_name, Item *item,
                                const char *op, Item **item_constant,
                                bool allow_insert_value) {
  if (item->type() != Item::FUNC_ITEM) return false;
  Item_func *item_func = static_cast<Item_func *>(item);
  if (strcmp(item_func->func_name(), op) != 0) return false;
  Item **arguments = item_func->arguments();
  uint n = item_func->argument_count();
  if (n != 2) return false;
  if (arguments[0]->type() != Item::FIELD_ITEM) return false;
  Item_field *arg0 = static_cast<Item_field *>(arguments[0]);
  if (strcmp(field_name, arg0->field_name) != 0) return false;
...
static bool check_decr_floor_expression(Field *lhs_field, Item *item) {
  if (item->type() != Item::FUNC_ITEM) return false;
  Item_func *item_func = static_cast<Item_func *>(item);
  if (strcmp(item_func->func_name(), "if") != 0) return false;
  Item **arguments = item_func->arguments();
  uint n = item_func->argument_count();
  if (n != 3) return false;
  if (!check_x_equal_0(lhs_field->field_name, arguments[0])) return false;
...
static bool check_update_expression(Item *lhs_item, Item *rhs_item,
                                    TABLE *table, bool allow_insert_value) {
  Field *lhs_field = find_field_by_name(table, lhs_item);
  if (lhs_field == NULL) return false;
  if (!lhs_field->part_of_key.is_clear_all()) return false;
  ...
  switch (lhs_type) {
    ...
    case MYSQL_TYPE_LONGLONG:
      if (check_int_result(rhs_item)) return true;
      Item *item_constant;
      if (check_x_op_constant(lhs_field->field_name, rhs_item, "+",
                              &item_constant, allow_insert_value))
        return true;
      if (check_x_op_constant(lhs_field->field_name, rhs_item, "-",
                              &item_constant, allow_insert_value))
        return true;
      if (check_decr_floor_expression(lhs_field, rhs_item)) return true;
      break;
    ...
    case MYSQL_TYPE_BLOB:
      if (rhs_type == Item::STRING_ITEM) return true;
      break;
    default:
      break;
  }
...
static bool check_all_update_expressions(List<Item> &fields, List<Item> &values,
                                         TABLE *table,
                                         bool allow_insert_value) {
...
static bool check_pk_field_equal_constant(Item *item, TABLE *table,
                                          MY_BITMAP *pk_fields) {
  if (item->type() != Item::FUNC_ITEM) return false;
  Item_func *func = static_cast<Item_func *>(item);
  if (strcmp(func->func_name(), "=") != 0) return false;
  uint n = func->argument_count();
  if (n != 2) return false;
  Item **arguments = func->arguments();
...
static bool check_point_update(Item *conds, TABLE *table) {
  ...
  if (conds == NULL) return false;  // no where condition on the update
  ...
  switch (conds->type()) {
    ...
    case Item::COND_ITEM: {
      ...
      if (strcmp(cond_item->func_name(), "and") != 0) break;
      List_iterator<Item> li(*cond_item->argument_list());
      Item *list_item;
      result = true;
  ...
  if (!bitmap_is_clear_all(&pk_fields)) result = false;
...
bool ha_tokudb::check_fast_update(THD *thd, List<Item> &fields,
  ...
  if (clustering_keys_exist(table)) return false;

  if (!check_all_update_expressions(fields, values, table, false)) return false;

  if (!check_point_update(conds, table)) return false;

  return true;
}

static void marshall_varchar_descriptor(tokudb::buffer &b, TABLE *table,
                                        KEY_AND_COL_INFO *kc_info,
                                        uint key_num) {
  b.append_ui<uint32_t>('v');
  b.append_ui<uint32_t>(table->s->null_bytes +
                        kc_info->mcp_info[key_num].fixed_field_size);
...
static void marshall_blobs_descriptor(tokudb::buffer &b, TABLE *table,
                                      KEY_AND_COL_INFO *kc_info) {
...
static void marshall_update(tokudb::buffer &b, Item *lhs_item, Item *rhs_item,
                            TABLE *table, TOKUDB_SHARE *share) {
...
static void count_update_types(Field *lhs_field, uint *num_varchars,
                               uint *num_blobs) {
  switch (lhs_field->type()) {
    case MYSQL_TYPE_VARCHAR:
      *num_varchars += 1;
...
int ha_tokudb::send_update_message(List<Item> &update_fields,
  ...
  DBT key_dbt;
  bool has_null;
  create_dbt_key_from_table(&key_dbt, primary_key, key_buff, table->record[0],
                            &has_null);
  ...
  if (1) {
    ...
    if (num_varchars > 0 || num_blobs > 0) num_updates++;
    if (num_blobs > 0) num_updates++;
  }
  ...
  if (num_varchars > 0 || num_blobs > 0)
    marshall_varchar_descriptor(update_message, table, &share->kc_info,
                                table->s->primary_key);
  if (num_blobs > 0)
  ...
  if (share->num_DBs > table->s->keys + tokudb_test(hidden_primary_key)) {
    ...
    DBT update_dbt;
    memset(&update_dbt, 0, sizeof update_dbt);
    update_dbt.data = update_message.data();
    update_dbt.size = update_message.size();
    error = share->key_file[primary_key]->update(share->key_file[primary_key],
                                                 txn, &key_dbt, &update_dbt, 0);
  }
...
// Return true if it can.
bool ha_tokudb::check_upsert(THD *thd, List<Item> &update_fields,
  ...
  if (table->s->primary_key >= table->s->keys) return false;
...
int ha_tokudb::send_upsert_message(List<Item> &update_fields,
  ...
  DBT key_dbt;
  bool has_null;
  create_dbt_key_from_table(&key_dbt, primary_key, primary_key_buff,
                            table->record[0], &has_null);
