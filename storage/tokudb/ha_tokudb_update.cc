...
static void dump_item(Item* item) {
    ...
    switch (item->type()) {
        ...
        case Item::INSERT_VALUE_ITEM: {
            Item_insert_value* value_item =
                static_cast<Item_insert_value*>(item);
...
static uint32_t var_field_index(TABLE* table,
                                KEY_AND_COL_INFO* kc_info,
                                uint idx,
                                uint field_num) {
...
int ha_tokudb::fast_update(THD* thd,
    ...
    error =
        send_update_message(update_fields, update_values, conds, transaction);
...
static bool check_x_op_constant(const char* field_name,
                                Item* item,
                                const char* op,
                                Item** item_constant,
...
static bool check_update_expression(Item* lhs_item,
                                    Item* rhs_item,
                                    TABLE* table,
                                    bool allow_insert_value) {
    ...
    switch (lhs_type) {
        ...
        case MYSQL_TYPE_LONGLONG:
            if (check_int_result(rhs_item))
                return true;
            Item* item_constant;
            if (check_x_op_constant(lhs_field->field_name,
                                    rhs_item,
                                    "+",
                                    &item_constant,
                                    allow_insert_value))
...
static bool check_all_update_expressions(List<Item>& fields,
                                         List<Item>& values,
                                         TABLE* table,
                                         bool allow_insert_value) {
    ...
    while (1) {
        ...
        if (!check_update_expression(
                lhs_item, rhs_item, table, allow_insert_value))
            return false;
    }
    return true;
}
...
static bool check_pk_field_equal_constant(Item* item,
                                          TABLE* table,
                                          MY_BITMAP* pk_fields) {
    if (item->type() != Item::FUNC_ITEM)
        return false;
    ...
    switch (conds->type()) {
        ...
        case Item::COND_ITEM: {
            ...
            while (result == true && (list_item = li++)) {
                result =
                    check_pk_field_equal_constant(list_item, table, &pk_fields);
            }
            break;
        }
        default:
...
bool ha_tokudb::check_fast_update(THD* thd,
                                  List<Item>& fields,
                                  List<Item>& values,
                                  Item* conds) {
    if (!transaction)
...
static void marshall_varchar_descriptor(tokudb::buffer& b,
                                        TABLE* table,
                                        KEY_AND_COL_INFO* kc_info,
                                        uint key_num) {
    ...
    b.append_ui<uint32_t>(var_offset_bytes == 0 ? 0
                                                : kc_info->num_offset_bytes);
}

static void marshall_blobs_descriptor(tokudb::buffer& b,
                                      TABLE* table,
                                      KEY_AND_COL_INFO* kc_info) {
...
static void marshall_update(tokudb::buffer& b,
                            Item* lhs_item,
                            Item* rhs_item,
                            TABLE* table,
                            TOKUDB_SHARE* share) {
    ...
    if (lhs_field->real_maybe_null()) {
        ...
        field_null_num =
            ((field_num / 8) * 8 + get_null_bit_position(lhs_field->null_bit)) +
            1;
    }
    uint32_t offset;
    void* v_ptr = NULL;
    ...
    switch (lhs_field->type()) {
        ...
        case MYSQL_TYPE_LONGLONG: {
            ...
            field_type =
                lhs_num->unsigned_flag ? UPDATE_TYPE_UINT : UPDATE_TYPE_INT;
            offset = fixed_field_offset(table->s->null_bytes,
                                        &share->kc_info,
                                        table->s->primary_key,
        ...
        case MYSQL_TYPE_VARCHAR: {
            update_operation = '=';
            field_type = lhs_field->binary() ? UPDATE_TYPE_VARBINARY
                                             : UPDATE_TYPE_VARCHAR;
            offset = var_field_index(table,
                                     &share->kc_info,
                                     table->s->primary_key,
                                     lhs_field->field_index);
            v_str = *rhs_item->val_str(&v_str);
        ...
        case MYSQL_TYPE_BLOB: {
            update_operation = '=';
            field_type =
                lhs_field->binary() ? UPDATE_TYPE_BLOB : UPDATE_TYPE_TEXT;
            offset = blob_field_index(
                table, &share->kc_info, lhs_field->field_index);
            v_str = *rhs_item->val_str(&v_str);
            v_length = v_str.length();
...
static void count_update_types(Field* lhs_field,
                               uint* num_varchars,
                               uint* num_blobs) {
    switch (lhs_field->type()) {
        case MYSQL_TYPE_VARCHAR:
...
int ha_tokudb::send_update_message(List<Item>& update_fields,
    ...
    create_dbt_key_from_table(
        &key_dbt, primary_key, key_buff, table->record[0], &has_null);
    ...
    if (1) {
        ...
        while ((lhs_item = lhs_i++)) {
            ...
            Field* lhs_field = find_field_by_name(table, lhs_item);
            assert_always(
                lhs_field);  // we found it before, so this should work
    ...
    if (num_varchars > 0 || num_blobs > 0)
        marshall_varchar_descriptor(
            update_message, table, &share->kc_info, table->s->primary_key);
    if (num_blobs > 0)
    ...
    if (share->num_DBs > table->s->keys + tokudb_test(hidden_primary_key)) {
        ...
        error = share->key_file[primary_key]->update(
            share->key_file[primary_key], txn, &key_dbt, &update_dbt, 0);
    }
...
// Return true if it can.
bool ha_tokudb::check_upsert(THD* thd,
                             List<Item>& update_fields,
                             List<Item>& update_values) {
    if (!transaction)
        return false;
    ...
    if (!check_all_update_expressions(
            update_fields, update_values, table, true))
        return false;
...
int ha_tokudb::send_upsert_message(List<Item>& update_fields,
    ...
    bool has_null;
    create_dbt_key_from_table(
        &key_dbt, primary_key, primary_key_buff, table->record[0], &has_null);
