...
static void get_var_field_info(
    ...
    switch (num_offset_bytes) {
        ...
        case (2):
            data_end_offset =
                uint2korr(var_field_offset_ptr + 2 * var_field_index);
            break;
        default:
            assert_unreachable();
    }
    ...
    if (var_field_index) {
        switch (num_offset_bytes) {
            case (1):
                data_start_offset =
                    (var_field_offset_ptr + var_field_index - 1)[0];
                break;
            case (2):
                data_start_offset =
...
static void get_blob_field_info(uint32_t* start_offset,
...
//
static inline uchar* pack_toku_int(uchar* to_tokudb,
                                   uchar* from_mysql,
                                   uint32_t num_bytes) {
    switch (num_bytes) {
        case (1):
...
//
static inline uchar* unpack_toku_int(uchar* to_mysql,
                                     uchar* from_tokudb,
                                     uint32_t num_bytes) {
    switch (num_bytes) {
        case (1):
...
static inline int cmp_toku_int(uchar* a_buf,
                               uchar* b_buf,
                               bool is_unsigned,
                               uint32_t num_bytes) {
    int ret_val = 0;
...
static inline uchar* pack_toku_binary(uchar* to_tokudb,
                                      uchar* from_mysql,
                                      uint32_t num_bytes) {
...
static inline uchar* unpack_toku_binary(uchar* to_mysql,
                                        uchar* from_tokudb,
                                        uint32_t num_bytes) {
...
static inline int cmp_toku_binary(uchar* a_buf,
...
static inline uchar* pack_toku_varbinary(
    ...
    // copy the string
    //
    memcpy(to_tokudb + length_bytes_in_tokudb,
...
static inline uchar* unpack_toku_varbinary(
    ...
    //
    memcpy(to_mysql + length_bytes_in_mysql,
           from_tokudb + length_bytes_in_tokudb,
           length);
    return from_tokudb + length_bytes_in_tokudb + length;
}

static inline int cmp_toku_varbinary(uchar* a_buf,
                                     uchar* b_buf,
    ...
    ret_val = cmp_toku_binary(
        a_buf + length_bytes, a_len, b_buf + length_bytes, b_len);
...
static uchar* pack_toku_varstring_from_desc(
    ...
    if (length > local_char_length) {
        local_char_length = my_charpos(
            charset, from_desc, from_desc + length, local_char_length);
        set_if_smaller(length, local_char_length);
    }
...
static inline int cmp_toku_string(uchar* a_buf,
                                  uint32_t a_num_bytes,
                                  uchar* b_buf,
                                  uint32_t b_num_bytes,
...
static inline int cmp_toku_varstring(uchar* a_buf,
                                     uchar* b_buf,
                                     uint32_t length_bytes,  // number of bytes
                                                             // used to encode
                                                             // length in a_buf
    ...
    ret_val = cmp_toku_string(
        a_buf + length_bytes, a_len, b_buf + length_bytes, b_len, charset_num);
    *a_bytes_read = a_len + length_bytes;
    *b_bytes_read = b_len + length_bytes;
    return ret_val;
}
    ...
    for (uint i = 0; i < key->user_defined_key_parts; i++) {
        ...
        *pos = (uchar)(type & 255);
        ...
        switch (type) {
            ...
            case (toku_type_varbinary):
                *pos = (uchar)(
                    get_length_bytes_from_max(key->key_part[i].length) & 255);
                pos++;
                break;
            //
...
//
//
static int create_toku_key_descriptor(uchar* buf,
                                      bool is_first_hpk,
                                      KEY* first_key,
                                      bool is_second_hpk,
                                      KEY* second_key) {
...
static inline int compare_toku_field(uchar* a_buf,
                                     uchar* b_buf,
                                     uchar* row_desc,
                                     uint32_t* a_bytes_read,
                                     uint32_t* b_bytes_read,
    ...
    switch (toku_type) {
        ...
        case (toku_type_varbinary):
            ...
            ret_val = cmp_toku_varbinary(
                a_buf, b_buf, length_bytes, a_bytes_read, b_bytes_read);
            row_desc_pos++;
            break;
        case (toku_type_fixstring):
        case (toku_type_varstring):
        case (toku_type_blob):
            ...
            row_desc_pos += sizeof(uint32_t);
            ret_val = cmp_toku_varstring(a_buf,
                                         b_buf,
                                         length_bytes,
                                         charset_num,
                                         a_bytes_read,
...
static uchar* pack_toku_key_field(
    ...
    switch (toku_type) {
        case (toku_type_int):
            ...
            new_pos =
                pack_toku_int(to_tokudb, from_mysql, field->pack_length());
            goto exit;
        case (toku_type_double):
...
static uchar* pack_key_toku_key_field(
    ...
    switch (toku_type) {
        ...
        case (toku_type_fixbinary):
        case (toku_type_fixstring):
            new_pos = pack_toku_key_field(
                to_tokudb, from_mysql, field, key_part_length);
            goto exit;
        case (toku_type_varbinary):
            new_pos = pack_toku_varbinary(to_tokudb,
                                          from_mysql,
...
uchar* unpack_toku_key_field(uchar* to_mysql,
                             uchar* from_tokudb,
                             Field* field,
    ...
    switch (toku_type) {
        case (toku_type_int):
            assert_always(key_part_length == field->pack_length());
            new_pos =
                unpack_toku_int(to_mysql, from_tokudb, field->pack_length());
            goto exit;
        case (toku_type_double):
            assert_always(field->pack_length() == sizeof(double));
            assert_always(key_part_length == sizeof(double));
        ...
        case (toku_type_fixstring):
            ...
            memset(to_mysql + num_bytes_copied,
...
static int tokudb_compare_two_keys(const void* new_key_data,
    ...
    while ((uint32_t)(new_key_ptr - (uchar*)new_key_data) < new_key_size &&
        ...
        ret_val = compare_toku_field(new_key_ptr,
                                     saved_key_ptr,
                                     row_desc_ptr,
...
static int tokudb_prefix_cmp_dbt_key(DB* file,
                                     const DBT* keya,
                                     const DBT* keyb) {
    ...
    bool read_string = false;
    int cmp =
        tokudb_compare_two_keys(keya->data,
                                keya->size,
                                keyb->data,
                                keyb->size,
                                (uchar*)file->cmp_descriptor->dbt.data + 4,
...
static int tokudb_compare_two_key_parts(const void* new_key_data,
                                        const uint32_t new_key_size,
                                        const void* saved_key_data,
                                        const uint32_t saved_key_size,
                                        const void* row_desc,
...
static int tokudb_cmp_dbt_key_parts(DB* file,
                                    const DBT* keya,
                                    const DBT* keyb,
                                    uint max_parts) {
...
static uint32_t pack_desc_pk_info(uchar* buf,
...
static uint32_t pack_desc_offset_info(uchar* buf,
                                      KEY_AND_COL_INFO* kc_info,
                                      uint pk_index,
                                      TABLE_SHARE* table_share,
                                      KEY_PART_INFO* key_part) {
...
static uint32_t pack_desc_key_length_info(uchar* buf,
                                          KEY_AND_COL_INFO* kc_info,
                                          TABLE_SHARE* table_share,
                                          KEY_PART_INFO* key_part) {
    uchar* pos = buf;
...
static uint32_t pack_some_row_info(uchar* buf,
                                   uint pk_index,
                                   TABLE_SHARE* table_share,
                                   KEY_AND_COL_INFO* kc_info) {
    uchar* pos = buf;
...
static uint32_t create_toku_clustering_val_pack_descriptor(
    ...
    for (uint i = 0; i < table_share->fields; i++) {
        ...
        bool col_filtered_in_pk =
            bitmap_is_set(&kc_info->key_filters[pk_index], i);
    ...
    for (uint i = 0; i < table_share->fields; i++) {
        ...
        if (col_filtered && start_range_set) {
            ...
            start_range_set = false;
            uint32_t end_offset =
                kc_info->cp_info[pk_index][last_col].col_pack_val +
            ...
            if (!start_range_set) {
                ...
                uint32_t start_offset =
                    kc_info->cp_info[pk_index][i].col_pack_val;
...
static uint32_t pack_clustering_val_from_desc(uchar* buf,
                                              void* row_desc,
    ...
    while ((uint32_t)(desc_pos - (uchar*)row_desc) < row_desc_size) {
        ...
        if (curr == CK_FIX_RANGE) {
            ...
            get_var_field_info(&start_data_size,
                               &start_data_offset,
                               start,
                               var_src_offset_ptr,
                               num_offset_bytes);
            get_var_field_info(&end_data_size,
                               &end_data_offset,
                               end,
                               var_src_offset_ptr,
                               num_offset_bytes);
    ...
    if (has_blobs) {
        ...
        get_blob_field_info(&start_offset,
        ...
        num_blob_bytes =
            pk_val->size -
            (start_offset + (var_src_data_ptr - null_bytes_src_ptr));
    ...
    if (has_hpk) {
        ...
        for (uint i = 0; i < prim_key->user_defined_key_parts; i++) {
            tmp += pack_desc_pk_info(
                tmp, kc_info, table_share, &prim_key->key_part[i]);
        }
        //
    ...
    for (uint i = 0; i < key_info->user_defined_key_parts; i++) {
        ...
        if (field->null_bit) {
            uint32_t null_offset =
                get_null_offset(table, table->field[field_index]);
        ...
        if (is_col_in_pk) {
            ...
            pos += pack_desc_offset_info(
                pos, kc_info, pk_index, table_share, &curr_kpi);
        }
...
static uint32_t pack_key_from_desc(uchar* buf,
                                   void* row_desc,
                                   uint32_t row_desc_size,
                                   const DBT* pk_key,
                                   const DBT* pk_val) {
    ...
    while ((uint32_t)(desc_pos - (uchar*)row_desc) < row_desc_size) {
        ...
        if (null_bit) {
            ...
            is_field_null =
                (null_bytes_ptr[null_offset] & null_bit) ? true : false;
            if (is_field_null) {
        ...
        if (col_fix_val == COL_FIX_FIELD || col_fix_val == COL_VAR_FIELD ||
            ...
            if (col_fix_val == COL_FIX_FIELD &&
                ...
                memcpy(
                    packed_key_pos, &fixed_field_ptr[col_pack_val], key_length);
                ...
                if (col_fix_val == COL_FIX_FIELD) {
                    ...
                    get_blob_field_info(&blob_offset,
                    ...
                    for (uint32_t i = 0; i < blob_index; i++) {
                        blob_ptr = unpack_toku_field_blob(
                            NULL, blob_ptr, blob_lengths[i], true);
                    }
            ...
            if (col_fix_val == COL_FIX_PK_OFFSET) {
                ...
                if (is_fix_field == COL_FIX_FIELD) {
                    ...
                    if (has_charset == COL_HAS_CHARSET) {
                        packed_key_pos =
                            pack_toku_varstring_from_desc(packed_key_pos,
                                                          data_start,
                                                          key_length,
                                                          data_size,
                                                          charset_num);
