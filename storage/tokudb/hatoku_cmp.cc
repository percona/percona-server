...
static void get_var_field_info(
    ...
    const uchar *var_field_offset_ptr,  // input, pointer to where offset
                                        // information for all var fields begins
    uint32_t num_offset_bytes  // input, number of bytes used to store offsets
                               // starting at var_field_offset_ptr
) {
  ...
  if (var_field_index) {
    switch (num_offset_bytes) {
      ...
      case (2):
        data_start_offset =
            uint2korr(var_field_offset_ptr + 2 * (var_field_index - 1));
        break;
      default:
        assert_unreachable();
    }
...
static void get_blob_field_info(uint32_t *start_offset, uint32_t len_of_offsets,
  ...
  // is at the end of the var stuff (if they exist), if var stuff does not
  // exist then the bottom variable will be 0, and var_field_data_ptr is
...
//
static inline uint32_t get_length_from_var_tokudata(uchar *buf,
...
//
// used to deduce the number of bytes used to store the length of a
// varstring/varbinary in a key field stored in tokudb
//
...
//
static inline uchar *pack_toku_int(uchar *to_tokudb, uchar *from_mysql,
                                   uint32_t num_bytes) {
  switch (num_bytes) {
    case (1):
...
//
static inline uchar *unpack_toku_int(uchar *to_mysql, uchar *from_tokudb,
                                     uint32_t num_bytes) {
  switch (num_bytes) {
    case (1):
...
static inline int cmp_toku_int(uchar *a_buf, uchar *b_buf, bool is_unsigned,
                               uint32_t num_bytes) {
  int ret_val = 0;
  //
...
static inline uchar *pack_toku_binary(uchar *to_tokudb, uchar *from_mysql,
                                      uint32_t num_bytes) {
  memcpy(to_tokudb, from_mysql, num_bytes);
  return to_tokudb + num_bytes;
}

static inline uchar *unpack_toku_binary(uchar *to_mysql, uchar *from_tokudb,
                                        uint32_t num_bytes) {
  memcpy(to_mysql, from_tokudb, num_bytes);
  return from_tokudb + num_bytes;
}

static inline int cmp_toku_binary(uchar *a_buf, uint32_t a_num_bytes,
                                  uchar *b_buf, uint32_t b_num_bytes) {
  int ret_val = 0;
  uint32_t num_bytes_to_cmp =
      (a_num_bytes < b_num_bytes) ? a_num_bytes : b_num_bytes;
...
static inline uchar *pack_toku_varbinary(
    uchar *to_tokudb, uchar *from_mysql,
  ...
  // copy the string
  //
  memcpy(to_tokudb + length_bytes_in_tokudb, from_mysql + length_bytes_in_mysql,
         length);
  return to_tokudb + length + length_bytes_in_tokudb;
}
...
static inline uchar *unpack_toku_varbinary(
    ...
    uint32_t length_bytes_in_tokudb,  // number of bytes used to encode length
                                      // in from_tokudb
    uint32_t length_bytes_in_mysql  // number of bytes used to encode length in
                                    // to_mysql
) {
  uint32_t length =
      get_length_from_var_tokudata(from_tokudb, length_bytes_in_tokudb);
  ...
  //
  memcpy(to_mysql + length_bytes_in_mysql, from_tokudb + length_bytes_in_tokudb,
         length);
  return from_tokudb + length_bytes_in_tokudb + length;
}

static inline int cmp_toku_varbinary(uchar *a_buf, uchar *b_buf,
                                     uint32_t length_bytes,  // number of bytes
                                                             // used to encode
                                                             // length in a_buf
                                                             // and b_buf
  ...
  ret_val =
      cmp_toku_binary(a_buf + length_bytes, a_len, b_buf + length_bytes, b_len);
...
static inline uchar *pack_toku_blob(
    ...
    uint32_t length_bytes_in_mysql,   // number of bytes used to encode the
                                      // length in from_mysql
    uint32_t max_num_bytes, const CHARSET_INFO *charset) {
  uint32_t length = 0;
...
static uchar *pack_toku_varstring_from_desc(
  ...
  //
  local_char_length =
      ((charset->mbmaxlen > 1) ? key_part_length / charset->mbmaxlen
                               : key_part_length);
  if (length > local_char_length) {
    local_char_length =
...
static inline int cmp_toku_string(uchar *a_buf, uint32_t a_num_bytes,
                                  uchar *b_buf, uint32_t b_num_bytes,
  ...
  ret_val = charset->coll->strnncollsp(charset, a_buf, a_num_bytes, b_buf,
                                       b_num_bytes);
  return ret_val;
}

static inline int cmp_toku_varstring(uchar *a_buf, uchar *b_buf,
                                     uint32_t length_bytes,  // number of bytes
                                                             // used to encode
                                                             // length in a_buf
                                                             // and b_buf
  ...
  for (uint i = 0; i < key->user_defined_key_parts; i++) {
    ...
    *pos = (uchar)(type & 255);
    ...
    switch (type) {
      ...
      case (toku_type_varbinary):
        *pos =
            (uchar)(get_length_bytes_from_max(key->key_part[i].length) & 255);
        pos++;
        break;
      //
      // five bytes follow: one for the number of bytes to encode the
      // length,
...
static inline int compare_toku_field(uchar *a_buf, uchar *b_buf,
                                     uchar *row_desc, uint32_t *a_bytes_read,
                                     uint32_t *b_bytes_read,
  ...
  switch (toku_type) {
    case (toku_type_hpk):
      ret_val = tokudb_compare_two_hidden_keys(
          a_buf, TOKUDB_HIDDEN_PRIMARY_KEY_LENGTH, b_buf,
    ...
    case (toku_type_int):
      ...
      ret_val = cmp_toku_int(a_buf, b_buf, is_unsigned, num_bytes);
    ...
    case (toku_type_varbinary):
      ...
      ret_val = cmp_toku_varbinary(a_buf, b_buf, length_bytes, a_bytes_read,
                                   b_bytes_read);
      row_desc_pos++;
      break;
    case (toku_type_fixstring):
    ...
    case (toku_type_blob):
      ...
      row_desc_pos += sizeof(uint32_t);
      ret_val = cmp_toku_varstring(a_buf, b_buf, length_bytes, charset_num,
...
static uchar *pack_toku_key_field(
  ...
  switch (toku_type) {
    case (toku_type_int):
      ...
      new_pos = pack_toku_int(to_tokudb, from_mysql, field->pack_length());
      goto exit;
    case (toku_type_double):
    ...
    case (toku_type_blob):
      new_pos = pack_toku_blob(
          ...
          ((Field_blob *)field)->row_pack_length(),  // only calling this
                                                     // because packlength
                                                     // is returned
          key_part_length, field->charset());
      goto exit;
...
static uchar *pack_key_toku_key_field(
  ...
  switch (toku_type) {
    ...
    case (toku_type_fixstring):
      new_pos =
          pack_toku_key_field(to_tokudb, from_mysql, field, key_part_length);
      goto exit;
    case (toku_type_varbinary):
      new_pos = pack_toku_varbinary(to_tokudb, from_mysql,
                                    2,  // for some idiotic reason, 2
                                        // bytes are always used here,
                                        // regardless of length of field
                                    key_part_length);
      goto exit;
    case (toku_type_varstring):
    case (toku_type_blob):
      new_pos = pack_toku_varstring(
          to_tokudb, from_mysql, get_length_bytes_from_max(key_part_length),
          2,  // for some idiotic reason, 2 bytes are always used here,
              // regardless of length of field
          key_part_length, field->charset());
      goto exit;
    default:
...
uchar *unpack_toku_key_field(uchar *to_mysql, uchar *from_tokudb, Field *field,
  ...
  switch (toku_type) {
    case (toku_type_int):
      assert_always(key_part_length == field->pack_length());
      new_pos = unpack_toku_int(to_mysql, from_tokudb, field->pack_length());
      goto exit;
    case (toku_type_double):
      assert_always(field->pack_length() == sizeof(double));
      assert_always(key_part_length == sizeof(double));
    ...
    case (toku_type_fixstring):
      ...
      new_pos = unpack_toku_varbinary(
          to_mysql, from_tokudb, get_length_bytes_from_max(key_part_length), 0);
      num_bytes_copied =
          new_pos - (from_tokudb + get_length_bytes_from_max(key_part_length));
      assert_always(num_bytes_copied <= num_bytes);
      memset(to_mysql + num_bytes_copied, field->charset()->pad_char,
...
static int tokudb_compare_two_keys(const void *new_key_data,
  ...
  while ((uint32_t)(new_key_ptr - (uchar *)new_key_data) < new_key_size &&
    ...
    ret_val = compare_toku_field(new_key_ptr, saved_key_ptr, row_desc_ptr,
                                 &new_key_field_length, &saved_key_field_length,
                                 &row_desc_field_length, read_string);
    ...
    assert_always((uint32_t)(new_key_ptr - (uchar *)new_key_data) <=
                  new_key_size);
    assert_always((uint32_t)(saved_key_ptr - (uchar *)saved_key_data) <=
                  saved_key_size);
    assert_always((uint32_t)(row_desc_ptr - (uchar *)row_desc) <=
                  row_desc_size);
  }
  new_key_bytes_left =
...
static int simple_memcmp(const DBT *keya, const DBT *keyb) {
  int cmp;
  int num_bytes_cmp = keya->size < keyb->size ? keya->size : keyb->size;
...
static int tokudb_cmp_dbt_key(DB *file, const DBT *keya, const DBT *keyb) {
  ...
  if (file->cmp_descriptor->dbt.size == 0) {
    ...
    cmp = tokudb_compare_two_keys(
        keya->data, keya->size, keyb->data, keyb->size,
        (uchar *)file->cmp_descriptor->dbt.data + 4,
        (*(uint32_t *)file->cmp_descriptor->dbt.data) - 4, false, &read_string);
...
static int tokudb_prefix_cmp_dbt_key(DB *file, const DBT *keya,
  ...
  int cmp = tokudb_compare_two_keys(
      keya->data, keya->size, keyb->data, keyb->size,
      (uchar *)file->cmp_descriptor->dbt.data + 4,
      *(uint32_t *)file->cmp_descriptor->dbt.data - 4, true, &read_string);
  return cmp;
}
...
static int tokudb_compare_two_key_parts(
  ...
  for (uint i = 0; i < max_parts; i++) {
    if (!((uint32_t)(new_key_ptr - (uchar *)new_key_data) < new_key_size &&
          (uint32_t)(saved_key_ptr - (uchar *)saved_key_data) <
              saved_key_size &&
          (uint32_t)(row_desc_ptr - (uchar *)row_desc) < row_desc_size))
      break;
    uint32_t new_key_field_length;
...
static int tokudb_cmp_dbt_key_parts(DB *file, const DBT *keya, const DBT *keyb,
  ...
  return tokudb_compare_two_key_parts(
      keya->data, keya->size, keyb->data, keyb->size,
      (uchar *)file->cmp_descriptor->dbt.data + 4,
      (*(uint32_t *)file->cmp_descriptor->dbt.data) - 4, max_parts);
}
...
#define COPY_OFFSET_TO_BUF                                           \
  memcpy(pos, &kc_info->cp_info[pk_index][field_index].col_pack_val, \
         sizeof(uint32_t));                                          \
  pos += sizeof(uint32_t);
...
static uint32_t pack_desc_offset_info(uchar *buf, KEY_AND_COL_INFO *kc_info,
...
static uint32_t pack_desc_key_length_info(uchar *buf, KEY_AND_COL_INFO *kc_info,
                                          TABLE_SHARE *table_share,
                                          KEY_PART_INFO *key_part) {
  uchar *pos = buf;
  uint16 field_index = key_part->field->field_index;
...
static uint32_t pack_some_row_info(uchar *buf, uint pk_index,
                                   TABLE_SHARE *table_share,
...
static uint32_t create_toku_clustering_val_pack_descriptor(
  ...
  pos += pack_some_row_info(pos, pk_index, table_share, kc_info);
  ...
  for (uint i = 0; i < table_share->fields; i++) {
    ...
    if (col_filtered && start_range_set) {
      ...
      uint32_t end_offset = kc_info->cp_info[pk_index][last_col].col_pack_val +
                            kc_info->field_lengths[last_col];
...
static uint32_t pack_clustering_val_from_desc(uchar *buf, void *row_desc,
  ...
  while ((uint32_t)(desc_pos - (uchar *)row_desc) < row_desc_size) {
    ...
    if (curr == CK_FIX_RANGE) {
      ...
      get_var_field_info(&start_data_size, &start_data_offset, start,
                         var_src_offset_ptr, num_offset_bytes);
      get_var_field_info(&end_data_size, &end_data_offset, end,
                         var_src_offset_ptr, num_offset_bytes);
      length = end_data_offset + end_data_size - start_data_offset;
      //
      // copy the data
      //
      memcpy(var_dest_data_ptr, var_src_data_ptr + start_data_offset, length);
      var_dest_data_ptr += length;

      //
      // put in offset info
      //
      offset_diffs = (end_data_offset + end_data_size) -
                     (uint32_t)(var_dest_data_ptr - orig_var_dest_data_ptr);
  ...
  //
  // copy blobs
  // at this point, var_dest_data_ptr is pointing to the end, where blobs
  // should be located so, we put the blobs at var_dest_data_ptr
  //
  if (has_blobs) {
    uint32_t num_blob_bytes;
    uint32_t start_offset;
    uchar *src_blob_ptr = NULL;
    get_blob_field_info(&start_offset, src_mcp_info.len_of_offsets,
                        var_src_data_ptr, num_offset_bytes);
    src_blob_ptr = var_src_data_ptr + start_offset;
    num_blob_bytes =
        pk_val->size - (start_offset + (var_src_data_ptr - null_bytes_src_ptr));
...
static uint32_t create_toku_secondary_key_pack_descriptor(
  ...
  if (has_hpk) {
    ...
    for (uint i = 0; i < prim_key->user_defined_key_parts; i++) {
      tmp +=
          pack_desc_pk_info(tmp, kc_info, table_share, &prim_key->key_part[i]);
    }
    //
  ...
  for (uint i = 0; i < key_info->user_defined_key_parts; i++) {
    ...
    if (is_col_in_pk) {
      ...
      pos +=
          pack_desc_offset_info(pos, kc_info, pk_index, table_share, &curr_kpi);
    }
    pos += pack_desc_key_length_info(pos, kc_info, table_share, &curr_kpi);
...
static uint32_t skip_key_in_desc(uchar *row_desc) {
  uchar *pos = row_desc;
  uchar col_bin_or_char;
  //
  // skip the byte that states if it is a fix field or var field, we do not
  // care
  //
  pos++;
...
static uint32_t max_key_size_from_desc(void *row_desc, uint32_t row_desc_size) {
  ...
  while ((uint32_t)(desc_pos - (uchar *)row_desc) < row_desc_size) {
    ...
    max_size += key_length;
    max_size += 2;  // 2 bytes for a potential length bytes, we are
                    // upperbounding, does not need to be super tight
...
static uint32_t pack_key_from_desc(uchar *buf, void *row_desc,
  ...
  while ((uint32_t)(desc_pos - (uchar *)row_desc) < row_desc_size) {
    ...
    if (null_bit) {
      //
      // column is NULLable, need to check the null bytes to see if it is
      // NULL
      //
    ...
    //
    if (col_fix_val == COL_FIX_FIELD || col_fix_val == COL_VAR_FIELD ||
      ...
      if (col_fix_val == COL_FIX_FIELD && has_charset == COL_HAS_NO_CHARSET) {
        ...
        get_var_field_info(&data_size, &data_start_offset, col_pack_val,
                           var_field_offset_ptr, num_offset_bytes);

        //
        // length of this field in this row is data_size
        // data is located beginning at var_field_data_ptr +
        // data_start_offset
        //
        packed_key_pos = pack_toku_varbinary_from_desc(
            packed_key_pos, var_field_data_ptr + data_start_offset,
        ...
        if (col_fix_val == COL_FIX_FIELD) {
          ...
          get_blob_field_info(&blob_offset, mcp_info.len_of_offsets,
                              var_field_data_ptr, num_offset_bytes);
          ...
          for (uint32_t i = 0; i < blob_index; i++) {
            blob_ptr =
                unpack_toku_field_blob(NULL, blob_ptr, blob_lengths[i], true);
          }
        ...
        packed_key_pos = pack_toku_varstring_from_desc(
            packed_key_pos, data_start, key_length, data_size, charset_num);
      }
    } else {
      ...
      if (col_fix_val == COL_FIX_PK_OFFSET) {
        ...
        if (is_fix_field == COL_FIX_FIELD) {
          ...
          if (has_charset == COL_HAS_CHARSET) {
            ...
            packed_key_pos = pack_toku_varbinary_from_desc(
                packed_key_pos, data_start, key_length, data_size);
          } else {
            assert_unreachable();
          }
        } else {
