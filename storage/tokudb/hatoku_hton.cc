...
static int tokudb_release_savepoint(handlerton* hton,
                                    THD* thd,
                                    void* savepoint);
#if defined(TOKU_INCLUDE_DISCOVER_FRM) && TOKU_INCLUDE_DISCOVER_FRM
static int tokudb_discover(handlerton* hton,
                           THD* thd,
                           const char* db,
                           const char* name,
                           uchar** frmblob,
                           size_t* frmlen);
static int tokudb_discover2(handlerton* hton,
                            THD* thd,
                            const char* db,
                            const char* name,
                            bool translate_name,
                            uchar** frmblob,
                            size_t* frmlen);
static int tokudb_discover3(handlerton* hton,
                            THD* thd,
                            const char* db,
                            const char* name,
                            char* path,
...
static void tokudb_lock_timeout_callback(DB* db,
...
void toku_hton_assert_fail(const char* expr_as_string,
                           const char* fun,
                           const char* file,
                           int line,
                           int caller_errno) {
...
#error
    ...
    if (r) {  // QQQ
        TOKUDB_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_INIT, "WARNING: flags=%x r=%d", tokudb_env_flags, r);
        // goto error;
    }
    ...
    if (tokudb::sysvars::cache_size) {
        ...
        r = db_env->set_cachesize(
            ...
            (uint32_t)(tokudb::sysvars::cache_size % (1024L * 1024L * 1024L)),
            1);
        if (r) {
    ...
    TOKUDB_TRACE_FOR_FLAGS(TOKUDB_DEBUG_INIT,
    ...
    TOKUDB_TRACE_FOR_FLAGS(
        TOKUDB_DEBUG_INIT, "env open:flags=%x", tokudb_init_flags);
    ...
    r = db_env->set_lock_timeout(db_env,
    ...
    db_env->set_killed_callback(db_env,
                                DEFAULT_TOKUDB_KILLED_TIME,
                                tokudb_get_killed_time_callback,
                                tokudb_killed_callback);

    r = db_env->get_engine_status_num_rows(db_env,
                                           &toku_global_status_max_rows);
    assert_always(r == 0);

    {
        const myf mem_flags = MY_FAE | MY_WME | MY_ZEROFILL |
                              MY_ALLOW_ZERO_PTR | MY_FREE_ON_ERROR;
...
static int tokudb_commit(handlerton* hton, THD* thd, bool all) {
    ...
    if (this_txn) {
        ...
        TOKUDB_TRACE_FOR_FLAGS(TOKUDB_DEBUG_TXN,
...
static int tokudb_rollback(handlerton* hton, THD* thd, bool all) {
    ...
    if (this_txn) {
        TOKUDB_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_TXN, "rollback %u txn %p", all, this_txn);
        tokudb_cleanup_handlers(trx, this_txn);
...
static int tokudb_xa_prepare(handlerton* hton, THD* thd, bool all) {
    ...
    if (txn) {
        ...
        TOKUDB_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_XA, "doing txn prepare:%d:%p", all, txn);
        ...
        if (r == 0 && all && thd->slave_thread) {
            TOKUDB_TRACE_FOR_FLAGS(
                TOKUDB_DEBUG_XA, "zap txn context %u", thd_sql_command(thd));
...
static int tokudb_xa_recover(TOKUDB_UNUSED(handlerton* hton),
    ...
    r = db_env->txn_xa_recover(
        db_env, (TOKU_XA_XID*)xid_list, len, &num_returned, DB_NEXT);
    assert_always(r == 0);
...
static int tokudb_rollback_to_savepoint(handlerton* hton,
    ...
    TOKUDB_TRACE_FOR_FLAGS(
        TOKUDB_DEBUG_TXN, "rollback txn %p", txn_to_rollback);
...
static int tokudb_discover2(handlerton* hton,
    ...
    build_table_filename(
        path, sizeof(path) - 1, db, name, "", translate_name ? 0 : FN_IS_TMP);
...
static int tokudb_discover3(TOKUDB_UNUSED(handlerton* hton),
    ...
    error = status_db->getf_set(
        status_db, txn, 0, &key, smart_dbt_callback_verify_frm, &value);
    if (error) {
        goto cleanup;
    }
...
extern sys_var* intern_find_sys_var(const char* str,
                                    uint length,
                                    bool no_error);
...
static bool tokudb_show_engine_status(THD* thd, stat_print_fn* stat_print) {
    ...
    error = db_env->get_engine_status(db_env,
                                      mystat,
                                      max_rows,
                                      &num_rows,
                                      &redzone_state,
    ...
    if (error == 0) {
        ...
        for (uint64_t row = 0; row < num_rows; row++) {
            switch (mystat[row].type) {
                ...
                case PARCOUNT: {
                    uint64_t v =
                        read_partitioned_counter(mystat[row].value.parcount);
...
static bool tokudb_show_status(TOKUDB_UNUSED(handlerton* hton),
                               THD* thd,
...
void tokudb_split_dname(const char* dname,
...
static void tokudb_pretty_key(const DBT* key,
        ...
        for (uint64_t row = 0; row < num_rows; row++) {
            ...
            switch (status_row.type) {
                ...
                case UNIXTIME: {
                    ...
                    snprintf(status_row.value.datebuf,
                ...
                case TOKUTIME:
                    ...
                    // Reuse the memory in status_row. (It belongs to us).
                    status_row.value.dnum =
                        tokutime_to_seconds(status_row.value.num);
                    status_var.value = (char*)&status_row.value.dnum;
                    break;
                case PARCOUNT: {
                    status_var.type = SHOW_LONGLONG;
                    uint64_t v =
                        read_partitioned_counter(status_row.value.parcount);
                    // Reuse the memory in status_row. (It belongs to us).
