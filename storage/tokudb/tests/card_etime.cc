...
static void load_db(DB_ENV *env, DB *db, uint32_t nrows) {
    ...
    r = env->create_loader(
        env, txn, &loader, db, 1, &db, db_flags, dbt_flags, loader_flags);
    assert(r == 0);
...
static int analyze_key_compare(DB *db __attribute__((unused)),
                               const DBT *a,
                               const DBT *b,
                               uint level) {
    assert(level == 1);
...
static void test_card(DB_ENV *env,
                      DB *db,
                      uint64_t expect_card,
                      uint64_t limit) {
    int r;
