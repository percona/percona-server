...
static void test_keys(DB_ENV *env) {
    ...
    error = tokudb::set_card_in_status(
    ...
    error = tokudb::get_card_from_status(
        status_db, txn, ta_rec_per_keys, current_rec_per_key);
    assert(error == 0);
...
static void test_drop_0(DB_ENV *env) {
    ...
    error = tokudb::get_card_from_status(
        status_db, txn, tb_rec_per_keys, current_rec_per_key);
    assert(error == 0);
...
static void test_drop_1_multiple_parts(DB_ENV *env) {
    ...
    DB *status_db = NULL;
    error = tokudb::create_status(
        env, &status_db, "status_drop_1_multiple_parts", txn);
    assert(error == 0);
...
static void test_add_0_multiple_parts(DB_ENV *env) {
    ...
    DB *status_db = NULL;
    error = tokudb::create_status(
        env, &status_db, "status_add_0_multiple_parts", txn);
    assert(error == 0);
