...
void job_manager_t::operator delete(void *p) { tokudb::memory::free(p); }
job_manager_t::job_manager_t() : _sem(0, 65535), _shutdown(false) {}
...
bool job_manager_t::run_job(job_t *newjob, bool background) {
  ...
  for (jobs_t::iterator it = _background_jobs.begin();
    ...
    if (!job->cancelled() && strcmp(job->key(), jobkey) == 0) {
      ...
      // and it is not running yet, we can cancel the background
      // job and just run this one in the foreground, might have
      // different params, but that is up to the user to figure
      // out.
