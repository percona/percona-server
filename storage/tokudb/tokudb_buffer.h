...
class buffer {
 public:
  inline buffer(void *the_data, size_t s, size_t l)
      : m_data(the_data), m_size(s), m_limit(l), m_is_static(true) {}
  inline buffer() : m_data(NULL), m_size(0), m_limit(0), m_is_static(false) {}
  virtual ~buffer() {
    if (!m_is_static) free(m_data);
  }
  ...
  void append(void *p, size_t s) { memcpy(append_ptr(s), p, s); }
  ...
  void *consume_ptr(size_t s) {
    if (m_size + s > m_limit) return NULL;
    void *p = (char *)m_data + m_size;
    m_size += s;
    return p;
  }
  ...
  void consume(void *p, size_t s) { memcpy(p, consume_ptr(s), s); }
  ...
  // number in *p.
  template <class T>
  size_t consume_ui(T *p) {
    size_t s =
        tokudb::vlq_decode_ui<T>(p, (char *)m_data + m_size, m_limit - m_size);
    m_size += s;
    return s;
  }
  ...
  // different, then readjust the size of the buffer and move things
  // around.
  void replace(size_t offset, size_t old_s, void *new_p, size_t new_s) {
    assert_always(offset + old_s <= m_size);
    if (new_s > old_s) maybe_realloc(new_s - old_s);
 ...
 private:
  // Maybe reallocate the buffer when it becomes full by doubling its
  // size.
  void maybe_realloc(size_t s) {
    if (m_size + s > m_limit) {
      size_t new_limit = m_limit * 2;
      if (new_limit < m_size + s) new_limit = m_size + s;
