...
namespace tokudb {
    ...
    int set_card_in_status(DB* status_db,
                           DB_TXN* txn,
                           uint rec_per_keys,
        ...
        int error = tokudb::metadata::write(
            status_db, hatoku_cardinality, b.data(), b.size(), txn);
        return error;
    }

    // Get the cardinality counters from the status dictionary.
    int get_card_from_status(DB* status_db,
                             DB_TXN* txn,
                             uint rec_per_keys,
        ...
        void* buf = 0;
        size_t buf_size = 0;
        int error = tokudb::metadata::read_realloc(
            status_db, txn, hatoku_cardinality, &buf, &buf_size);
        if (error == 0) {
    ...
    bool find_index_of_key(const char* key_name,
    ...
    int alter_card(DB* status_db,
        ...
        error = get_card_from_status(
            status_db, txn, table_total_key_parts, rec_per_key);
        ...
        if (error == 0) {
            ...
            for (uint i = 0; error == 0 && i < altered_table_share->keys; i++) {
                ...
                uint orig_key_index;
                if (find_index_of_key(altered_table_share->key_info[i].name,
                                      table_share,
                                      &orig_key_index)) {
        ...
        if (error == 0) {
            error = set_card_in_status(status_db,
                                       txn,
