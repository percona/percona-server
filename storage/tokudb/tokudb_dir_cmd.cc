...
namespace tokudb {
    ...
    static int attach(THD *thd,
        ...
        error =
            db_env->dirtool_attach(db_env, txn, dname.c_str(), iname.c_str());
    cleanup:
        if (txn) {
            if (error) {
                abort_txn(txn);
    ...
    static int move(THD *thd,
        ...
        error = db_env->dirtool_move(
            db_env, txn, old_dname.c_str(), new_dname.c_str());
    cleanup:
        if (txn) {
            if (error) {
                abort_txn(txn);
    ...
    void process_dir_cmd(THD *thd,
        ...
        if (!cmd.compare("attach")) {
            if (tokens.size() != 3) {
                if (cb.set_error)
                    cb.set_error(
                        thd, EINVAL, "attach command requires two arguments");
            } else {
            ...
            if (tokens.size() != 2) {
                if (cb.set_error)
                    cb.set_error(
                        thd, EINVAL, "detach command requires one argument");
            } else {
            ...
            if (tokens.size() != 3) {
                if (cb.set_error)
                    cb.set_error(
                        thd, EINVAL, "move command requires two arguments");
            } else {
