...
static int MDL_and_TDC(THD *thd, const char *db, const char *table,
  ...
  if (error) {
    if (cb.set_error)
      cb.set_error(thd, error, "Can't lock table '%s.%s'", db, table);
    return error;
  }
...
static bool parse_db_and_table(const char *dname, std::string /*out*/ &db_name,
...
static int attach(THD *thd, const std::string &dname, const std::string &iname,
                  const dir_cmd_callbacks &cb) {
  int error;
  DB_TXN *txn = NULL;
  ...
  if (trx && trx->sub_sp_level) parent_txn = trx->sub_sp_level;
  error = txn_begin(db_env, parent_txn, &txn, 0, thd);
  if (error) goto cleanup;

  error = db_env->dirtool_attach(db_env, txn, dname.c_str(), iname.c_str());
cleanup:
  if (txn) {
    if (error) {
      abort_txn(txn);
...
static int detach(THD *thd, const std::string &dname,
                  const dir_cmd_callbacks &cb) {
  int error;
  DB_TXN *txn = NULL;
  DB_TXN *parent_txn = NULL;
  ...
  error = db_env->dirtool_detach(db_env, txn, dname.c_str());
cleanup:
  if (txn) {
    if (error) {
      abort_txn(txn);
...
static int move(THD *thd, const std::string &old_dname,
                const std::string &new_dname, const dir_cmd_callbacks &cb) {
  int error;
  DB_TXN *txn = NULL;
  DB_TXN *parent_txn = NULL;
  tokudb_trx_data *trx = NULL;
  ...
  error =
      db_env->dirtool_move(db_env, txn, old_dname.c_str(), new_dname.c_str());
cleanup:
  if (txn) {
    if (error) {
      abort_txn(txn);
...
void process_dir_cmd(THD *thd, const char *cmd_str,
  ...
  if (!cmd.compare("attach")) {
    if (tokens.size() != 3) {
      if (cb.set_error)
        cb.set_error(thd, EINVAL, "attach command requires two arguments");
    } else {
      int r = attach(thd, tokens[1], tokens[2], cb);
      if (r && cb.set_error) cb.set_error(thd, r, "Attach command error");
    }
  } else if (!cmd.compare("detach")) {
    if (tokens.size() != 2) {
      if (cb.set_error)
        cb.set_error(thd, EINVAL, "detach command requires one argument");
    } else {
      int r = detach(thd, tokens[1], cb);
      if (r && cb.set_error) cb.set_error(thd, r, "detach command error");
    }
  } else if (!cmd.compare("move")) {
    if (tokens.size() != 3) {
      if (cb.set_error)
        cb.set_error(thd, EINVAL, "move command requires two arguments");
    } else {
      int r = move(thd, tokens[1], tokens[2], cb);
      if (r && cb.set_error) cb.set_error(thd, r, "move command error");
    }
  } else {
    if (cb.set_error)
      cb.set_error(thd, ENOENT, "Unknown command '%s'", cmd.c_str());
  }
