...
namespace tokudb {
    namespace information_schema {
        ...
        int trx_callback(
            ...
            table->field[2]->store(tnow >= start_time ? tnow - start_time : 0,
                                   false);
        ...
        int trx_fill_table(THD* thd,
            ...
            if (!tokudb_hton_initialized) {
                ...
                error =
                    db_env->iterate_live_transactions(db_env, trx_callback, &e);
                if (error)
        ...
        int trx_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        int lock_waits_callback(DB* db,
            ...
            table->field[3]->store(
                left_str.ptr(), left_str.length(), system_charset_info);
            String right_str;
            tokudb_pretty_right_key(right_key, &right_str);
            table->field[4]->store(
                right_str.ptr(), right_str.length(), system_charset_info);
            table->field[5]->store(start_time, false);

            String database_name, table_name, dictionary_name;
            tokudb_split_dname(
                dname, database_name, table_name, dictionary_name);
            table->field[6]->store(database_name.c_ptr(),
                                   database_name.length(),
                                   system_charset_info);
            table->field[7]->store(
                table_name.c_ptr(), table_name.length(), system_charset_info);
            table->field[8]->store(dictionary_name.c_ptr(),
        ...
        int lock_waits_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        int locks_callback(DB_TXN* txn,
            ...
            while (error == 0 &&
                   iterate_locks(&db, &left_key, &right_key, locks_extra) ==
                       0) {
                table->field[0]->store(txn_id, false);
                table->field[1]->store(client_id, false);
                ...
                size_t dname_length = strlen(dname);
                table->field[2]->store(
                    dname, dname_length, system_charset_info);
                ...
                tokudb_split_dname(
                    dname, database_name, table_name, dictionary_name);
                table->field[5]->store(database_name.c_ptr(),
                                       database_name.length(),
                                       system_charset_info);
                table->field[6]->store(table_name.c_ptr(),
                                       table_name.length(),
                                       system_charset_info);
                table->field[7]->store(dictionary_name.c_ptr(),
                                       dictionary_name.length(),
                                       system_charset_info);
        ...
        int locks_fill_table(THD* thd,
            ...
            if (!tokudb_hton_initialized) {
                ...
                error = db_env->iterate_live_transactions(
                    db_env, locks_callback, &e);
                if (error)
                    my_error(ER_GET_ERRNO, MYF(0), error, tokudb_hton_name);
            }
        ...
        int locks_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        int report_file_map(TABLE* table, THD* thd) {
            ...
            while (error == 0) {
                error = tmp_cursor->c_get(
                    tmp_cursor, &curr_key, &curr_val, DB_NEXT);
                if (!error) {
                    // We store the NULL terminator in the directory so it's
                    // included in the size. See #5789 Recalculate and check
                    // just to be safe.
                    const char* dname = (const char*)curr_key.data;
                    size_t dname_len = strlen(dname);
                    assert(dname_len == curr_key.size - 1);
                    table->field[0]->store(
                        dname, dname_len, system_charset_info);

                    const char* iname = (const char*)curr_val.data;
                    size_t iname_len = strlen(iname);
                    assert(iname_len == curr_val.size - 1);
                    table->field[1]->store(
                        iname, iname_len, system_charset_info);

                    // split the dname
                    String database_name, table_name, dictionary_name;
                    tokudb_split_dname(
                        dname, database_name, table_name, dictionary_name);
                    table->field[2]->store(database_name.c_ptr(),
                                           database_name.length(),
                                           system_charset_info);
                    table->field[3]->store(table_name.c_ptr(),
                                           table_name.length(),
                                           system_charset_info);
                    table->field[4]->store(dictionary_name.c_ptr(),
                                           dictionary_name.length(),
                                           system_charset_info);
        ...
        int file_map_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        int report_fractal_tree_info_for_db(const DBT* dname,
                                            const DBT* iname,
                                            TABLE* table,
                                            THD* thd) {
            int error;
            ...
            error =
                db->open(db, NULL, (char*)dname->data, NULL, DB_BTREE, 0, 0666);
            if (error) {
                goto exit;
            }
            ...
            // We store the NULL terminator in the directory so it's included in
            // the size. See #5789 Recalculate and check just to be safe.
            {
                size_t dname_len = strlen((const char*)dname->data);
                assert(dname_len == dname->size - 1);
                table->field[0]->store(
                    (char*)dname->data, dname_len, system_charset_info);
                size_t iname_len = strlen((const char*)iname->data);
                assert(iname_len == iname->size - 1);
                table->field[1]->store(
                    (char*)iname->data, iname_len, system_charset_info);
            }
            ...
            {
                String database_name, table_name, dictionary_name;
                tokudb_split_dname((const char*)dname->data,
                                   database_name,
                                   table_name,
                                   dictionary_name);
                table->field[6]->store(database_name.c_ptr(),
                                       database_name.length(),
                                       system_charset_info);
                table->field[7]->store(table_name.c_ptr(),
                                       table_name.length(),
                                       system_charset_info);
                table->field[8]->store(dictionary_name.c_ptr(),
                                       dictionary_name.length(),
        ...
        int fractal_tree_info_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        // As a compromise, we'll do some mallocs inside the lock on the first
        // call, but everything else should be fast.
        int report_fractal_tree_block_map_iterator(uint64_t checkpoint_count,
            ...
            assert(num_rows > 0);
            if (e->num_rows == 0) {
                e->checkpoint_counts = (uint64_t*)tokudb::memory::malloc(
                    num_rows * (sizeof *e->checkpoint_counts),
                    MYF(MY_WME | MY_ZEROFILL | MY_FAE));
                e->blocknums = (int64_t*)tokudb::memory::malloc(
                    num_rows * (sizeof *e->blocknums),
                    MYF(MY_WME | MY_ZEROFILL | MY_FAE));
                e->diskoffs = (int64_t*)tokudb::memory::malloc(
                    num_rows * (sizeof *e->diskoffs),
                    MYF(MY_WME | MY_ZEROFILL | MY_FAE));
                e->sizes = (int64_t*)tokudb::memory::malloc(
        ...
        int report_fractal_tree_block_map_for_db(const DBT* dname,
                                                 const DBT* iname,
                                                 TABLE* table,
                                                 THD* thd) {
            int error;
            DB* db;
            // avoid struct initializers so that we can compile with older gcc
            // versions
            ...
            error = db->iterate_fractal_tree_block_map(
                db, report_fractal_tree_block_map_iterator, &e);
            {
                int close_error = db->close(db, 0);
                if (!error) {
                    error = close_error;
            ...
            // If not, we should have gotten an error and skipped this section
            // of code
            assert(e.i == e.num_rows);
            for (int64_t i = 0; error == 0 && i < e.num_rows; ++i) {
                // We store the NULL terminator in the directory so it's
                // included in the size. See #5789 Recalculate and check just to
                // be safe.
                size_t dname_len = strlen((const char*)dname->data);
                assert(dname_len == dname->size - 1);
                table->field[0]->store(
                ...
                if (e.diskoffs[i] == diskoff_unused ||
                    e.diskoffs[i] == freelist_null) {
        ...
        int fractal_tree_block_map_fill_table(THD* thd,
                                              TABLE_LIST* tables,
                                              TOKUDB_UNUSED(Item* cond)) {
            TOKUDB_DBUG_ENTER("");
            int error;
        ...
        int fractal_tree_block_map_done(TOKUDB_UNUSED(void* p)) { return 0; }
        ...
        void background_job_status_callback(
            ...
            if (job->user_scheduled())
                table->field[5]->store(
                    "USER", strlen("USER"), system_charset_info);
            else
                table->field[5]->store(
                    "AUTO", strlen("AUTO"), system_charset_info);
        ...
        int report_background_job_status(TABLE* table, THD* thd) {
            int error = 0;
            background_job_status_extra extra = {thd, table};
            tokudb::background::_job_manager->iterate_jobs(
                background_job_status_callback, &extra);
            return error;
        }
        ...
        int background_job_status_done(TOKUDB_UNUSED(void* p)) { return 0; }
