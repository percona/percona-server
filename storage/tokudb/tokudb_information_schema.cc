...
int trx_callback(DB_TXN *txn,
                 ...
                 TOKUDB_UNUSED(void *locks_extra), void *extra) {
  ...
  if (!error && thd_killed(thd)) error = ER_QUERY_INTERRUPTED;
  return error;
}
...
int trx_fill_table(THD *thd, Table_ref *tables, TOKUDB_UNUSED(Item *cond)) {
  ...
  if (!tokudb_hton_initialized) {
    ...
    if (error) my_error(ER_GET_ERRNO, MYF(0), error, tokudb_hton_name);
  }
...
int lock_waits_callback(DB *db, uint64_t requesting_txnid, const DBT *left_key,
  ...
  table->field[3]->store(left_str.ptr(), left_str.length(),
                         system_charset_info);
  String right_str;
  tokudb_pretty_right_key(right_key, &right_str);
  table->field[4]->store(right_str.ptr(), right_str.length(),
                         system_charset_info);
  ...
  table->field[6]->store(database_name.c_ptr(), database_name.length(),
                         system_charset_info);
  table->field[7]->store(table_name.c_ptr(), table_name.length(),
                         system_charset_info);
  table->field[8]->store(dictionary_name.c_ptr(), dictionary_name.length(),
                         system_charset_info);
...
int lock_waits_fill_table(THD *thd, Table_ref *tables,
  ...
  if (!tokudb_hton_initialized) {
    ...
    error =
        db_env->iterate_pending_lock_requests(db_env, lock_waits_callback, &e);
    if (error) my_error(ER_GET_ERRNO, MYF(0), error, tokudb_hton_name);
  }
...
int locks_callback(DB_TXN *txn, iterate_row_locks_callback iterate_locks,
  ...
  while (error == 0 &&
    ...
    String database_name, table_name, dictionary_name;
    tokudb_split_dname(dname, database_name, table_name, dictionary_name);
    table->field[5]->store(database_name.c_ptr(), database_name.length(),
                           system_charset_info);
    table->field[6]->store(table_name.c_ptr(), table_name.length(),
                           system_charset_info);
    table->field[7]->store(dictionary_name.c_ptr(), dictionary_name.length(),
                           system_charset_info);
...
int report_file_map(TABLE *table, THD *thd) {
  ...
  while (error == 0) {
    ...
    if (!error) {
      ...
      String database_name, table_name, dictionary_name;
      tokudb_split_dname(dname, database_name, table_name, dictionary_name);
      table->field[2]->store(database_name.c_ptr(), database_name.length(),
                             system_charset_info);
      table->field[3]->store(table_name.c_ptr(), table_name.length(),
                             system_charset_info);
      table->field[4]->store(dictionary_name.c_ptr(), dictionary_name.length(),
                             system_charset_info);
...
int report_fractal_tree_info_for_db(const DBT *dname, const DBT *iname,
  ...
  error = db->get_fractal_tree_info64(db, &bt_num_blocks_allocated,
                                      &bt_num_blocks_in_use, &bt_size_allocated,
                                      &bt_size_in_use);
  if (error) {
    goto exit;
  }
  ...
  {
    String database_name, table_name, dictionary_name;
    tokudb_split_dname((const char *)dname->data, database_name, table_name,
                       dictionary_name);
    table->field[6]->store(database_name.c_ptr(), database_name.length(),
                           system_charset_info);
    table->field[7]->store(table_name.c_ptr(), table_name.length(),
...
int report_fractal_tree_info(TABLE *table, THD *thd) {
  ...
  while (error == 0) {
    ...
    if (!error) {
      ...
      if (error) error = 0;  // ignore read uncommitted errors
    }
    if (!error && thd_killed(thd)) error = ER_QUERY_INTERRUPTED;
  }
  if (error == DB_NOTFOUND) {
...
int report_fractal_tree_block_map_iterator(uint64_t checkpoint_count,
  ...
  struct report_fractal_tree_block_map_iterator_extra_t *e =
      static_cast<struct report_fractal_tree_block_map_iterator_extra_t *>(
          iter_extra);
  ...
  if (e->num_rows == 0) {
    ...
    e->blocknums = (int64_t *)tokudb::memory::malloc(
        num_rows * (sizeof *e->blocknums), MYF(MY_WME | MY_ZEROFILL | MY_FAE));
    e->diskoffs = (int64_t *)tokudb::memory::malloc(
        num_rows * (sizeof *e->diskoffs), MYF(MY_WME | MY_ZEROFILL | MY_FAE));
    e->sizes = (int64_t *)tokudb::memory::malloc(
        num_rows * (sizeof *e->sizes), MYF(MY_WME | MY_ZEROFILL | MY_FAE));
    e->num_rows = num_rows;
  }
...
int report_fractal_tree_block_map(TABLE *table, THD *thd) {
  ...
  while (error == 0) {
    error = tmp_cursor->c_get(tmp_cursor, &curr_key, &curr_val, DB_NEXT);
    if (!error) {
      error = report_fractal_tree_block_map_for_db(&curr_key, &curr_val, table,
                                                   thd);
    }
    if (!error && thd_killed(thd)) error = ER_QUERY_INTERRUPTED;
  }
...
void background_job_status_callback(
    tokudb::background::job_manager_t::job_t *job, void *extra) {
...
int report_background_job_status(TABLE *table, THD *thd) {
  ...
  tokudb::background::_job_manager->iterate_jobs(background_job_status_callback,
                                                 &extra);
  return error;
}
