...
namespace tokudb {
    namespace metadata {
        ...
        // returns 0 if successful.
        int write(DB* status_db,
                  HA_METADATA_KEY curr_key_data,
                  void* val,
                  size_t val_size,
                  DB_TXN* txn) {
            return tokudb::metadata::write_low(status_db,
                                               &curr_key_data,
        ...
        int remove(DB* status_db, HA_METADATA_KEY curr_key_data, DB_TXN* txn) {
            return tokudb::metadata::remove_low(
                status_db, &curr_key_data, sizeof(curr_key_data), txn);
        }
        ...
        int create(DB_ENV* env,
            ...
            if (error == 0) {
                error =
                    status_db->set_pagesize(status_db, status_dict_pagesize);
            }
            if (error == 0) {
                error = status_db->open(status_db,
                                        txn,
                                        name,
                                        NULL,
                                        DB_BTREE,
        ...
        int open(DB_ENV* env,
            ...
            error = db_create(&status_db, env, 0);
            if (error == 0) {
                error = status_db->open(
                    status_db, txn, name, NULL, DB_BTREE, DB_THREAD, 0);
            }
            if (error == 0) {
                uint32_t pagesize = 0;
                error = status_db->get_pagesize(status_db, &pagesize);
                if (error == 0 && pagesize > status_dict_pagesize) {
                    error = status_db->change_pagesize(status_db,
                                                       status_dict_pagesize);
                }
            }
            if (error == 0) {
                *status_db_ptr = status_db;
        ...
        int strip_frm_data(DB_ENV* env) {
            ...
            fprintf(stderr,
                    "TokuDB strip_frm_data : Beginning stripping process.\n");
            ...
            while (1) {
                ...
                if (strstr(iname, "_status_")) {
                    ...
                    void* p = NULL;
                    size_t sz;
                    r = tokudb::metadata::read_realloc(
                    ...
                    r = tokudb::metadata::remove(
                        status_db, hatoku_frm_data, txn);
                    if (r != 0) {
                        fprintf(
                            stderr,
