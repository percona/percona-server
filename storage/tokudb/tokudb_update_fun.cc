...
// Update operation codes.  These codes get stuffed into update messages, so
// they can not change. The operations are currently stored in a single byte in
// the update message, so only 256 operations are supported.  When we need more,
// we can use the last (255) code to indicate that the operation code is
// expanded beyond 1 byte.
enum {
...
//     uint32 field type
//     uint32 null num 0 => not nullable, otherwise encoded as field_null_num +
//     1 uint32 offset uint32 value length = N uint8  value [ N ]
//
...
//
static inline void set_overall_null_position(uchar *data, uint32_t pos,
                                             bool is_null) {
  uint32_t offset = pos / 8;
  uchar remainder = pos % 8;
  uchar null_bit = 1 << remainder;
...
static inline void copy_null_bits(uint32_t start_old_pos,
                                  uint32_t start_new_pos, uint32_t num_bits,
...
static inline void copy_var_fields(
    ...
    uint32_t new_num_offset_bytes, uint32_t *num_data_bytes_written,
  ...
  for (uint32_t i = 0; i < num_var_fields; i++) {
    ...
    get_var_field_info(&field_len, &start_read_offset, curr_old,
                       old_var_field_offset_ptr, old_num_offset_bytes);
    ...
    curr_new_var_field_data_ptr = write_var_field(
        curr_new_var_field_offset_ptr, curr_new_var_field_data_ptr,
        new_var_field_data_ptr, data_to_copy, field_len, new_num_offset_bytes);
  ...
  *num_offset_bytes_written = (uint32_t)(curr_new_var_field_offset_ptr -
                                         start_new_var_field_offset_ptr);
}

static inline uint32_t copy_toku_blob(uchar *to_ptr, uchar *from_ptr,
                                      uint32_t len_bytes, bool skip) {
  uint32_t length = 0;
  if (!skip) {
...
static int tokudb_hcad_update_fun(const DBT *old_val, const DBT *extra,
  ...
  new_val_data = (uchar *)tokudb::memory::malloc(max_num_bytes, MYF(MY_FAE));
  ...
  for (uint32_t i = 0; i < num_columns; i++) {
    ...
    if (nullable) {
      ...
      copy_null_bits(curr_old_null_pos, curr_new_null_pos, num_bits,
      ...
      if (op_type == COL_DROP) {
        ...
        extra_pos++;
        set_overall_null_position(new_null_bytes, null_bit_position,
                                  is_null_default);
    ...
    if (col_type == COL_FIXED) {
      ...
      memcpy(new_fixed_field_ptr + curr_new_fixed_offset,
             old_fixed_field_ptr + curr_old_fixed_offset, num_bytes_to_copy);
      ...
      if (op_type == COL_DROP) {
        ...
        if (is_null_default) {
          ...
          memcpy(new_fixed_field_ptr + curr_new_fixed_offset, extra_pos,
                 col_size);
          extra_pos += col_size;
        }
      ...
      copy_var_fields(curr_old_num_var_field, num_var_fields_to_copy,
                      old_var_field_offset_ptr, old_num_offset_bytes,
                      ...
                      // number of offset bytes used in new row
                      new_num_offset_bytes, &num_data_bytes_written,
  ...
  memcpy(new_fixed_field_ptr + curr_new_fixed_offset,
         old_fixed_field_ptr + curr_old_fixed_offset, num_bytes_left);
  ...
  copy_var_fields(curr_old_num_var_field, num_var_fields_to_copy,
                  old_var_field_offset_ptr, old_num_offset_bytes,
                  curr_new_var_field_data_ptr, curr_new_var_field_offset_ptr,
                  // pointer to beginning of var fields in new row
                  new_var_field_data_ptr,
                  // pointer to beginning of var fields in old row
                  old_var_field_data_ptr,
  ...
  get_blob_field_info(&start_blob_offset, old_len_of_offsets,
                      old_var_field_data_ptr, old_num_offset_bytes);
    ...
    while ((extra_pos - extra_pos_start) < extra->size) {
      ...
      for (uint32_t i = 0; i < num_blobs_to_copy; i++) {
        uint32_t num_bytes_written =
            copy_toku_blob(curr_new_var_field_data_ptr, curr_old_blob_ptr,
                           len_bytes[curr_old_blob + i], false);
      ...
      if (op_type == COL_DROP) {
        ...
        uint32_t num_bytes = copy_toku_blob(NULL, curr_old_blob_ptr,
                                            len_bytes[curr_old_blob], true);
        curr_old_blob++;
        ...
        extra_pos++;
        uint32_t num_bytes = copy_toku_blob(curr_new_var_field_data_ptr,
                                            extra_pos, new_len_bytes, false);
        curr_new_blob++;
...
// in the extra.
static int tokudb_expand_variable_offsets(const DBT *old_val, const DBT *extra,
                                          void (*set_val)(const DBT *new_val,
                                                          void *set_extra),
                                          void *set_extra) {
  int error = 0;
  ...
  if (old_val != NULL) {
    ...
    uchar *new_val_ptr = (uchar *)tokudb::memory::malloc(
        number_of_offsets + old_val->size, MYF(MY_FAE));
    if (!new_val_ptr) {
      error = ENOMEM;
      goto cleanup;
    }
...
static int tokudb_expand_int_field(const DBT *old_val, const DBT *extra,
                                   void (*set_val)(const DBT *new_val,
                                                   void *set_extra),
                                   void *set_extra) {
  int error = 0;
  ...
  uint8_t operation;
  extra_val.consume(&operation, sizeof operation);
  assert_always(operation == UPDATE_OP_EXPAND_INT ||
  ...
  if (old_val != NULL) {
    ...
    // allocate space for the new val's data
    uchar *new_val_ptr = (uchar *)tokudb::memory::malloc(
        old_val->size + (new_length - old_length), MYF(MY_FAE));
    if (!new_val_ptr) {
      error = ENOMEM;
      goto cleanup;
    }
    ...
    switch (operation) {
      case UPDATE_OP_EXPAND_INT:
        ...
        memset(new_val_ptr, (old_val_ptr[old_length - 1] & 0x80) ? 0xff : 0x00,
               new_length);
...
static int tokudb_expand_char_field(const DBT *old_val, const DBT *extra,
                                    void (*set_val)(const DBT *new_val,
                                                    void *set_extra),
                                    void *set_extra) {
  int error = 0;
  ...
  uint8_t operation;
  extra_val.consume(&operation, sizeof operation);
  assert_always(operation == UPDATE_OP_EXPAND_CHAR ||
...
class var_fields {
 public:
  inline var_fields() {}
  inline void init_var_fields(uint32_t var_offset, uint32_t offset_bytes,
                              uint32_t bytes_per_offset,
                              tokudb::buffer *val_buffer) {
    assert_always(bytes_per_offset == 0 || bytes_per_offset == 1 ||
                  bytes_per_offset == 2);
...
uint32_t var_fields::read_offset(uint32_t var_index) {
  uint32_t offset = 0;
  m_val_buffer->read(&offset, m_bytes_per_offset,
...
void var_fields::write_offset(uint32_t var_index, uint32_t new_offset) {
  m_val_buffer->write(&new_offset, m_bytes_per_offset,
                      m_var_offset + var_index * m_bytes_per_offset);
}
...
void var_fields::update_offsets(uint32_t var_index, uint32_t old_s,
                                uint32_t new_s) {
  assert_always(var_index < m_num_fields);
...
void var_fields::replace(uint32_t var_index, void *new_val_ptr,
...
class blob_fields {
 public:
  ...
  void start_blobs(uint32_t offset) { m_blob_offset = offset; }
  void replace(uint32_t blob_index, uint32_t length, void *p);

  void expand_length(uint32_t blob_index, uint8_t old_length_length,
...
void blob_fields::write_length(uint32_t offset, size_t size,
                               uint32_t new_length) {
...
void blob_fields::replace(uint32_t blob_index, uint32_t new_length,
                          void *new_value) {
  assert_always(blob_index < m_num_blobs);
  ...
  // replace the data
  m_val_buffer->replace(offset + blob_length, old_length, new_value,
                        new_length);
...
void blob_fields::expand_length(uint32_t blob_index, uint8_t old_length_length,
                                uint8_t new_length_length) {
  assert_always(blob_index < m_num_blobs);
  ...
  // expand the length
  m_val_buffer->replace(offset, old_length_length, &blob_length,
                        new_length_length);
}
...
class value_map {
 public:
  ...
  void init_var_fields(uint32_t var_offset, uint32_t offset_bytes,
                       uint32_t bytes_per_offset) {
    m_var_fields.init_var_fields(var_offset, offset_bytes, bytes_per_offset,
                                 m_val_buffer);
  }
  ...
  void replace_fixed(uint32_t the_offset, uint32_t field_null_num,
                     void *new_val_ptr, uint32_t new_val_length) {
    m_val_buffer->replace(the_offset, new_val_length, new_val_ptr,
                          new_val_length);
    maybe_clear_null(field_null_num);
  }

  // Replace the value of a variable length field
  void replace_varchar(uint32_t var_index, uint32_t field_null_num,
                       void *new_val_ptr, uint32_t new_val_length) {
  ...
  void replace_blob(uint32_t blob_index, uint32_t field_null_num,
                    void *new_val_ptr, uint32_t new_val_length) {
  ...
  void expand_blob_lengths(uint32_t num_blob, const uint8_t *old_length,
                           const uint8_t *new_length);

  void int_op(uint32_t operation, uint32_t the_offset, uint32_t length,
              uint32_t field_null_num, tokudb::buffer &old_val,
              void *extra_val);

  void uint_op(uint32_t operation, uint32_t the_offset, uint32_t length,
               uint32_t field_null_num, tokudb::buffer &old_val,
               void *extra_val);
...
void value_map::int_op(uint32_t operation, uint32_t the_offset, uint32_t length,
                       uint32_t field_null_num, tokudb::buffer &old_val,
                       void *extra_val) {
  assert_always(the_offset + length <= m_val_buffer->size());
  assert_always(the_offset + length <= old_val.size());
  assert_always(length == 1 || length == 2 || length == 3 || length == 4 ||
                length == 8);
...
// Update an unsigned field: unsigned newval@offset = old_val@offset OP
// extra_val
void value_map::uint_op(uint32_t operation, uint32_t the_offset,
                        uint32_t length, uint32_t field_null_num,
...
void value_map::expand_blob_lengths(uint32_t num_blob,
...
static int tokudb_expand_blobs(const DBT *old_val_dbt, const DBT *extra,
  ...
  if (old_val_dbt != NULL) {
    ...
    // set the new val
    DBT new_val_dbt;
    memset(&new_val_dbt, 0, sizeof new_val_dbt);
...
static void apply_1_updates(tokudb::value_map &vd, tokudb::buffer &old_val,
  ...
  for (; num_updates > 0; num_updates--) {
    ...
    switch (field_type) {
      case UPDATE_TYPE_INT:
        if (update_operation == '=')
          vd.replace_fixed(the_offset, field_null_num, extra_val_ptr,
                           extra_val_length);
        else
          vd.int_op(update_operation, the_offset, extra_val_length,
                    field_null_num, old_val, extra_val_ptr);
        break;
      case UPDATE_TYPE_UINT:
        if (update_operation == '=')
          ...
          vd.uint_op(update_operation, the_offset, extra_val_length,
                     field_null_num, old_val, extra_val_ptr);
        break;
...
static int tokudb_update_1_fun(const DBT *old_val_dbt, const DBT *extra,
                               void (*set_val)(const DBT *new_val_dbt,
                                               void *set_extra),
                               void *set_extra) {
  tokudb::buffer extra_val(extra->data, 0, extra->size);
  ...
  if (old_val_dbt != NULL) {
    ...
    tokudb::buffer old_val(old_val_dbt->data, old_val_dbt->size,
                           old_val_dbt->size);
    ...
    tokudb::value_map vd(&new_val);
    vd.init_var_fields(m_var_field_offset, m_var_offset_bytes,
                       m_bytes_per_offset);
...
// new value.
static int tokudb_upsert_1_fun(const DBT *old_val_dbt, const DBT *extra,
                               void (*set_val)(const DBT *new_val_dbt,
                                               void *set_extra),
                               void *set_extra) {
  tokudb::buffer extra_val(extra->data, 0, extra->size);
...
static void apply_2_updates(tokudb::value_map &vd, tokudb::buffer &old_val,
                            tokudb::buffer &extra_val) {
    ...
    if (update_operation == 'v') {
      ...
      switch (field_type) {
        ...
        case UPDATE_TYPE_VARCHAR:
          if (update_operation == '=')
            vd.replace_varchar(the_offset, field_null_num, extra_val_ptr,
                               extra_val_length);
          else
            assert_unreachable();
          break;
        ...
        case UPDATE_TYPE_BLOB:
          if (update_operation == '=')
            vd.replace_blob(the_offset, field_null_num, extra_val_ptr,
                            extra_val_length);
          else
            assert_unreachable();
          break;
...
static int tokudb_update_2_fun(const DBT *old_val_dbt, const DBT *extra,
                               void (*set_val)(const DBT *new_val_dbt,
                                               void *set_extra),
                               void *set_extra) {
  tokudb::buffer extra_val(extra->data, 0, extra->size);
...
// Otherwise, apply the update operations to the old value, and then set the
// new value.
static int tokudb_upsert_2_fun(const DBT *old_val_dbt, const DBT *extra,
                               void (*set_val)(const DBT *new_val_dbt,
                                               void *set_extra),
                               void *set_extra) {
  tokudb::buffer extra_val(extra->data, 0, extra->size);
...
int tokudb_update_fun(TOKUDB_UNUSED(DB *db), TOKUDB_UNUSED(const DBT *key),
