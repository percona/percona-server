...
//
static inline void set_overall_null_position(uchar* data,
                                             uint32_t pos,
                                             bool is_null) {
    uint32_t offset = pos / 8;
    uchar remainder = pos % 8;
...
static inline void copy_var_fields(
    ...
    for (uint32_t i = 0; i < num_var_fields; i++) {
        ...
        curr_new_var_field_data_ptr =
            write_var_field(curr_new_var_field_offset_ptr,
                            curr_new_var_field_data_ptr,
...
static inline uint32_t copy_toku_blob(uchar* to_ptr,
                                      uchar* from_ptr,
                                      uint32_t len_bytes,
                                      bool skip) {
    uint32_t length = 0;
...
static int tokudb_hcad_update_fun(const DBT* old_val,
    ...
    for (uint32_t i = 0; i < num_columns; i++) {
        ...
        if (nullable) {
            ...
            copy_null_bits(curr_old_null_pos,
                           curr_new_null_pos,
                           num_bits,
                           old_null_bytes,
                           new_null_bytes);
            ...
            if (op_type == COL_DROP) {
                ...
                set_overall_null_position(
                    new_null_bytes, null_bit_position, is_null_default);
        ...
        if (col_type == COL_FIXED) {
            ...
            if (op_type == COL_DROP) {
                ...
                if (is_null_default) {
                    // copy zeroes
                    memset(new_fixed_field_ptr + curr_new_fixed_offset,
                           0,
                           col_size);
                } else {
            ...
            copy_var_fields(curr_old_num_var_field,
    ...
    get_blob_field_info(&start_blob_offset,
                        old_len_of_offsets,
                        old_var_field_data_ptr,
                        old_num_offset_bytes);
        ...
        while ((extra_pos - extra_pos_start) < extra->size) {
            ...
            for (uint32_t i = 0; i < num_blobs_to_copy; i++) {
                uint32_t num_bytes_written =
                    copy_toku_blob(curr_new_var_field_data_ptr,
                                   curr_old_blob_ptr,
            ...
            if (op_type == COL_DROP) {
                ...
                uint32_t num_bytes = copy_toku_blob(
                    NULL, curr_old_blob_ptr, len_bytes[curr_old_blob], true);
                curr_old_blob++;
...
namespace tokudb {
    ...
    class var_fields {
       public:
        inline var_fields() {}
        inline void init_var_fields(uint32_t var_offset,
        ...
        uint32_t end_offset();
        void replace(uint32_t var_index,
                     void* new_val_ptr,
    ...
    void var_fields::update_offsets(uint32_t var_index,
                                    uint32_t old_s,
                                    uint32_t new_s) {
        assert_always(var_index < m_num_fields);
        if (old_s == new_s)
    ...
    void var_fields::replace(uint32_t var_index,
                             void* new_val_ptr,
                             uint32_t new_val_length) {
    ...
    class blob_fields {
       public:
        blob_fields() {}
        void init_blob_fields(uint32_t num_blobs,
        ...
        void expand_length(uint32_t blob_index,
    ...
    void blob_fields::write_length(uint32_t offset,
                                   size_t size,
                                   uint32_t new_length) {
    ...
    void blob_fields::replace(uint32_t blob_index,
                              uint32_t new_length,
                              void* new_value) {
        assert_always(blob_index < m_num_blobs);
        ...
        m_val_buffer->replace(
            offset + blob_length, old_length, new_value, new_length);

        // write the new length
        write_length(offset, blob_length, new_length);
    }

    void blob_fields::expand_length(uint32_t blob_index,
                                    uint8_t old_length_length,
                                    uint8_t new_length_length) {
        assert_always(blob_index < m_num_blobs);
        ...
        // expand the length
        m_val_buffer->replace(
            offset, old_length_length, &blob_length, new_length_length);
    }
    ...
    class value_map {
       public:
        ...
        void replace_blob(uint32_t blob_index,
                          uint32_t field_null_num,
                          void* new_val_ptr,
                          uint32_t new_val_length) {
        ...
        void expand_blob_lengths(uint32_t num_blob,
                                 const uint8_t* old_length,
                                 const uint8_t* new_length);

        void int_op(uint32_t operation,
                    uint32_t the_offset,
                    uint32_t length,
                    uint32_t field_null_num,
        ...
        void uint_op(uint32_t operation,
                     uint32_t the_offset,
                     uint32_t length,
                     uint32_t field_null_num,
                     tokudb::buffer& old_val,
        ...
        bool is_null(uint32_t null_num, uchar* null_bytes) {
            ...
            if (null_num) {
                if (null_num & (1 << 31))
                    ...
                    null_num -= 1;
                field_is_null =
                    is_overall_null_position_set(null_bytes, null_num);
            }
        ...
        void maybe_clear_null(uint32_t null_num) {
            if (null_num) {
                if (null_num & (1 << 31))
                    ...
                    null_num -= 1;
                set_overall_null_position(
                    (uchar*)m_val_buffer->data(), null_num, false);
            }
        }
    ...
    void value_map::int_op(uint32_t operation,
                           uint32_t the_offset,
                           uint32_t length,
                           uint32_t field_null_num,
                           tokudb::buffer& old_val,
        ...
        assert_always(length == 1 || length == 2 || length == 3 ||
                      length == 4 || length == 8);
    ...
    // extra_val
    void value_map::uint_op(uint32_t operation,
                            uint32_t the_offset,
                            uint32_t length,
                            uint32_t field_null_num,
                            tokudb::buffer& old_val,
    ...
    void value_map::expand_blob_lengths(uint32_t num_blob,
        ...
        for (uint32_t i = 0; i < num_blob; i++) {
            if (new_length[i] > current_length[i]) {
                ...
                m_blob_fields.start_blobs(m_var_fields.end_offset());
                m_blob_fields.expand_length(
                    i, current_length[i], new_length[i]);
...
static int tokudb_expand_blobs(const DBT* old_val_dbt,
    ...
    if (old_val_dbt != NULL) {
        ...
        vd.init_var_fields(
            var_field_offset, var_offset_bytes, bytes_per_offset);
    ...
    for (; num_updates > 0; num_updates--) {
        ...
        switch (field_type) {
            case UPDATE_TYPE_INT:
                if (update_operation == '=')
                    vd.replace_fixed(the_offset,
                                     field_null_num,
                                     extra_val_ptr,
                                     extra_val_length);
                else
                    vd.int_op(update_operation,
                              the_offset,
                              extra_val_length,
                              field_null_num,
                              old_val,
...
static int tokudb_update_1_fun(const DBT* old_val_dbt,
    ...
    if (old_val_dbt != NULL) {
        ...
        tokudb::buffer old_val(
            old_val_dbt->data, old_val_dbt->size, old_val_dbt->size);
        ...
        tokudb::value_map vd(&new_val);
        vd.init_var_fields(
            m_var_field_offset, m_var_offset_bytes, m_bytes_per_offset);
    ...
    switch (operation) {
        ...
        case UPDATE_OP_EXPAND_VARIABLE_OFFSETS:
            error = tokudb_expand_variable_offsets(
                old_val, extra, set_val, set_extra);
            break;
        case UPDATE_OP_EXPAND_INT:
        case UPDATE_OP_EXPAND_UINT:
        ...
        case UPDATE_OP_EXPAND_BINARY:
            error =
                tokudb_expand_char_field(old_val, extra, set_val, set_extra);
            break;
