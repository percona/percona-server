...
TEST_F(Keyring_vault_api_test, StoreStoreStoreFetchRemove)
{
  ...
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key").c_str(), "AES", "Robert",
                               sample_key_data.c_str(),
                               sample_key_data.length()));
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key1").c_str(), "AES",
                               "Robert", key_data1.c_str(),
                               key_data1.length()));
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key2").c_str(), "AES",
                               "Robert", key_data2.c_str(),
                               key_data2.length()));
  char * key_type;
  size_t key_len;
  void * key;
...
TEST_F(Keyring_vault_api_test, StoreValidTypes)
{
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key").c_str(), "AES", "Robert",
                               sample_key_data.c_str(),
                               sample_key_data.length()));
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key3").c_str(), "RSA",
                               "Robert", sample_key_data.c_str(),
                               sample_key_data.length()));
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key4").c_str(), "DSA",
                               "Robert", sample_key_data.c_str(),
                               sample_key_data.length()));
  // clean up
  EXPECT_FALSE(mysql_key_remove((uuid + "Robert_key").c_str(), "Robert"));
...
{
  EXPECT_FALSE(mysql_key_generate((uuid + "Robert_key").c_str(), "AES",
                                  "Robert", 128));
  char * key_type;
  size_t key_len;
  void * key;
...
{
  EXPECT_FALSE(mysql_key_store((uuid + "Robert_key").c_str(), "AES", NULL,
                               sample_key_data.c_str(),
                               sample_key_data.length() + 1));
  char * key_type;
  size_t key_len;
  void * key;
  EXPECT_FALSE(mysql_key_fetch((uuid + "Robert_key").c_str(), &key_type, NULL,
                               &key, &key_len));
  EXPECT_STREQ("AES", key_type);
  ...
  my_free(key);
  key= NULL;
  EXPECT_TRUE(mysql_key_store((uuid + "Robert_key").c_str(), "RSA", NULL,
                              sample_key_data.c_str(),
                              sample_key_data.length() + 1));
  EXPECT_FALSE(mysql_key_store((uuid + "Kamil_key").c_str(), "AES", NULL,
                               sample_key_data.c_str(),
                               sample_key_data.length() + 1));
  EXPECT_FALSE(mysql_key_fetch((uuid + "Kamil_key").c_str(), &key_type, NULL,
                               &key, &key_len));
  EXPECT_STREQ("AES", key_type);
  EXPECT_EQ(key_len, sample_key_data.length() + 1);
  ASSERT_TRUE(memcmp((char *)key, sample_key_data.c_str(), key_len) == 0);
  ...
  EXPECT_FALSE(mysql_key_store((uuid + "Artur_key").c_str(), "AES", "Artur",
                               sample_key_data.c_str(),
                               sample_key_data.length() + 1));
  EXPECT_FALSE(mysql_key_fetch((uuid + "Artur_key").c_str(), &key_type,
                               "Artur", &key, &key_len));
  EXPECT_STREQ("AES", key_type);
  EXPECT_EQ(key_len, sample_key_data.length() + 1);
  ASSERT_TRUE(memcmp((char *)key, sample_key_data.c_str(), key_len) == 0);
...
int main(int argc, char **argv)
{
  ...
  keyring__api_unittest::keyring_filename.reset(
      new char[strlen("./keyring_vault.conf") + 1]);
  strcpy(keyring__api_unittest::keyring_filename.get(),
         "./keyring_vault.conf");
