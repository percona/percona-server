...
class Vault_keys_container_test : public Vault_test_base {
 ...
 protected:
  void SetUp() override {
    ...
    std::string sample_key_id = get_vault_env()->get_uuid() + "Roberts_key";
    sample_key =
        new Vault_key(sample_key_id.c_str(), "AES", "Robert",
                      sample_key_data.c_str(), sample_key_data.length());

    vault_keys_container.reset(new Vault_keys_container(get_logger()));
    vault_parser = new Vault_parser_composer(get_logger());
    vault_curl = new Vault_curl(get_logger(), vault_parser, 0);
  }

  void TearDown() override { Vault_test_base::TearDown(); }

 protected:
  std::unique_ptr<Vault_keys_container> vault_keys_container;
...
TEST_F(Vault_keys_container_test, InitWithCorrectCredential) {
  ...
  EXPECT_FALSE(vault_keys_container->init(
      vault_io, get_vault_env()->get_default_conf_file_name()));
...
TEST_F(Vault_keys_container_test, InitWithEmptyCredentialFile) {
  std::string empty_conf = get_vault_env()->get_invalid_conf_file_name();
...
TEST_F(Vault_keys_container_test, StoreFetchRemove) {
  ...
  Vault_key key_id((get_vault_env()->get_uuid() + "Roberts_key").c_str(),
  ...
  std::string expected_key_signature =
      get_vault_env()->get_key_signature("Roberts_key", "Robert");
...
TEST_F(Vault_keys_container_test, FetchNotExisting) {
  ...
  EXPECT_FALSE(vault_keys_container->init(
      keyring_io, get_vault_env()->get_default_conf_file_name()));
  keyring::Key key_id((get_vault_env()->get_uuid() + "Roberts_key").c_str(),
                      nullptr, "Robert", nullptr, 0);
  IKey *fetched_key = vault_keys_container->fetch_key(&key_id);
  ASSERT_TRUE(fetched_key == nullptr);
  delete sample_key;  // unused in this test
...
TEST_F(Vault_keys_container_test, StoreFetchNotExistingDelete) {
  ...
  EXPECT_FALSE(vault_keys_container->store_key(sample_key));
  ASSERT_TRUE(vault_keys_container->get_number_of_keys() == 1);
  keyring::Key key_id((get_vault_env()->get_uuid() + "NotRoberts_key").c_str(),
                      nullptr, "NotRobert", nullptr, 0);
  IKey *fetched_key = vault_keys_container->fetch_key(&key_id);
  ASSERT_TRUE(fetched_key == nullptr);
  ASSERT_TRUE(vault_keys_container->get_number_of_keys() == 1);
...
TEST_F(Vault_keys_container_test, StoreStoreStoreFetchRemove) {
  ...
  Vault_key *key1 =
      new Vault_key((get_vault_env()->get_uuid() + "Roberts_key1").c_str(),
  ...
  Vault_key *key2 =
      new Vault_key((get_vault_env()->get_uuid() + "Roberts_key2").c_str(),
  ...
  Vault_key *key3 =
      new Vault_key((get_vault_env()->get_uuid() + "Roberts_key3").c_str(),
  ...
  Vault_key key2_id((get_vault_env()->get_uuid() + "Roberts_key2").c_str(),
                    nullptr, "Robert", nullptr, 0);
  IKey *fetched_key = vault_keys_container->fetch_key(&key2_id);

  ASSERT_TRUE(fetched_key != nullptr);
  std::string expected_key_signature =
      get_vault_env()->get_key_signature("Roberts_key2", "Robert");
  EXPECT_STREQ(fetched_key->get_key_signature()->c_str(),
               expected_key_signature.c_str());
  EXPECT_EQ(fetched_key->get_key_signature()->length(),
            expected_key_signature.length());
  ...
  Vault_key key3_id((get_vault_env()->get_uuid() + "Roberts_key3").c_str(),
                    nullptr, "Robert", nullptr, 0);
...
TEST_F(Vault_keys_container_test,
  ...
  expected_key_signature =
      Vault_environment::get_key_signature_ex("", "percona_innodb1_2_3:0", "");
...
class Vault_keys_container_with_mocked_io_test : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    std::string sample_key_data_ = "Robi";
    sample_key = new Vault_key(
        (get_vault_env()->get_uuid() + "Roberts_key").c_str(), "AES", "Robert",
        sample_key_data_.c_str(), sample_key_data_.length());
    credential_file_url = get_vault_env()->get_non_existing_conf_file_name();
  }
...
bool Vault_keys_container_with_mocked_io_test::is_env_configured = false;
...
TEST_F(Vault_keys_container_with_mocked_io_test,
  ...
  vault_keys_container.reset(new Vault_keys_container(logger));
...
TEST_F(Vault_keys_container_with_mocked_io_test,
  ...
  Vault_key *key_from_list =
      new Vault_key((get_vault_env()->get_uuid() + "key1").c_str(), nullptr,
                    "Robert", nullptr, 0);
  ...
  Vault_key key_fetched((get_vault_env()->get_uuid() + "key1").c_str(), nullptr,
                        "Robert", nullptr, 0);
...
TEST_F(Vault_keys_container_with_mocked_io_test,
  ...
  Vault_key key_to_fetch((get_vault_env()->get_uuid() + "key1").c_str(),
                         nullptr, "Robert", nullptr, 0);
  ...
  Vault_key key_to_re_fetch((get_vault_env()->get_uuid() + "key1").c_str(),
                            nullptr, "Robert", nullptr, 0);
