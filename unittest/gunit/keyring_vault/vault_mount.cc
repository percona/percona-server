...
namespace keyring {
  ...
  bool Vault_mount::init(std::string *keyring_storage_url, std::string *secret_mount_point)
  {
  ...
  bool Vault_mount::mount_secret_backend()
  {
    ...
    if ((list= curl_slist_append(list, token_header.c_str())) == NULL ||
        ...
        (curl_res= curl_easy_setopt(curl, CURLOPT_HTTPHEADER, list)) !=
            CURLE_OK ||
        (curl_res= curl_easy_setopt(curl, CURLOPT_ERRORBUFFER,
                                    curl_errbuf)) != CURLE_OK ||
